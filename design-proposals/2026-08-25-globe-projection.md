...
- Core: `style::Style::getProjection()` / `setProjection(ProjectionDefinition)` alongside `getLight()` / `setLight()`. `CustomLayerRenderParameters` gains projection fields, appended after the existing ones. That is source-compatible and an ABI change for custom-layer hosts built against older headers, so phase 9 ships in a release that already carries a major bump or the fields go in a pointer-to-struct member; either is fine, and the choice is called out in that PR.
