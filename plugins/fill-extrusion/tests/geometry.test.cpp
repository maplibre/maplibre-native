...
mln_plugin_status capture(const mln_plugin_descriptor_v1* d, char*, size_t) { descriptor = d; return MLN_PLUGIN_STATUS_OK; }
struct Geometry {
    const mln_plugin_layer_type_v1* layer;
    void* layout = nullptr;
    mln_plugin_bucket_v1 bucket{sizeof(bucket)};
    Geometry() {
...
TEST(FillExtrusionGeometry, RoofHolesWallsRangesAndQueries) {
    Geometry g;
    const mln_plugin_tile_point_v1 points[] = {{0,0},{100,0},{100,100},{0,100},{0,0},
    ...
    for (const auto& seg : std::vector<mln_plugin_segment_v1>(g.bucket.drawables[0].segments,
                          g.bucket.drawables[0].segments + g.bucket.drawables[0].segment_count)) {
        for (size_t i = seg.index_offset; i < seg.index_offset + seg.index_length; i += 3) {
            ...
            if (a[5] == 1) roofArea += std::abs((b[0]-a[0])*(c[1]-a[1])-(b[1]-a[1])*(c[0]-a[0]))/2;
            else ++walls;
        }
    }
    EXPECT_DOUBLE_EQ(7500, roofArea); // Hole must not be roofed over.
    EXPECT_EQ(16u, walls);
    mln_plugin_query_context_v1 context{}; context.struct_size = sizeof(context); context.pixels_to_tile_units = 1;
    ...
    const uint32_t invalidPaths[] = {0,11,10}; f.path_offsets = invalidPaths;
...
TEST(FillExtrusionGeometry, SegmentsNeverOverflow16BitIndices) {
    ...
    for (int i = 0; i < 4000; ++i) {
        ...
        points.insert(points.end(), {{x,y},{int16_t(x+30),y},{int16_t(x+30),int16_t(y+30)},
                                    {x,int16_t(y+30)},{x,y}});
        paths.push_back(uint32_t(points.size()));
    }
    mln_plugin_feature_v1 f{sizeof(f), MLN_PLUGIN_GEOMETRY_POLYGON, 7, points.data(), points.size(), paths.data(), paths.size()-1};
    ASSERT_EQ(MLN_PLUGIN_STATUS_OK, g.layer->layout_feature(g.layout, &f));
    g.finish();
