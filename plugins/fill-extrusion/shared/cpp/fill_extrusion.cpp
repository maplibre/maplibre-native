...
namespace {
template <size_t N> constexpr mln_plugin_string str(const char (&s)[N]) { return {s, N - 1}; }
...
bool validFeature(const mln_plugin_feature_v1* f) {
    if (!f || f->struct_size < sizeof(*f) || f->geometry_type != MLN_PLUGIN_GEOMETRY_POLYGON ||
        (f->point_count && !f->points) || !f->path_offsets || f->point_count > UINT32_MAX ||
        f->path_offsets[0] != 0 || f->path_offsets[f->path_count] != f->point_count) return false;
...
void reservePrimitive(Layout& l, uint32_t count) {
    ...
    if (l.segments.empty() || l.segments.back().vertex_length + count > UINT16_MAX)
        l.segments.push_back({sizeof(mln_plugin_segment_v1), uint32_t(l.vertices.size()), uint32_t(l.indices.size()), 0, 0});
}
...
void emitPolygon(Layout& l, Polygon& poly) {
    ...
    if (poly.size() > 501) {
        std::nth_element(poly.begin() + 1, poly.begin() + 501, poly.end(),
            [](const Ring& a, const Ring& b) { return std::abs(area(a)) > std::abs(area(b)); });
        poly.resize(501);
    }
    ...
    for (const auto& ring : poly) {
        ...
        for (size_t i = 0; i + 1 < ring.size(); ++i) {
            ...
            l.vertices.insert(l.vertices.end(), {vertex(b, 0, nx, ny, 0), vertex(b, 1, nx, ny, 0),
                                                 vertex(a, 0, nx, ny, 0), vertex(a, 1, nx, ny, 0)});
            for (uint32_t n : {0u, 2u, 1u, 1u, 2u, 3u}) l.indices.push_back(uint16_t(start + n));
            seg.vertex_length += 4; seg.index_length += 6;
        }
    }
...
} catch (...) { return MLN_PLUGIN_STATUS_CALLBACK_ERROR; }
mln_plugin_status layoutFeature(void* instance, const mln_plugin_feature_v1* f) try {
    ...
    if (l.vertices.size() != first)
        l.ranges.push_back({sizeof(mln_plugin_feature_vertex_range_v1), f->feature_index, 1, first, uint32_t(l.vertices.size() - first)});
    return MLN_PLUGIN_STATUS_OK;
} catch (...) { return MLN_PLUGIN_STATUS_CALLBACK_ERROR; }
mln_plugin_status finishLayout(void* instance, mln_plugin_bucket_v1* out) {
    ...
    auto& l = *static_cast<Layout*>(instance);
    l.stream = {sizeof(l.stream), 0, reinterpret_cast<const uint8_t*>(l.vertices.data()), l.vertices.size() * sizeof(Vertex), uint32_t(l.vertices.size()), sizeof(Vertex)};
    ...
    out->vertex_streams = &l.stream; out->vertex_stream_count = l.vertices.empty() ? 0 : 1;
    out->indices = l.indices.data(); out->index_count = l.indices.size();
    out->drawables = &l.drawable; out->drawable_count = l.indices.empty() ? 0 : 1;
    out->feature_vertex_ranges = l.ranges.data(); out->feature_vertex_range_count = l.ranges.size();
    out->query_radius = 0;
    return MLN_PLUGIN_STATUS_OK;
}
void destroyLayout(void* p) { delete static_cast<Layout*>(p); }
...
mln_plugin_status updateUniform(const mln_plugin_uniform_context_v1* c, uint32_t id, uint8_t* out, size_t size) {
    if (!c || c->struct_size < sizeof(*c) || id || !out || size != sizeof(Uniforms)) return MLN_PLUGIN_STATUS_INVALID_ARGUMENT;
    Uniforms u{};
...
mln_plugin_status evaluateLayer(const mln_plugin_property_value_v1* props, size_t count, mln_plugin_layer_evaluation_v1* out) {
    ...
    if (const auto* a = property(props, count, "fill-extrusion-translate-anchor"))
        out->translation_anchor_viewport = std::string_view(a->data.string_value.data, a->data.string_value.size) == "viewport";
    return MLN_PLUGIN_STATUS_OK;
}
float queryRadius(const mln_plugin_property_statistics_v1*, size_t, const mln_plugin_property_value_v1* props, size_t count) {
    if (const auto* t = property(props, count, "fill-extrusion-translate")) return std::hypot(t->data.float2_value.x, t->data.float2_value.y);
    return 0;
}
...
bool intersects(const Point& a, const Point& b, const Point& c, const Point& d) {
    if (std::max(a[0],b[0]) < std::min(c[0],d[0]) || std::max(c[0],d[0]) < std::min(a[0],b[0]) ||
        std::max(a[1],b[1]) < std::min(c[1],d[1]) || std::max(c[1],d[1]) < std::min(a[1],b[1])) return false;
    return side(a,b,c)*side(a,b,d) <= 0 && side(c,d,a)*side(c,d,b) <= 0;
}
uint8_t queryFeature(const mln_plugin_feature_v1* f, const mln_plugin_tile_point_v1* q, size_t count,
                    const mln_plugin_query_context_v1* c, const mln_plugin_property_value_v1* props, size_t propCount) try {
    ...
    double tx = evaluated.translation.x, ty = evaluated.translation.y;
    if (evaluated.translation_anchor_viewport) {
        const auto cosine = std::cos(-c->bearing), sine = std::sin(-c->bearing);
        const auto x = tx * cosine - ty * sine; ty = tx * sine + ty * cosine; tx = x;
    }
    Ring query;
    for (size_t i = 0; i < count; ++i) query.push_back({q[i].x - tx*c->pixels_to_tile_units, q[i].y - ty*c->pixels_to_tile_units});
    for (const auto& poly : polygons(*f)) {
        for (const auto& p : query) {
            bool in = false;
            for (const auto& ring : poly) if (contains(p, ring)) in = !in;
            if (in) return 1;
        }
        if (query.size() > 1) for (const auto& ring : poly) {
            for (const auto& p : ring) if (contains(p, query)) return 1;
            for (size_t i = 0; i + 1 < ring.size(); ++i)
...
constexpr mln_plugin_value number(float f) {
    mln_plugin_value v{sizeof(v), MLN_PLUGIN_VALUE_FLOAT, {}}; v.data.float_value = f; return v;
}
constexpr mln_plugin_value boolean(bool b) {
    mln_plugin_value v{sizeof(v), MLN_PLUGIN_VALUE_BOOLEAN, {}}; v.data.boolean_value = b; return v;
}
constexpr mln_plugin_value black() {
    mln_plugin_value v{sizeof(v), MLN_PLUGIN_VALUE_COLOR, {}}; v.data.color_value = {0,0,0,1}; return v;
}
constexpr mln_plugin_value translation() {
    mln_plugin_value v{sizeof(v), MLN_PLUGIN_VALUE_FLOAT2, {}}; return v;
}
constexpr mln_plugin_value anchor() {
    mln_plugin_value v{sizeof(v), MLN_PLUGIN_VALUE_STRING, {}}; v.data.string_value = str("map"); return v;
}
...
constexpr mln_plugin_property_descriptor_v1 paint(mln_plugin_string name, mln_plugin_value value, bool data = false) {
    ...
    p.struct_size = sizeof(p); p.name = name; p.type = value.type; p.default_value = value;
    p.expression_capabilities = MLN_PLUGIN_EXPRESSION_CAMERA;
    if (data) p.expression_capabilities |= MLN_PLUGIN_EXPRESSION_FEATURE | MLN_PLUGIN_EXPRESSION_COMPOSITE | MLN_PLUGIN_EXPRESSION_FEATURE_STATE;
    p.supports_transitions = value.type != MLN_PLUGIN_VALUE_BOOLEAN && value.type != MLN_PLUGIN_VALUE_STRING;
    if (value.type == MLN_PLUGIN_VALUE_STRING) { p.enum_values = anchors; p.enum_value_count = 2; }
    return p;
}
...
constexpr mln_plugin_shader_property_binding_v1 bindings[] = {
    {sizeof(mln_plugin_shader_property_binding_v1), str("fill-extrusion-color"), MLN_PLUGIN_PROPERTY_ENCODING_COLOR, 0, offsetof(Uniforms,color), 2, 3, 0, offsetof(Uniforms,interpolation)},
    {sizeof(mln_plugin_shader_property_binding_v1), str("fill-extrusion-base"), MLN_PLUGIN_PROPERTY_ENCODING_FLOAT, 0, offsetof(Uniforms,base), 4, 4, 0, offsetof(Uniforms,interpolation)+4},
    {sizeof(mln_plugin_shader_property_binding_v1), str("fill-extrusion-height"), MLN_PLUGIN_PROPERTY_ENCODING_FLOAT, 0, offsetof(Uniforms,height), 5, 5, 0, offsetof(Uniforms,interpolation)+8},
    {sizeof(mln_plugin_shader_property_binding_v1), str("fill-extrusion-opacity"), MLN_PLUGIN_PROPERTY_ENCODING_FLOAT, 0, offsetof(Uniforms,opacity), 6, 6, 0, offsetof(Uniforms,interpolation)+12},
    {sizeof(mln_plugin_shader_property_binding_v1), str("fill-extrusion-vertical-gradient"), MLN_PLUGIN_PROPERTY_ENCODING_BOOLEAN_FLOAT, 0, offsetof(Uniforms,gradient), 7, 7, 0, offsetof(Uniforms,interpolation)+16},
};
constexpr mln_plugin_shader_source_v1 source{sizeof(source), MLN_PLUGIN_BACKEND_VULKAN, str(vertexSource), str(fragmentSource), {}, {}};
constexpr mln_plugin_uniform_block_descriptor_v1 uniform{sizeof(uniform), 0, str("ExtrusionUniforms"), sizeof(Uniforms), MLN_PLUGIN_SHADER_STAGE_VERTEX, MLN_PLUGIN_UNIFORM_DRAWABLE};
constexpr mln_plugin_shader_descriptor_v1 shader{sizeof(shader), str("solid"), &source, 1, attributes, std::size(attributes), &uniform, 1, bindings, std::size(bindings)};
...
const mln_plugin_layer_type_v1 layer = [] {
    mln_plugin_layer_type_v1 l{}; l.struct_size = sizeof(l); l.layer_type = str("fill-extrusion");
    l.backend_mask = MLN_PLUGIN_BACKEND_VULKAN; l.properties = properties; l.property_count = std::size(properties);
    l.geometry_type_mask = MLN_PLUGIN_GEOMETRY_POLYGON; l.shaders = &shader; l.shader_count = 1;
    l.create_layout = createLayout; l.layout_feature = layoutFeature; l.finish_layout = finishLayout; l.destroy_layout = destroyLayout;
    l.query_feature = queryFeature; l.get_query_radius = queryRadius; l.update_uniform_block = updateUniform;
    l.replace_builtin = 1; l.is_3d = 1; l.draw_passes = passes; l.draw_pass_count = std::size(passes);
    l.evaluate_layer = evaluateLayer; l.enable_near_clipped_matrix = 1; return l;
}();
const mln_plugin_descriptor_v1 descriptor{sizeof(descriptor), MLN_PLUGIN_ABI_VERSION_1, str("org.maplibre.fill-extrusion"), str("0.1.0"), 1, 1, &layer, 1};
} // namespace
extern "C" mln_plugin_status mln_fill_extrusion_register(mln_plugin_register_function_v1 host, char* error, size_t capacity) {
