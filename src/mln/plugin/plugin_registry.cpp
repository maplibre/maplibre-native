...
mln_plugin_status PluginRegistry::registerPlugin(const mln_plugin_descriptor_v1& descriptor, std::string& error) {
    ...
    for (const auto& type : newLayerTypes) {
        ...
        factories.push_back(std::make_unique<PluginLayerFactory>(type));
    }
    for (const auto& property : newProperties) {
...
uint8_t mln_plugin_is_registered_v1(const char* pluginID) try {
...
size_t mln_plugin_id_at_v1(size_t index, char* output, size_t outputCapacity) try {
