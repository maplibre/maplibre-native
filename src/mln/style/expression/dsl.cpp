...
std::unique_ptr<Expression> toVerticalGradient(std::unique_ptr<Expression> value, std::unique_ptr<Expression> def) {
    return coercion(type::VerticalGradient, std::move(value), std::move(def));
}
