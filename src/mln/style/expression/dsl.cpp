...
std::unique_ptr<Expression> toProjectionDefinition(std::unique_ptr<Expression> value, std::unique_ptr<Expression> def) {
    return coercion(type::ProjectionDefinition, std::move(value), std::move(def));
}
