...
void PluginLayerHost::invoke(Instance& instance,
    ...
    constexpr auto compiledBackend = MLN_PLUGIN_BACKEND_OPENGL;
#elif MLN_RENDER_BACKEND_VULKAN
    constexpr auto compiledBackend = MLN_PLUGIN_BACKEND_VULKAN;
#elif MLN_RENDER_BACKEND_METAL
    constexpr auto compiledBackend = MLN_PLUGIN_BACKEND_METAL;
#else
    constexpr uint32_t compiledBackend = 0;
#endif
    if ((instance.extension.backendMask & compiledBackend) == 0) return;
    ...
    backend.backend = static_cast<mln_plugin_backend>(compiledBackend);
    ...
    const mln_plugin_frame_context_v1 frame{sizeof(mln_plugin_frame_context_v1),
                                            prepare ? MLN_PLUGIN_RENDER_STAGE_PREPARE : stageFor(parameters.pass),
                                            size.width,
                                            size.height,
                                            parameters.frameCount,
                                            &backend,
                                            properties.values.data(),
                                            properties.values.size(),
                                            packets.data(),
                                            packets.size()};
