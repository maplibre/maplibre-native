...
std::optional<PropertyExpression<T>> convertFunctionToExpression(const Convertible& value,
                                                                 ...
                                                                 bool convertTokens) {
    auto expression =
        convertFunctionToExpression(expression::valueTypeToExpressionType<T>(), value, error, convertTokens);
    if (!expression) {
        return std::nullopt;
    }
...
enum class FunctionType { Interval, Exponential, Categorical, Identity, Invalid };
...
static std::optional<std::map<double, std::unique_ptr<Expression>>> convertStops(const type::Type& type,
    ...
    for (std::size_t i = 0; i < arrayLength(*stopsValue); ++i) {
        ...
        std::optional<std::unique_ptr<Expression>> e =
            convertLiteral(type, arrayMember(stopValue, 1), error, convertTokens);
        if (!e) {
            return std::nullopt;
        }
...
std::unique_ptr<Expression> categorical<bool>(const type::Type& type,
    ...
    std::unique_ptr<Expression> trueCase =
        it == branches.end() ? error("replaced with default") : std::move(it->second);

    it = branches.find(false);
    std::unique_ptr<Expression> falseCase =
        it == branches.end() ? error("replaced with default") : std::move(it->second);
