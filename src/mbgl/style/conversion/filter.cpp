...
std::optional<Filter> Converter<Filter>::operator()(const Convertible &value,
                                                    Error &error) const {
  if (isExpression(value)) {
    ...
    return Filter(
        std::optional<std::unique_ptr<Expression>>(std::move(*expression)),
        serializeLegacyFilter(value));
  }
}
...
bool isExpression(const Convertible &filter) {
  ...
  std::optional<std::string> op = toString(arrayMember(filter, 0));
  ...
  if (!op) {
    ...
    if (arrayLength(filter) < 2)
      return false;
    std::optional<std::string> operand = toString(arrayMember(filter, 1));
    ...
    std::optional<std::string> str = toString(arrayMember(filter, 1));

    return arrayLength(filter) >= 3 &&
           (!str || isArray(arrayMember(filter, 2)));

  } else if (*op == "==" || *op == "!=" || *op == ">" || *op == ">=" ||
             *op == "<" || *op == "<=") {
    return arrayLength(filter) != 3 || isArray(arrayMember(filter, 1)) ||
           isArray(arrayMember(filter, 2));
...
ParseResult
createExpression(const std::string &op,
  ...
  if (op == "any") {
    ...
    ParseResult parseResult =
        createCompoundExpression(op, std::move(*args), parsingContext);
    if (!parseResult) {
      error.message = parsingContext.getCombinedErrors();
      return {};
    } else {
...
ParseResult createExpression(const std::string &op, ParseResult arg,
                             Error &error) {
  if (!arg) {
    return {};
  }
...
convertLiteralArray(const Convertible &input, Error &error,
...
ParseResult convertLegacyComparisonFilter(
    const Convertible &values, Error &error,
    const std::optional<std::string> &opOverride = {}) {
  std::optional<std::string> op =
      opOverride ? opOverride : toString(arrayMember(values, 0));
  ...
  if (!property) {
    ...
    return createExpression("filter-type-" + *op,
                            convertLiteralArray(values, error, 2), error);
  } else if (*property == "$id") {
    return createExpression("filter-id-" + *op,
                            convertLiteralArray(values, error, 2), error);
  } else {
    return createExpression("filter-" + *op,
                            convertLiteralArray(values, error, 1), error);
  }
}
...
ParseResult convertLegacyHasFilter(const Convertible &values, Error &error) {
  ...
  if (!property) {
    ...
    return createExpression("filter-has-id",
                            std::vector<std::unique_ptr<Expression>>(), error);
  } else {
    return createExpression("filter-has",
                            {std::make_unique<Literal>(*property)}, error);
  }
}
...
ParseResult convertLegacyInFilter(const Convertible &values, Error &error) {
  ...
  if (!property) {
    ...
    return createExpression("filter-type-in",
                            convertLiteralArray(values, error, 2), error);
  } else if (*property == "$id") {
    return createExpression("filter-id-in",
                            convertLiteralArray(values, error, 2), error);
  } else {
    return createExpression("filter-in", convertLiteralArray(values, error, 1),
                            error);
  }
}

std::optional<std::vector<std::unique_ptr<Expression>>>
convertLegacyFilterArray(const Convertible &input, Error &error,
                         std::size_t startIndex = 0) {
  std::vector<std::unique_ptr<Expression>> output;
  output.reserve(arrayLength(input));
  for (std::size_t i = startIndex; i < arrayLength(input); ++i) {
    std::optional<std::unique_ptr<Expression>> child =
  ...
  if (!op) {
    ...
    return {
        *op == "==" || *op == "<" || *op == ">" || *op == "<=" || *op == ">="
        ...
        : *op == "any"
            ? createExpression(
                  "any", convertLegacyFilterArray(values, error, 1), error)
        : *op == "all"
            ? createExpression(
        ...
        : *op == "!in"
            ? createExpression("!", convertLegacyInFilter(values, error), error)
        : *op == "has"  ? convertLegacyHasFilter(values, error)
        : *op == "!has" ? createExpression(
                              "!", convertLegacyHasFilter(values, error), error)
