...
namespace {
void WarnIfOverscaleFactorCapsPrefetchDelta(
    const std::optional<uint8_t> &overscale,
    const std::optional<uint8_t> &prefetch) {
  const uint8_t prefetchDelta = std::max<uint8_t>(
...
void Source::Impl::setMaxOverscaleFactorForParentTiles(
...
Source::Impl::getMaxOverscaleFactorForParentTiles() const noexcept {
