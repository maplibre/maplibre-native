...
Length::Length(std::unique_ptr<Expression> input_)
    : Expression(Kind::Length, type::Number), input(std::move(input_)) {}
...
ParseResult Length::parse(const Convertible &value, ParsingContext &ctx) {
  ...
  if (length != 2) {
    ctx.error("Expected one argument, but found " + util::toString(length) +
              " instead.");
    return ParseResult();
  }
  ...
  if (!type.is<type::Array>() && !type.is<type::StringType>() &&
      !type.is<type::ValueType>()) {
    ctx.error("Expected argument of type string or array, but found " +
              toString(type) + " instead.");
    return ParseResult();
  }
