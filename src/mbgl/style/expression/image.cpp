...
Image::Image(std::string imageID_, bool available_)
    : imageID(std::move(imageID_)), available(available_) {}
...
const std::string &Image::id() const { return imageID; }

bool Image::isAvailable() const { return available; }

bool Image::empty() const { return imageID.empty(); }

} // namespace expression

namespace conversion {
using namespace mbgl::style::expression;
std::optional<Image> Converter<Image>::operator()(const Convertible &value,
                                                  Error &error) const {
  if (isArray(value)) {
    ...
    std::optional<std::string> imageID =
        toString(arrayMember(imageParameters, 0));
    if (!imageID) {
      error.message = "Image has to contain an ID.";
      return {};
    }
