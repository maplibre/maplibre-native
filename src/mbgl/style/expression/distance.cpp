...
const mapbox::cheap_ruler::CheapRuler::Unit UnitInMeters =
    mapbox::cheap_ruler::CheapRuler::Unit::Meters;
...
std::pair<std::optional<IndexRange>, std::optional<IndexRange>>
splitRange(const IndexRange &range, bool isLine) {
...
DistanceBBox getBBox(const mapbox::geometry::multi_point<double> &points,
                     const IndexRange &range) {
  if (!isRangeSafe(range, points.size()))
    return DefaultDistanceBBox;
...
DistanceBBox getBBox(const mapbox::geometry::line_string<double> &line,
                     const IndexRange &range) {
  if (!isRangeSafe(range, line.size()))
    return DefaultDistanceBBox;
  DistanceBBox bbox = DefaultDistanceBBox;
  for (std::size_t i = range.first; i <= range.second; ++i) {
...
bool isMultiPointValid(const mapbox::geometry::multi_point<double> &points) {
  if (points.empty()) {
    mbgl::Log::Error(mbgl::Event::Style,
                     "Invalid MultiPoint with empty geometry points");
    return false;
  }
  return true;
}
...
bool isLineStringValid(const mapbox::geometry::line_string<double> &line) {
  if (line.size() < 2) {
    mbgl::Log::Error(mbgl::Event::Style,
                     "Invalid LineString with fewer than 2 geometry points");
    return false;
  }
  return true;
}
...
bool isPolygonValid(const mapbox::geometry::polygon<double> &polygon) {
  ...
  for (const auto &ring : polygon) {
    if (ring.size() < 3) {
      mbgl::Log::Error(
          mbgl::Event::Style,
          "Invalid Polygon with ring having fewer than 3 geometry points");
      return false;
    }
  }
  return true;
...
// and {dx, dy} to calculate the distance. Distance will be 0.0 if bounding box
// are overlapping.
double bboxToBBoxDistance(const DistanceBBox &bbox1, const DistanceBBox &bbox2,
  ...
  return ruler.distance(mapbox::geometry::point<double>{0.0, 0.0},
                        mapbox::geometry::point<double>{dx, dy});
}
...
double segmentToSegmentDistance(const mapbox::geometry::point<double> &p1,
                                ...
                                mapbox::cheap_ruler::CheapRuler &ruler) {
  auto dist1 =
      std::min(pointToLineDistance(
                   p1, mapbox::geometry::line_string<double>{q1, q2}, ruler),
               pointToLineDistance(
                   p2, mapbox::geometry::line_string<double>{q1, q2}, ruler));
  auto dist2 =
      std::min(pointToLineDistance(
                   q1, mapbox::geometry::line_string<double>{p1, p2}, ruler),
               pointToLineDistance(
                   q2, mapbox::geometry::line_string<double>{p1, p2}, ruler));
...
double lineToLineDistance(const mapbox::geometry::line_string<double> &line1,
                          ...
                          mapbox::cheap_ruler::CheapRuler &ruler) {
  bool rangeSafe =
      isRangeSafe(range1, line1.size()) && isRangeSafe(range2, line2.size());
  if (!rangeSafe)
    return InvalidDistance;
...
double pointsToPointsDistance(
    const mapbox::geometry::multi_point<double> &points1, IndexRange &range1,
    const mapbox::geometry::multi_point<double> &points2, IndexRange &range2,
    mapbox::cheap_ruler::CheapRuler &ruler) {
  bool rangeSafe = isRangeSafe(range1, points1.size()) &&
                   isRangeSafe(range2, points2.size());
  if (!rangeSafe)
    return InvalidDistance;
  double dist = InfiniteDistance;
  for (std::size_t i = range1.first; i <= range1.second; ++i) {
...
double lineToPolygonDistance(const mapbox::geometry::line_string<double> &line,
  ...
  for (std::size_t i = range.first; i < range.second; ++i) {
    ...
    for (const auto &ring : polygon) {
      for (std::size_t j = 0, len = ring.size(), k = len - 1; j < len;
           k = j++) {
...
double
polygonToPolygonDistance(const mapbox::geometry::polygon<double> &polygon1,
  ...
  if (currentMiniDist != InfiniteDistance &&
      bboxToBBoxDistance(bbox1, bbox2, ruler) >= currentMiniDist) {
    return currentMiniDist;
  }
  const auto polygonIntersect =
      [](const mapbox::geometry::polygon<double> &poly1,
         const mapbox::geometry::polygon<double> &poly2) {
        for (const auto &ring : poly1) {
          for (std::size_t i = 0; i <= ring.size() - 1; ++i) {
            if (pointWithinPolygon(ring[i], poly2, true /*trueOnBoundary*/))
              return true;
          }
        }
        return false;
  ...
  for (const auto &ring1 : polygon1) {
    for (std::size_t i = 0, len1 = ring1.size(), l = len1 - 1; i < len1;
         l = i++) {
      const auto &p1 = ring1[l];
      const auto &p2 = ring1[i];
      for (const auto &ring2 : polygon2) {
        for (std::size_t j = 0, len2 = ring2.size(), k = len2 - 1; j < len2;
             k = j++) {
          ...
          if (segmentIntersectSegment(p1, p2, q1, q2))
            return 0.0;
          dist =
              std::min(dist, segmentToSegmentDistance(p1, p2, q1, q2, ruler));
        }
      }
    }
  }
...
// The priority queue will ensure the top element would always be the pair that
// has the biggest distance
using DistQueue =
    std::priority_queue<DistPair, std::deque<DistPair>, Comparator>;
...
double
pointsToPolygonDistance(const mapbox::geometry::multi_point<double> &points,
                        const mapbox::geometry::polygon<double> &polygon,
                        mapbox::cheap_ruler::CheapRuler &ruler,
                        double currentMiniDist = InfiniteDistance) {
  auto miniDist =
      std::min(ruler.distance(points[0], polygon[0][0]), currentMiniDist);
  if (miniDist == 0.0)
    return miniDist;
  DistQueue distQueue;
  distQueue.push(std::forward_as_tuple(0, IndexRange(0, points.size() - 1),
                                       IndexRange(0, 0)));
  ...
  while (!distQueue.empty()) {
    ...
    // In case the set size are relatively small, we could use brute-force
    // directly
    if (getRangeSize(range) <= MinLinePointsSize) {
      ...
      const auto updateQueue = [&distQueue, &miniDist, &ruler, &points,
                                &polyBBox](std::optional<IndexRange> &rangeA) {
        if (!rangeA)
          return;
        auto tempDist =
            bboxToBBoxDistance(getBBox(points, *rangeA), polyBBox, ruler);
        ...
        if (tempDist < miniDist)
          distQueue.push(
              std::make_tuple(tempDist, std::move(*rangeA), IndexRange(0, 0)));
      };
...
double lineToPolygonDistance(const mapbox::geometry::line_string<double> &line,
                             ...
                             double currentMiniDist = InfiniteDistance) {
  auto miniDist =
      std::min(ruler.distance(line[0], polygon[0][0]), currentMiniDist);
  if (miniDist == 0.0)
    return miniDist;
  DistQueue distQueue;
  distQueue.push(std::forward_as_tuple(0, IndexRange(0, line.size() - 1),
                                       IndexRange(0, 0)));
  ...
  while (!distQueue.empty()) {
    ...
    if (getRangeSize(range) <= MinLinePointsSize) {
      ...
      const auto updateQueue = [&distQueue, &miniDist, &ruler, &line,
                                &polyBBox](std::optional<IndexRange> &rangeA) {
        if (!rangeA)
          return;
        auto tempDist =
            bboxToBBoxDistance(getBBox(line, *rangeA), polyBBox, ruler);
        // Insert new pair to the queue if the bbox distance is less than
        // miniDist, The pair with biggest distance will be at the top
        if (tempDist < miniDist)
          distQueue.push(
...
double lineToLineDistance(const mapbox::geometry::line_string<double> &line1,
  ...
  if (miniDist == 0.0)
    return miniDist;
  DistQueue distQueue;
  distQueue.push(std::forward_as_tuple(0, IndexRange(0, line1.size() - 1),
                                       IndexRange(0, line2.size() - 1)));
  ...
  while (!distQueue.empty()) {
    ...
    // In case the set size are relatively small, we could use brute-force
    // directly
    if (getRangeSize(rangeA) <= MinLinePointsSize &&
        getRangeSize(rangeB) <= MinLinePointsSize) {
      ...
      const auto updateQueue = [&distQueue, &miniDist, &ruler, &line1,
                                &line2](std::optional<IndexRange> &range1,
                                        std::optional<IndexRange> &range2) {
        if (!range1 || !range2)
          return;
        auto tempDist = bboxToBBoxDistance(getBBox(line1, *range1),
                                           getBBox(line2, *range2), ruler);
        // Insert new pair to the queue if the bbox distance is less than
        // miniDist, The pair with biggest distance will be at the top
        if (tempDist < miniDist)
          distQueue.push(std::make_tuple(tempDist, std::move(*range1),
                                         std::move(*range2)));
      };
...
double
pointsToPointsDistance(const mapbox::geometry::multi_point<double> &pointSet1,
  ...
  if (miniDist == 0.0)
    return miniDist;
  DistQueue distQueue;
  distQueue.push(std::forward_as_tuple(0, IndexRange(0, pointSet1.size() - 1),
  ...
  while (!distQueue.empty()) {
    ...
    // In case the set size are relatively small, we could use brute-force
    // directly
    if (getRangeSize(rangeA) <= MinPointsSize &&
        getRangeSize(rangeB) <= MinPointsSize) {
      auto tempDist =
          pointsToPointsDistance(pointSet1, rangeA, pointSet2, rangeB, ruler);
      if (std::isnan(tempDist))
        return tempDist;
      miniDist = std::min(miniDist, tempDist);
      if (miniDist == 0.0)
      ...
      const auto updateQueue = [&distQueue, &miniDist, &ruler, &pointSet1,
                                &pointSet2](std::optional<IndexRange> &range1,
                                            std::optional<IndexRange> &range2) {
        if (!range1 || !range2)
          return;
        auto tempDist = bboxToBBoxDistance(getBBox(pointSet1, *range1),
                                           getBBox(pointSet2, *range2), ruler);
        // Insert new pair to the queue if the bbox distance is less than
        // miniDist, The pair with biggest distance will be at the top
        if (tempDist < miniDist)
          distQueue.push(std::make_tuple(tempDist, std::move(*range1),
...
double pointsToLineDistance(const mapbox::geometry::multi_point<double> &points,
  ...
  while (!distQueue.empty()) {
    ...
    if (getRangeSize(rangeA) <= MinPointsSize &&
        getRangeSize(rangeB) <= MinLinePointsSize) {
      bool rangeSafe = isRangeSafe(rangeA, points.size()) &&
                       isRangeSafe(rangeB, line.size());
      if (!rangeSafe) {
        mbgl::Log::Error(mbgl::Event::Style, "Index is out of range");
        return InvalidDistance;
      }
      ...
      for (std::size_t i = rangeA.first; i <= rangeA.second; ++i) {
        miniDist =
            std::min(miniDist, pointToLineDistance(points[i], subLine, ruler));
        if (miniDist == 0.0)
          return 0.0;
      }
    } else {
      ...
      auto newRangesB = splitRange(rangeB, true /*isLine*/);
      const auto updateQueue = [&distQueue, &miniDist, &ruler, &points,
                                &line](std::optional<IndexRange> &range1,
                                       std::optional<IndexRange> &range2) {
        if (!range1 || !range2)
          return;
        auto tempDist = bboxToBBoxDistance(getBBox(points, *range1),
                                           getBBox(line, *range2), ruler);
        // Insert new pair to the queue if the bbox distance is less than
        // miniDist, The pair with biggest distance will be at the top
        if (tempDist < miniDist)
          distQueue.push(std::make_tuple(tempDist, std::move(*range1),
...
double
pointsToLinesDistance(const mapbox::geometry::multi_point<double> &points,
...
double
lineToLinesDistance(const mapbox::geometry::line_string<double> &line,
                    const mapbox::geometry::multi_line_string<double> &lines,
                    mapbox::cheap_ruler::CheapRuler &ruler) {
  double dist = InfiniteDistance;
...
double
pointsToGeometryDistance(const mapbox::geometry::multi_point<double> &points,
                         const Feature::geometry_type &geoSet) {
  if (!isMultiPointValid(points))
    return InvalidDistance;
  mapbox::cheap_ruler::CheapRuler ruler(points.front().y, UnitInMeters);
  return geoSet.match(
      [&points, &ruler](const mapbox::geometry::point<double> &p) {
        return pointsToPointsDistance(mapbox::geometry::multi_point<double>{p},
                                      points, ruler);
      },
      ...
      [&points, &ruler](const mapbox::geometry::line_string<double> &line) {
        if (!isLineStringValid(line))
          return InvalidDistance;
        return pointsToLineDistance(points, line, ruler);
      },
      [&points,
       &ruler](const mapbox::geometry::multi_line_string<double> &lines) {
        for (const auto &line : lines) {
          if (!isLineStringValid(line))
            return InvalidDistance;
        }
      ...
      [&points,
       &ruler](const mapbox::geometry::polygon<double> &polygon) -> double {
        if (!isPolygonValid(polygon))
          return InvalidDistance;
        return pointsToPolygonDistance(points, polygon, ruler);
      },
      [&points, &ruler](
          const mapbox::geometry::multi_polygon<double> &polygons) -> double {
        double dist = InfiniteDistance;
...
double lineToGeometryDistance(const mapbox::geometry::line_string<double> &line,
  ...
  return geoSet.match(
      [&line, &ruler](const mapbox::geometry::point<double> &p) {
        return pointsToLineDistance(mapbox::geometry::multi_point<double>{p},
                                    line, ruler);
      },
...
double
polygonToGeometryDistance(const mapbox::geometry::polygon<double> &polygon,
                          const Feature::geometry_type &geoSet) {
  if (!isPolygonValid(polygon))
    return InvalidDistance;
  mapbox::cheap_ruler::CheapRuler ruler(polygon.front().front().y,
                                        UnitInMeters);
  return geoSet.match(
      ...
      [&polygon, &ruler](const mapbox::geometry::polygon<double> &polygon1) {
        if (!isPolygonValid(polygon1))
          return InvalidDistance;
        return polygonToPolygonDistance(polygon, polygon1, ruler);
      },
      [&polygon,
       &ruler](const mapbox::geometry::multi_polygon<double> &polygons) {
        double dist = InfiniteDistance;
        for (const auto &polygon1 : polygons) {
          if (!isPolygonValid(polygon1))
            return InvalidDistance;
          auto tempDist =
              polygonToPolygonDistance(polygon, polygon1, ruler, dist);
          if (std::isnan(tempDist))
            return tempDist;
          dist = std::min(dist, tempDist);
          if (dist == 0.0)
  ...
  return convertGeometry(feature, canonical)
      .match(
          [&geoSet](const mapbox::geometry::point<double> &point) -> double {
            return pointsToGeometryDistance(
                mapbox::geometry::multi_point<double>{point}, geoSet);
          },
          [&geoSet](const mapbox::geometry::multi_point<double> &points)
              -> double { return pointsToGeometryDistance(points, geoSet); },
          [&geoSet](const mapbox::geometry::line_string<double> &line)
              -> double { return lineToGeometryDistance(line, geoSet); },
          [&geoSet](const mapbox::geometry::multi_line_string<double> &lines)
              -> double {
            double dist = InfiniteDistance;
            for (const auto &line : lines) {
          ...
          [&geoSet](const mapbox::geometry::polygon<double> &polygon)
              -> double { return polygonToGeometryDistance(polygon, geoSet); },
          [&geoSet](const mapbox::geometry::multi_polygon<double> &polygons)
              -> double {
            double dist = InfiniteDistance;
            for (const auto &polygon : polygons) {
...
std::optional<GeoJSON> parseValue(const style::conversion::Convertible &value,
  ...
  ctx.error("'distance' expression needs to be an array with format "
            "[\"distance\", GeoJSONObj].");
  return {};
}

std::optional<Feature::geometry_type>
getGeometry(const Feature &feature,
  ...
  ctx.error("'distance' expression requires valid geojson object with valid "
...
Distance::Distance(GeoJSON geojson, Feature::geometry_type geometries_)
    : Expression(Kind::Distance, type::Number),
      geoJSONSource(std::move(geojson)), geometries(std::move(geometries_)) {}
...
EvaluationResult Distance::evaluate(const EvaluationContext &params) const {
  ...
  if (geometryType == FeatureType::Point ||
    ...
    auto distance =
        calculateDistance(*params.feature, *params.canonical, geometries);
    if (!std::isnan(distance)) {
      assert(distance >= 0.0);
      return distance;
    }
  }
  return EvaluationError{"distance expression currently only evaluates valid "
...
ParseResult Distance::parse(const Convertible &value, ParsingContext &ctx) {
  ...
  return parsedValue->match(
      [&parsedValue,
       &ctx](const mapbox::geometry::geometry<double> &geometrySet) {
        if (auto ret = getGeometry(mbgl::Feature(geometrySet), ctx)) {
          return ParseResult(
              std::make_unique<Distance>(*parsedValue, std::move(*ret)));
        }
        return ParseResult();
      },
      ...
      [&parsedValue,
       &ctx](const mapbox::feature::feature_collection<double> &features) {
      ...
      [&ctx](const auto &) {
        ctx.error("'distance' expression requires valid geojson that contains "
                  "Point/LineString/Polygon geometries.");
        return ParseResult();
      });
...
mbgl::Value convertValue(const mapbox::geojson::rapidjson_value &v) {
  if (v.IsNumber()) {
    if (v.IsInt64())
      return std::int64_t(v.GetInt64());
    if (v.IsUint64())
      return std::uint64_t(v.GetUint64());
    return v.GetDouble();
  }
  if (v.IsBool()) {
    return v.GetBool();
...
mbgl::Value Distance::serialize() const {
  ...
  const mapbox::geojson::rapidjson_value value =
      mapbox::geojson::convert(geoJSONSource, allocator);
  if (value.IsObject()) {
    ...
    mbgl::Log::Error(mbgl::Event::Style,
                     "Failed to serialize 'distance' expression, converted "
                     "rapidJSON is not an object");
  }
...
std::string Distance::getOperator() const { return "distance"; }
