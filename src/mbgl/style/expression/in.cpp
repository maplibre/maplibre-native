...
bool isComparableType(const type::Type &type) {
  return type == type::Boolean || type == type::String ||
         type == type::Number || type == type::Null || type == type::Value;
}

bool isComparableRuntimeType(const type::Type &type) {
  return type == type::Boolean || type == type::String ||
         type == type::Number || type == type::Null;
}
...
In::In(std::unique_ptr<Expression> needle_,
       std::unique_ptr<Expression> haystack_)
    : Expression(Kind::In, type::Boolean), needle(std::move(needle_)),
...
EvaluationResult In::evaluate(const EvaluationContext &params) const {
  ...
  if (!isComparableRuntimeType(evaluatedNeedleType)) {
    return EvaluationError{"Expected first argument to be of type boolean, "
  ...
  if (evaluatedNeedleType == type::Null ||
      evaluatedHaystackType == type::Null) {
  ...
  if (evaluatedHaystackType == type::String) {
    ...
    return EvaluationResult(haystackString.find(needleString) !=
                            std::string::npos);
  } else {
    const auto haystackArray = evaluatedHaystack->get<std::vector<Value>>();
    return EvaluationResult(std::find(haystackArray.begin(),
                                      haystackArray.end(),
                                      *evaluatedNeedle) != haystackArray.end());
  }
}
...
ParseResult In::parse(const Convertible &value, ParsingContext &ctx) {
  ...
  if (length != 3) {
    ctx.error("Expected 2 arguments, but found " + util::toString(length - 1) +
              " instead.");
    return ParseResult();
  }
  ...
  if (!isComparableType(needleType)) {
    ctx.error("Expected first argument to be of type boolean, string or "
              "number, but found " +
  ...
  return ParseResult(
      std::make_unique<In>(std::move(*needle), std::move(*haystack)));
}
...
std::string In::getOperator() const { return "in"; }
