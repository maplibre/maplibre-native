...
EvaluationResult Coalesce::evaluate(const EvaluationContext &params) const {
  ...
  for (const auto &arg : args) {
    ...
    // We need to keep track of the first requested image in a coalesce
    // statement. If coalesce can't find a valid image, we return the first
    // requested image.
    if (getType() == type::Image && result) {
      const auto image = fromExpressionValue<Image>(*result);
      if (image && !image->isAvailable()) {
        if (!requestedImage)
          requestedImage = Image(image->id());
        if (!argsCount)
          result = *requestedImage;
        continue;
      }
    }
...
ParseResult Coalesce::parse(const Convertible &value, ParsingContext &ctx) {
  ...
  return ParseResult(std::make_unique<Coalesce>(
      needsAnnotation ? type::Value : *outputType, std::move(args)));
}
