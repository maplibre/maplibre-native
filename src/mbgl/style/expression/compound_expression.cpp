...
template <typename T> struct Varargs : std::vector<T> {
  template <class... Args>
...
struct SignatureBase {
  ...
  SignatureBase(type::Type result_,
                ...
                std::string name_)
      : result(std::move(result_)), params(std::move(params_)),
        name(std::move(name_)) {}
  virtual ~SignatureBase() = default;

  virtual EvaluationResult apply(const EvaluationContext &,
                                 const Args &) const = 0;
...
*/
template <class, class Enable = void> struct Signature;
  ...
  Signature(R (*evaluate_)(Params...), const std::string &name_)
      : SignatureBase(
            ...
            std::vector<type::Type>{
                valueTypeToExpressionType<std::decay_t<Params>>()...},
            name_),
        evaluate(evaluate_) {}

  EvaluationResult apply(const EvaluationContext &evaluationParameters,
                         const Args &args) const override {
    return applyImpl(evaluationParameters, args,
...
private:
  ...
  EvaluationResult applyImpl(const EvaluationContext &evaluationParameters,
    ...
    for (int i = 0; i < int(sizeof...(Params)); ++i) {
      const EvaluationResult evaluatedArg =
          args.at(i)->evaluate(evaluationParameters);
      if (!evaluatedArg)
        return evaluatedArg.error();
      evaluated[i] = *evaluatedArg;
    }
    const R value =
        evaluate(*fromExpressionValue<std::decay_t<Params>>(evaluated[I])...);
    if (!value)
      return value.error();
    return *value;
  }
...
struct Signature<R(const Varargs<T> &)> : SignatureBase {
  Signature(R (*evaluate_)(const Varargs<T> &), const std::string &name_)
      : SignatureBase(
            valueTypeToExpressionType<std::decay_t<typename R::Value>>(),
            VarargsType{valueTypeToExpressionType<T>()}, name_),
        evaluate(evaluate_) {}
...
template <class R, typename T>
struct Signature<R(const EvaluationContext &, const Varargs<T> &)>
    : SignatureBase {
...
template <class, class = void> struct SignatureType;
...
static std::unique_ptr<detail::SignatureBase>
makeSignature(std::string name, Fn evaluateFunction) {
  return std::make_unique<Signature<typename SignatureType<Fn>::Type>>(
      evaluateFunction, std::move(name));
}
...
Value featureIdAsExpressionValue(const EvaluationContext &params) {
  ...
  if (id.is<NullValue>())
    return Null;
  return id.match(
      [](const auto &idid) { return toExpressionValue(mbgl::Value(idid)); });
};

std::optional<Value>
featurePropertyAsExpressionValue(const EvaluationContext &params,
                                 const std::string &key) {
  assert(params.feature);
  auto property = params.feature->getValue(key);
  return property ? std::optional<Value>(toExpressionValue(*property))
                  : std::optional<Value>();
};
...
std::optional<double> featurePropertyAsDouble(const EvaluationContext &params,
                                              const std::string &key) {
  assert(params.feature);
  auto property = params.feature->getValue(key);
  if (!property)
  ...
  return property->match(
      ...
      [](uint64_t value) -> std::optional<double> {
        return {static_cast<double>(value)};
      },
      [](int64_t value) -> std::optional<double> {
        return {static_cast<double>(value)};
      },
...
featurePropertyAsString(const EvaluationContext &params,
                        const std::string &key) {
  assert(params.feature);
  auto property = params.feature->getValue(key);
  if (!property)
...
const auto &eCompoundExpression() {
  static auto signature = detail::makeSignature(
      "e", []() -> Result<double> { return 2.718281828459045; });
  return signature;
}

const auto &piCompoundExpression() {
  static auto signature = detail::makeSignature(
      "pi", []() -> Result<double> { return 3.141592653589793; });
  return signature;
}

const auto &ln2CompoundExpression() {
  static auto signature = detail::makeSignature(
      "ln2", []() -> Result<double> { return 0.6931471805599453; });
  return signature;
}
...
const auto &rgbaCompoundExpression() {
  static auto signature =
      detail::makeSignature("rgba", [](double r, double g, double b, double a) {
        return rgba(r, g, b, a);
      });
  return signature;
}
...
const auto &rgbCompoundExpression() {
  static auto signature = detail::makeSignature(
      "rgb", [](double r, double g, double b) { return rgba(r, g, b, 1.0f); });
  return signature;
}
...
const auto &zoomCompoundExpression() {
  static auto signature = detail::makeSignature(
      "zoom", [](const EvaluationContext &params) -> Result<double> {
        if (!params.zoom) {
          return EvaluationError{"The 'zoom' expression is unavailable in the "
...
const auto &heatmapDensityCompoundExpression() {
  static auto signature = detail::makeSignature(
      "heatmap-density", [](const EvaluationContext &params) -> Result<double> {
        if (!params.colorRampParameter) {
          return EvaluationError{
              "The 'heatmap-density' expression is unavailable in the current "
              "evaluation context."};
        }
...
const auto &lineProgressCompoundExpression() {
  static auto signature = detail::makeSignature(
      "line-progress", [](const EvaluationContext &params) -> Result<double> {
        if (!params.colorRampParameter) {
          return EvaluationError{
              "The 'line-progress' expression is unavailable in the current "
              "evaluation context."};
        }
        return *(params.colorRampParameter);
      });
  ...
  const static auto signature = detail::makeSignature(
      "accumulated", [](const EvaluationContext &params) -> Result<Value> {
        if (!params.accumulated) {
          return EvaluationError{"The 'accumulated' expression is unavailable "
...
const auto &hasContextCompoundExpression() {
  static auto signature = detail::makeSignature(
      "has",
      [](const EvaluationContext &params,
         const std::string &key) -> Result<bool> {
        if (!params.feature) {
          return EvaluationError{
...
const auto &getObjectCompoundExpression() {
  static auto signature = detail::makeSignature(
      "get",
      [](const std::string &key,
         const std::unordered_map<std::string, Value> &object)
          -> Result<Value> {
...
const auto &idCompoundExpression() {
  static auto signature = detail::makeSignature(
      "id", [](const EvaluationContext &params) -> Result<Value> {
        if (!params.feature) {
          return EvaluationError{
              "Feature data is unavailable in the current evaluation context."};
        }
        ...
        auto id = params.feature->getID();
        return id.match([](const auto &idValue) {
...
const auto &plusCompoundExpression() {
  static auto signature = detail::makeSignature(
      "+", [](const Varargs<double> &args) -> Result<double> {
        double sum = 0.0f;
        for (auto arg : args) {
          sum += arg;
        }
...
const auto &negateCompoundExpression() {
  static auto signature =
      detail::makeSignature("-", [](double a) -> Result<double> { return -a; });
  return signature;
}
...
const auto &divideCompoundExpression() {
  static auto signature =
      detail::makeSignature("/", [](double a, double b) -> Result<double> {
        if (b == 0) {
          if (a == 0)
...
const auto &modCompoundExpression() {
  static auto signature = detail::makeSignature(
      "%", [](double a, double b) -> Result<double> { return fmod(a, b); });
  return signature;
}

const auto &powCompoundExpression() {
  static auto signature = detail::makeSignature(
      "^", [](double a, double b) -> Result<double> { return pow(a, b); });
  return signature;
}

const auto &sqrtCompoundExpression() {
  static auto signature = detail::makeSignature(
      "sqrt", [](double x) -> Result<double> { return sqrt(x); });
  return signature;
}

const auto &log10CompoundExpression() {
  static auto signature = detail::makeSignature(
      "log10", [](double x) -> Result<double> { return log10(x); });
  return signature;
}

const auto &lnCompoundExpression() {
  static auto signature = detail::makeSignature(
      "ln", [](double x) -> Result<double> { return log(x); });
  return signature;
}

const auto &log2CompoundExpression() {
  static auto signature = detail::makeSignature(
      "log2", [](double x) -> Result<double> { return util::log2(x); });
  return signature;
}

const auto &sinCompoundExpression() {
  static auto signature = detail::makeSignature(
      "sin", [](double x) -> Result<double> { return sin(x); });
  return signature;
}

const auto &cosCompoundExpression() {
  static auto signature = detail::makeSignature(
      "cos", [](double x) -> Result<double> { return cos(x); });
  return signature;
}

const auto &tanCompoundExpression() {
  static auto signature = detail::makeSignature(
      "tan", [](double x) -> Result<double> { return tan(x); });
  return signature;
}

const auto &asinCompoundExpression() {
  static auto signature = detail::makeSignature(
      "asin", [](double x) -> Result<double> { return asin(x); });
  return signature;
}

const auto &acosCompoundExpression() {
  static auto signature = detail::makeSignature(
      "acos", [](double x) -> Result<double> { return acos(x); });
  return signature;
}

const auto &atanCompoundExpression() {
  static auto signature = detail::makeSignature(
      "atan", [](double x) -> Result<double> { return atan(x); });
  return signature;
}

const auto &minCompoundExpression() {
  static auto signature = detail::makeSignature(
      "min", [](const Varargs<double> &args) -> Result<double> {
...
const auto &maxCompoundExpression() {
  static auto signature = detail::makeSignature(
      "max", [](const Varargs<double> &args) -> Result<double> {
        double result = -std::numeric_limits<double>::infinity();
        for (double arg : args) {
...
const auto &roundCompoundExpression() {
  static auto signature = detail::makeSignature(
      "round", [](double x) -> Result<double> { return ::round(x); });
  return signature;
}

const auto &floorCompoundExpression() {
  static auto signature = detail::makeSignature(
      "floor", [](double x) -> Result<double> { return std::floor(x); });
  return signature;
}

const auto &ceilCompoundExpression() {
  static auto signature = detail::makeSignature(
      "ceil", [](double x) -> Result<double> { return std::ceil(x); });
  return signature;
}

const auto &absCompoundExpression() {
  static auto signature = detail::makeSignature(
      "abs", [](double x) -> Result<double> { return std::abs(x); });
  return signature;
}

const auto &notCompoundExpression() {
  static auto signature =
      detail::makeSignature("!", [](bool e) -> Result<bool> { return !e; });
  return signature;
}

const auto &isSupportedScriptCompoundExpression() {
  static auto signature = detail::makeSignature(
      "is-supported-script", [](const std::string &x) -> Result<bool> {
...
const auto &upcaseCompoundExpression() {
  static auto signature = detail::makeSignature(
      "upcase", [](const std::string &input) -> Result<std::string> {
...
const auto &downcaseCompoundExpression() {
  static auto signature = detail::makeSignature(
      "downcase", [](const std::string &input) -> Result<std::string> {
...
const auto &concatCompoundExpression() {
  static auto signature = detail::makeSignature(
      "concat", [](const Varargs<Value> &args) -> Result<std::string> {
        std::string s;
...
const auto &errorCompoundExpression() {
  static auto signature = detail::makeSignature(
      "error", [](const std::string &input) -> Result<type::ErrorType> {
...
const auto &featureStateCompoundExpression() {
  static auto signature =
      detail::makeSignature("feature-state",
                            [](const EvaluationContext &params,
                               const std::string &key) -> Result<Value> {
                              mbgl::Value state;
...
const auto &filterEqualsCompoundExpression() {
  static auto signature = detail::makeSignature(
      "filter-==",
      [](const EvaluationContext &params, const std::string &key,
...
const auto &filterTypeEqualsCompoundExpression() {
  static auto signature = detail::makeSignature(
      "filter-type-==",
      [](const EvaluationContext &params,
         const std::string &lhs) -> Result<bool> {
        if (!params.feature)
          return false;
...
const auto &filterIdLessThanStringCompoundExpression() {
  static auto signature =
      detail::makeSignature("filter-id-<",
                            [](const EvaluationContext &params,
                               const std::string &lhs) -> Result<bool> {
...
const auto &filterHasCompoundExpression() {
  static auto signature =
      detail::makeSignature("filter-has",
                            [](const EvaluationContext &params,
                               const std::string &key) -> Result<bool> {
                              assert(params.feature);
...
const auto &filterHasIdCompoundExpression() {
  static auto signature = detail::makeSignature(
      "filter-has-id", [](const EvaluationContext &params) -> Result<bool> {
        assert(params.feature);
...
const auto &filterTypeInCompoundExpression() {
  static auto signature = detail::makeSignature(
      "filter-type-in",
      [](const EvaluationContext &params,
         const Varargs<std::string> &types) -> Result<bool> {
        assert(params.feature);
        std::optional<std::string> type =
            featureTypeAsString(params.feature->getType());
...
const auto &filterIdInCompoundExpression() {
  static auto signature = detail::makeSignature(
      "filter-id-in",
      [](const EvaluationContext &params,
         const Varargs<Value> &ids) -> Result<bool> {
...
const auto &filterInCompoundExpression() {
  static auto signature = detail::makeSignature(
      "filter-in",
      [](const EvaluationContext &params,
         const Varargs<Value> &varargs) -> Result<bool> {
        if (varargs.size() < 2)
          return false;
        assert(varargs[0].is<std::string>());
        auto value = featurePropertyAsExpressionValue(
            params, varargs[0].get<std::string>());
        return value ? std::find(varargs.begin() + 1, varargs.end(), *value) !=
                           varargs.end()
                     : false;
      });
  return signature;
...
using ParseCompoundFunction =
    const std::unique_ptr<detail::SignatureBase> &(*)();
...
std::string
expectedTypesError(const Definitions &definitions,
                   const std::vector<std::unique_ptr<Expression>> &args) {
  std::vector<std::string>
      availableOverloads; // Only used if there are no overloads with matching
                          // number of args
  ...
  return "Expected arguments of type " + signatures + ", but found (" +
         actualTypes + ") instead.";
}

static ParseResult
createCompoundExpression(const Definitions &definitions,
  ...
  for (auto it = definitions.first; it != definitions.second; ++it) {
    ...
    if (signature->params.is<std::vector<type::Type>>()) {
      ...
      for (std::size_t i = 0; i < args.size(); i++) {
        ...
        std::optional<std::string> err =
            type::checkSubtype(params.at(i), arg->getType());
        if (err) {
      ...
      for (std::size_t i = 0; i < args.size(); i++) {
        const std::unique_ptr<Expression> &arg = args[i];
        std::optional<std::string> err =
            type::checkSubtype(paramType, arg->getType());
        if (err) {
          signatureContext.error(*err, i + 1);
        }
      }
    ...
    if (signatureContext.getErrors().empty()) {
      return ParseResult(
          std::make_unique<CompoundExpression>(*signature, std::move(args)));
    }
  }
  ...
  for (auto it = definitions.first; it != definitions.second; ++it) {
    ...
    if (signature->params.is<VarargsType>() ||
      ...
      if (argParseFailed) {
        ...
        ParseResult parseWithArgs =
            createCompoundExpression(definitions, std::move(args), ctx);
        if (parseWithArgs) {
...
createCompoundExpression(const std::string &name,
  ...
  return createCompoundExpression(
      compoundExpressionRegistry.equal_range(name.c_str()), std::move(args),
      ctx);
}

CompoundExpression::CompoundExpression(
    const detail::SignatureBase &signature_,
    std::vector<std::unique_ptr<Expression>> args_)
    : Expression(Kind::CompoundExpression, signature_.result),
      signature(signature_), args(std::move(args_)) {}

std::string CompoundExpression::getOperator() const { return signature.name; }

EvaluationResult
CompoundExpression::evaluate(const EvaluationContext &evaluationParams) const {
...
std::optional<std::size_t> CompoundExpression::getParameterCount() const {
  return signature.params.match(
      [&](const VarargsType &) -> std::optional<std::size_t> { return {}; },
...
void CompoundExpression::eachChild(
