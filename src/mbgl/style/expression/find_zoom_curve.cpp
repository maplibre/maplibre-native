    ...
    e.eachChild([&](const Expression& child) {
        if (const ZoomCurveOrError childResult = findZoomCurve(child)) {
            if (childResult->is<ParsingError>()) {
                ...
                result = {ParsingError{
                    .message=R"("zoom" expression may only be used as input to a top-level "step" or "interpolate" expression.)",
                    .key=""}};
            } else if (result && childResult && result != childResult) {
                result = {ParsingError{
                    .message=R"(Only one zoom-based "step" or "interpolate" subexpression may be used in an expression.)", .key=""}};
            }
        }
    });
