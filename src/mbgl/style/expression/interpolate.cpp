...
template <typename T> class InterpolateImpl : public Interpolate {
public:
  InterpolateImpl(const type::Type &type_, const Interpolator &interpolator_,
                  std::unique_ptr<Expression> input_,
                  // NOLINTNEXTLINE(performance-unnecessary-value-param)
                  std::map<double, std::unique_ptr<Expression>> stops_)
      : Interpolate(type_, interpolator_, std::move(input_),
                    std::move(stops_)) {
    static_assert(
    ...
    if (it == stops.end()) {
      ...
      if (!lower->is<T>()) {
        return EvaluationError{"Expected value to be of type " +
                               toString(valueTypeToExpressionType<T>()) +
                               ", but found " + toString(typeOf(*lower)) +
                               " instead."};
      }
      ...
      if (!upper->is<T>()) {
        return EvaluationError{"Expected value to be of type " +
                               toString(valueTypeToExpressionType<T>()) +
                               ", but found " + toString(typeOf(*upper)) +
                               " instead."};
      }
...
ParseResult parseInterpolate(const Convertible &value, ParsingContext &ctx) {
  ...
  const std::optional<std::string> interpName =
      toString(arrayMember(interp, 0));
...
Interpolate::Interpolate(const type::Type &type_, Interpolator interpolator_,
                         std::unique_ptr<Expression> input_,
                         std::map<double, std::unique_ptr<Expression>> stops_)
    : Expression(Kind::Interpolate, type_),
      interpolator(std::move(interpolator_)), input(std::move(input_)),
      ...
      [&](const ExponentialInterpolator &exponential) {
        if (exponential.base == 1) {
          serialized.emplace_back(
              std::vector<mbgl::Value>{{std::string("linear")}});
        } else {
          serialized.emplace_back(std::vector<mbgl::Value>{
              {std::string("exponential"), exponential.base}});
        }
      },
