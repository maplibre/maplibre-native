...
Glyph FreeTypeFace::rasterizeGlyph(GlyphID glyphID) {
    Glyph fixedMetrics;
    
    FT_Int32 flags = FT_LOAD_DEFAULT;

    FT_Load_Glyph(face,
        (FT_UInt)glyphID.complex.code, // the glyph_index in the font file
        flags
    );
    ...
    for (uint32_t h = 0; h < ftBitmap.rows; ++h) {
        std::memcpy(&fixedMetrics.bitmap.data[size.width * (h + Glyph::borderSize) + Glyph::borderSize], &ftBitmap.buffer[ftBitmap.width * h], ftBitmap.width);
    }
