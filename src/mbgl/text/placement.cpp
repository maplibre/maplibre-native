...
OpacityState::OpacityState(bool placed_, bool skipFade)
    : opacity((skipFade && placed_) ? 1.0f : 0.0f), placed(placed_) {}
...
bool Placement::updateBucketDynamicVertices(SymbolBucket& bucket,
    ...
    if (alongLine) {
        ...
        const auto tileScale =
            static_cast<float>(std::pow(2, state.getZoom() - tile.getOverscaledTileID().overscaledZ));
        ...
        for (std::size_t i = 0; i < bucket.text.placedSymbols.size(); ++i) {
            ...
            if (!variableOffset) {
                ...
                const float perspectiveRatio =
                    0.5f + 0.5f * (state.getCameraToCenterDistance() / projectedAnchor.second);
                float renderTextSize =
                    evaluateSizeForFeature(partiallyEvaluatedSize, symbol) * perspectiveRatio / util::ONE_EM;
                if (pitchWithMap) {
                ...
                if (pitchWithMap) {
                    ...
                    shiftedAnchor =
                        Point<float>(projectedAnchor.first.x + rotated.x, projectedAnchor.first.y + rotated.y);
                } else {
...
void Placement::updateBucketOpacities(SymbolBucket& bucket,
    ...
    for (SymbolInstance& symbolInstance : bucket.symbolInstances) {
        ...
        if (bucket.hasTextCollisionBoxData()) {
            ...
            if (bucket.allowVerticalPlacement && symbolInstance.verticalTextCollisionFeature) {
                verticalTextShift =
                    updateTextCollisionBox(*symbolInstance.verticalTextCollisionFeature, opacityState.text.placed);
            }
        }
