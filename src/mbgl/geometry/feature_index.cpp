...
namespace {
mbgl::LatLng screenCoordinateToLatLng(
...
void FeatureIndex::insert(
    ...
    for (const auto& ring : geometries) {
        ...
        if (envelope.min.x < util::EXTENT && envelope.min.y < util::EXTENT && envelope.max.x >= 0 &&
            envelope.max.y >= 0) {
            grid.insert(
                IndexedSubfeature(index, sourceLayerName, bucketLeaderID, featureSortIndex),
...
void FeatureIndex::query(
    ...
    const SourceFeatureState& sourceFeatureState
) const {
    if (!tileData) {
        return;
    }
...
std::unordered_map<std::string, std::vector<Feature>> FeatureIndex::lookupSymbolFeatures(
    ...
    const FeatureSortOrder& featureSortOrder
) const {
    std::unordered_map<std::string, std::vector<Feature>> result;
    if (!tileData) {
        return result;
    ...
    std::sort(
        ...
        [featureSortOrder](const IndexedSubfeature& a, const IndexedSubfeature& b) {
            ...
            // The 'featureSortOrder' is relatively inefficient for querying but cheap to build on every bucket
            // sort
            if (featureSortOrder) {
...
void FeatureIndex::addFeature(
    ...
    for (const std::string& layerID : bucketLayerIDs.at(indexedFeature.bucketLeaderID)) {
        ...
        if (options.filter && !(*options.filter)(style::expression::EvaluationContext{
                                  static_cast<float>(tileID.z), geometryTileFeature.get()})) {
            continue;
        }
...
std::optional<GeometryCoordinates> FeatureIndex::translateQueryGeometry(
    ...
    GeometryCoordinate translateVec(
        static_cast<int16_t>(translate[0] * pixelsToTileUnits), static_cast<int16_t>(translate[1] * pixelsToTileUnits)
    );
...
void DynamicFeatureIndex::query(
    std::unordered_map<std::string, std::vector<Feature>>& result,
...
void DynamicFeatureIndex::insert(
    std::shared_ptr<Feature> feature, std::shared_ptr<mapbox::geometry::polygon<int64_t>> envelope
) {
