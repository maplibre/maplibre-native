...
void computeTileMasks(
    ...
    // Compute the difference between the root tile ID and the reference tile
    // ID, since TileMask elements are always relative (see below for
    // explanation).
    const uint8_t diffZ = ref.canonical.z - root.z;
    mask.emplace(diffZ, ref.canonical.x - (root.x << diffZ), ref.canonical.y - (root.y << diffZ));
}

// Updates the TileMasks for all renderable tiles. Each renderable tile has a
// corresponding UnwrappedTileID indicating where it should be rendered on the
// screen. A TileMask describes all regions within a renderable tile that are
// *not* covered by other renderable tiles. Example: Renderable tiles in our
// list are 2/1/3, 3/3/6, and 4/5/13. The schematic for creating the TileMask
// for 2/1/3 looks like this:
//
//    ┌────────┬────────┬─────────────────┐
//    │        │        │#################│
//    │ 4/4/12 │ 4/5/12 │#################│
...
//    └─────────────────┴─────────────────┘
//
// The TileMask for 2/1/3 thus consists of the tiles 4/4/12, 4/5/12, 4/4/13,
// 3/2/7, and 3/3/7, but it does *not* include 4/5/13, and 3/3/6, since these
// are other renderable tiles. A TileMask always contains TileIDs *relative* to
// the tile it is generated for, so 2/1/3 is "subtracted" from these TileIDs.
// The final TileMask for 2/1/3 will thus be:
//
//    ┌────────┬────────┬─────────────────┐
//    │        │        │#################│
//    │ 2/0/0  │ 2/1/0  │#################│
...
//    └─────────────────┴─────────────────┘
//
// Only other renderable tiles that are *children* of the renderable tile we are
// generating the mask for will will be considered. For example, adding a
// renderable tile with TileID 4/8/13 won't affect the TileMask for 2/1/3, since
// it is not a descendant of it.
...
void updateTileMasks(RenderableTilesMap& renderables) {
    ...
    for (auto it = renderables.begin(); it != end; ++it) {
        ...
        // by z earlier, so all preceding items cannot be children of the
        // current tile. We also compute the lower bound of the next wrap,
        // because items of the next wrap can never be children of the current
        // wrap.
        auto child_it = std::next(it);
        const auto children_end = std::lower_bound(
            child_it, end, UnwrappedTileID{static_cast<int16_t>(id.wrap + 1), {0, 0, 0}}, [](auto& a, auto& b) {
                return a.first < b;
            });
