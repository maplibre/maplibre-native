...
RasterTile::RasterTile(const OverscaledTileID &id_,
    ...
    : Tile(Kind::Raster, id_), loader(*this, id_, parameters, tileset),
...
void RasterTile::setMetadata(std::optional<Timestamp> modified_,
...
bool RasterTile::layerPropertiesUpdated(
    const Immutable<style::LayerProperties> &) {
  return bool(bucket);
}
