...
SymbolLayout::SymbolLayout(const BucketParameters& parameters,
    ...
    const bool zOrderByViewportY =
        symbolZOrder == SymbolZOrderType::ViewportY || (symbolZOrder == SymbolZOrderType::Auto && !sortFeaturesByKey);
    ...
    for (auto it = features.begin(); it != features.end(); ++it) {
        ...
        if (feature.formattedText && layoutTextSize > 0.0f) {
            ...
            if (!textAlongLine && !variableTextAnchor.empty()) {
                ...
                Shaping shaping =
                    applyShaping(*feature.formattedText, WritingModeType::Horizontal, textAnchor, textJustify);
                if (shaping) {
                ...
                if (textAlongLine && feature.formattedText->allowsVerticalWritingMode()) {
                    ...
                    shapedTextOrientations.vertical =
                        applyShaping(*feature.formattedText, WritingModeType::Vertical, textAnchor, textJustify);
                }
            }
        }
...
std::size_t SymbolLayout::addSymbolGlyphQuads(SymbolBucket& bucket,
    ...
    placedSymbol.angle =
        (allowVerticalPlacement && writingMode == WritingModeType::Vertical) ? static_cast<float>(M_PI_2) : 0.0f;
    ...
    for (const auto& symbolQuad : glyphQuads) {
        ...
        size_t index =
            addSymbol(bucket.text, sizeData, symbolQuad, symbolInstance.anchor, placedSymbol, feature.sortKey);
        if (firstSymbol) {
