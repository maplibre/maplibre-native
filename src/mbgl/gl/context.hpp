...
public:
  ...
  void
  initializeExtensions(const std::function<gl::ProcAddress(const char *)> &);

  void enableDebugging();

  UniqueShader createShader(ShaderType type,
  ...
  Framebuffer createFramebuffer(
      const gfx::Renderbuffer<gfx::RenderbufferPixelType::RGBA> &,
  ...
  template <typename Image,
            gfx::TexturePixelType format = Image::channels == 4
                                               ? gfx::TexturePixelType::RGBA
                                               : gfx::TexturePixelType::Alpha>
  Image readFramebuffer(const Size size, bool flip = true) {
    static_assert(Image::channels ==
                      (format == gfx::TexturePixelType::RGBA ? 4 : 1),
                  "image format mismatch");
    return {size, readFramebuffer(size, format, flip)};
  }

#if !MBGL_USE_GLES2
  template <typename Image> void drawPixels(const Image &image) {
    auto format = image.channels == 4 ? gfx::TexturePixelType::RGBA
                                      : gfx::TexturePixelType::Alpha;
    drawPixels(image.size, image.data.get(), format);
  }
#endif // MBGL_USE_GLES2

  void clear(std::optional<mbgl::Color> color, std::optional<float> depth,
  ...
  void draw(const gfx::DrawMode &, std::size_t indexOffset,
  ...
  // Drain pools and remove abandoned objects, in preparation for destroying the
  // store. Only call this while the OpenGL context is exclusive to this thread.
  void reset();

  bool empty() const {
    return pooledTextures.empty() && abandonedPrograms.empty() &&
           abandonedShaders.empty() && abandonedBuffers.empty() &&
           abandonedTextures.empty() && abandonedVertexArrays.empty() &&
...
public:
  ...
  VertexArrayState globalVertexArrayState{
      UniqueVertexArray(0, {const_cast<Context *>(this)})};
...
private:
  ...
  std::unique_ptr<gfx::OffscreenTexture>
      createOffscreenTexture(Size, gfx::TextureChannelDataType) override;

  std::unique_ptr<gfx::TextureResource>
      createTextureResource(Size, gfx::TexturePixelType,
  ...
  std::unique_ptr<uint8_t[]> readFramebuffer(Size, gfx::TexturePixelType,
                                             bool flip);
#if !MBGL_USE_GLES2
