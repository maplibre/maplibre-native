...
static_assert(underlying_type(ShaderType::Vertex) == GL_VERTEX_SHADER,
              "OpenGL type mismatch");
static_assert(underlying_type(ShaderType::Fragment) == GL_FRAGMENT_SHADER,
              "OpenGL type mismatch");
...
static_assert(underlying_type(UniformDataType::Float) == GL_FLOAT,
              "OpenGL type mismatch");
static_assert(underlying_type(UniformDataType::FloatVec2) == GL_FLOAT_VEC2,
              "OpenGL type mismatch");
static_assert(underlying_type(UniformDataType::FloatVec3) == GL_FLOAT_VEC3,
              "OpenGL type mismatch");
static_assert(underlying_type(UniformDataType::FloatVec4) == GL_FLOAT_VEC4,
              "OpenGL type mismatch");
static_assert(underlying_type(UniformDataType::Int) == GL_INT,
              "OpenGL type mismatch");
static_assert(underlying_type(UniformDataType::IntVec2) == GL_INT_VEC2,
              "OpenGL type mismatch");
static_assert(underlying_type(UniformDataType::IntVec3) == GL_INT_VEC3,
              "OpenGL type mismatch");
static_assert(underlying_type(UniformDataType::IntVec4) == GL_INT_VEC4,
              "OpenGL type mismatch");
static_assert(underlying_type(UniformDataType::Bool) == GL_BOOL,
              "OpenGL type mismatch");
static_assert(underlying_type(UniformDataType::BoolVec2) == GL_BOOL_VEC2,
              "OpenGL type mismatch");
static_assert(underlying_type(UniformDataType::BoolVec3) == GL_BOOL_VEC3,
              "OpenGL type mismatch");
static_assert(underlying_type(UniformDataType::BoolVec4) == GL_BOOL_VEC4,
              "OpenGL type mismatch");
static_assert(underlying_type(UniformDataType::FloatMat2) == GL_FLOAT_MAT2,
              "OpenGL type mismatch");
static_assert(underlying_type(UniformDataType::FloatMat3) == GL_FLOAT_MAT3,
              "OpenGL type mismatch");
static_assert(underlying_type(UniformDataType::FloatMat4) == GL_FLOAT_MAT4,
              "OpenGL type mismatch");
static_assert(underlying_type(UniformDataType::Sampler2D) == GL_SAMPLER_2D,
              "OpenGL type mismatch");
...
void Context::initializeExtensions(
  ...
  if (const auto *extensions = reinterpret_cast<const char *>(
          MBGL_CHECK_ERROR(glGetString(GL_EXTENSIONS)))) {

    auto fn =
        [&](std::initializer_list<std::pair<const char *, const char *>> probes)
        -> ProcAddress {
    ...
    // Block Adreno 4xx as it crashes in a driver when VBOs are destructed
    // (Android 5.1.1) Block ARM Mali-T720 (in some MT8163 chipsets) as it
    // crashes on glBindVertexArray Block ANGLE on Direct3D as the combination
    // of Qt + Windows + ANGLE leads to crashes
    ...
    constexpr const char *halfFloatColorBufferExtensionName =
...
void Context::enableDebugging() {
  if (!debugging || !debugging->debugMessageControl ||
      !debugging->debugMessageCallback) {
    return;
  }
  ...
  MBGL_CHECK_ERROR(debugging->debugMessageControl(
      GL_DONT_CARE, GL_DONT_CARE, GL_DEBUG_SEVERITY_HIGH, 0, nullptr, GL_TRUE));
  MBGL_CHECK_ERROR(debugging->debugMessageControl(GL_DONT_CARE, GL_DONT_CARE,
  ...
  MBGL_CHECK_ERROR(debugging->debugMessageCallback(
      extension::Debugging::DebugCallback, nullptr));
}

UniqueShader
Context::createShader(ShaderType type,
                      const std::initializer_list<const char *> &sources) {
  UniqueShader result{
      MBGL_CHECK_ERROR(glCreateShader(static_cast<GLenum>(type))), {this}};

  MBGL_CHECK_ERROR(glShaderSource(result, static_cast<GLsizei>(sources.size()),
                                  sources.begin(), nullptr));
...
UniqueProgram Context::createProgram(ShaderID vertexShader,
                                     ShaderID fragmentShader,
                                     const char *location0AttribName) {
...
bool Context::supportsVertexArrays() const {
  return vertexArray && vertexArray->genVertexArrays &&
...
VertexArray Context::createVertexArray() {
  if (supportsVertexArrays()) {
    ...
    // On GL implementations which do not support vertex arrays, attribute
    // bindings are global state. So return a VertexArray which shares our
    // global state tracking and whose deleter is a no-op.
...
Context::createTextureResource(const Size size,
  ...
  MBGL_CHECK_ERROR(glTexImage2D(
      GL_TEXTURE_2D, 0, Enum<gfx::TexturePixelType>::to(format), size.width,
      size.height, 0, Enum<gfx::TexturePixelType>::to(format),
      Enum<gfx::TextureChannelDataType>::to(type), nullptr));

  // We are using clamp to edge here since OpenGL ES doesn't allow GL_REPEAT on
  // NPOT textures. We use those when the pixelRatio isn't a power of two, e.g.
  // on iPhone 6 Plus.
  MBGL_CHECK_ERROR(
...
Context::createRenderbufferResource(const gfx::RenderbufferPixelType type,
                                    const Size size) {
  RenderbufferID id = 0;
  ...
  MBGL_CHECK_ERROR(glRenderbufferStorage(
      GL_RENDERBUFFER, Enum<gfx::RenderbufferPixelType>::to(type), size.width,
      size.height));
  bindRenderbuffer = 0;
...
Context::readFramebuffer(const Size size, const gfx::TexturePixelType format,
                         const bool flip) {
  const size_t stride =
      size.width * (format == gfx::TexturePixelType::RGBA ? 4 : 1);
...
#if !MBGL_USE_GLES2
void Context::drawPixels(const Size size, const void *data,
  ...
  MBGL_CHECK_ERROR(glDrawPixels(size.width, size.height,
                                Enum<gfx::TexturePixelType>::to(format),
  ...
  if (status != GL_FRAMEBUFFER_COMPLETE) {
    ...
    case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT:
      throw std::runtime_error(
          "Couldn't create framebuffer: incomplete attachment");
    ...
    case GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER:
      throw std::runtime_error(
          "Couldn't create framebuffer: incomplete draw buffer");
#endif
#ifdef GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER
    case GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER:
      throw std::runtime_error(
          "Couldn't create framebuffer: incomplete read buffer");
#endif
#ifdef GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS
    case GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS:
      throw std::runtime_error(
          "Couldn't create framebuffer: incomplete dimensions");
#endif
...
void bindDepthStencilRenderbuffer(
    const gfx::Renderbuffer<gfx::RenderbufferPixelType::DepthStencil>
        &depthStencil) {
  auto &depthStencilResource =
      depthStencil.getResource<gl::RenderbufferResource>();
...
Framebuffer Context::createFramebuffer(
  ...
  MBGL_CHECK_ERROR(
      glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,
                                GL_RENDERBUFFER, depthResource.renderbuffer));
  checkFramebuffer();
...
void Context::reset() {
  std::copy(pooledTextures.begin(), pooledTextures.end(),
            std::back_inserter(abandonedTextures));
...
void Context::setDepthMode(const gfx::DepthMode &depth) {
  if (depth.func == gfx::DepthFunctionType::Always &&
      depth.mask != gfx::DepthMaskType::ReadWrite) {
    depthTest = false;

    // Workaround for rendering errors on Adreno 2xx GPUs. Depth-related state
    // should not matter when the depth test is disabled, but on these GPUs it
    // apparently does. https://github.com/mapbox/mapbox-gl-native/issues/9164
...
gfx::RenderingStats &Context::renderingStats() { return stats; }

const gfx::RenderingStats &Context::renderingStats() const { return stats; }

void Context::finish() { MBGL_CHECK_ERROR(glFinish()); }

void Context::draw(const gfx::DrawMode &drawMode, std::size_t indexOffset,
...
void Context::performCleanup() {
  ...
  if (!abandonedTextures.empty()) {
    ...
    MBGL_CHECK_ERROR(glDeleteTextures(int(abandonedTextures.size()),
  ...
  if (!abandonedVertexArrays.empty()) {
    ...
    MBGL_CHECK_ERROR(vertexArray->deleteVertexArrays(
        int(abandonedVertexArrays.size()), abandonedVertexArrays.data()));
  ...
  if (!abandonedFramebuffers.empty()) {
    ...
    MBGL_CHECK_ERROR(glDeleteFramebuffers(int(abandonedFramebuffers.size()),
                                          abandonedFramebuffers.data()));
...
void Context::visualizeStencilBuffer() {
  ...
  auto image = readFramebuffer<AlphaImage, gfx::TexturePixelType::Stencil>(
      viewportValue.size, false);
...
void Context::visualizeDepthBuffer(const float depthRangeSize) {
  ...
  // Scales the values in the depth buffer so that they cover the entire
  // grayscale range. This makes it easier to spot tiny differences.
  const float base = 1.0f / (1.0f - depthRangeSize);
  pixelTransferDepth = {base, 1.0f - base};

  // Read the stencil buffer
  auto viewportValue = viewport.getCurrentValue();
  auto image = readFramebuffer<AlphaImage, gfx::TexturePixelType::Depth>(
      viewportValue.size, false);
