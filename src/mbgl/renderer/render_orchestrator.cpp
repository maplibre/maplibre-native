...
std::unique_ptr<RenderTree> RenderOrchestrator::createRenderTree(
        ...
        for (std::size_t index = 0; index < orderedLayers.size(); ++index) {
            ...
            if (layerIsVisible && zoomFitsLayer && sourceImpl.get() == sourceImpls->at(0).get()) {
                ...
                renderItemsEmplaceHint =
                    layerRenderItems.emplace_hint(renderItemsEmplaceHint, layer, nullptr, static_cast<uint32_t>(index));
            }
        }
    ...
    if (isMapModeContinuous) {
        ...
        renderTreeParameters->placementChanged =
            !placementController.placementIsRecent(updateParameters->timePoint,
...
std::vector<Feature> RenderOrchestrator::queryRenderedFeatures(
    ...
    for (const auto& sourceID : sourceIDs) {
        if (RenderSource* renderSource = getRenderSource(sourceID)) {
            auto sourceResults =
                renderSource->queryRenderedFeatures(geometry, transformState, filteredLayers, options, projMatrix);
            std::move(
