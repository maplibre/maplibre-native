    ...
    for (auto& updatedImageVersion : imageManager.updatedImageVersions) {
        ...
        if (it != imagePositions.end()) {
            ...
            if (maxPatchWidth < imagePatch->image.size.width || maxPatchHeight < imagePatch->image.size.height) {
                ...
                // won't fit the new image. ImageManager is unaware of the the atlas packing. One possibility is to
                // repack the atlas. A workaround implemented here is to resize the image to fit the paddedRect
                const auto& imageImpl = *updatedImage->get();
                Log::Error(Event::General,
                           imageImpl.id + " does not fit the atlas (Issue #xxxx). " + imageImpl.id +
                ...
                resizedImage.resize({maxPatchWidth, maxPatchHeight});
