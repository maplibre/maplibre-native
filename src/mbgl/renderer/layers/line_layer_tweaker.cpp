...
void LineLayerTweaker::execute(LayerGroup& layerGroup, const PaintParameters& parameters) {
    ...
    layerGroup.observeDrawables([&](gfx::Drawable& drawable) {
        ...
        /* mat4 matrix = renderTile.translatedMatrix(properties.get<LineTranslate>(), properties.get<LineTranslateAnchor>(), state) */
        const UnwrappedTileID tileID = drawable.getTileID()->toUnwrapped();
        mat4 matrix;
        parameters.state.matrixFor(matrix, tileID);
        matrix = translateVtxMatrix(tileID, matrix, evaluated.get<LineTranslate>(), evaluated.get<LineTranslateAnchor>(), parameters.state, false);
