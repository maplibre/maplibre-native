...
public:
    ...
    const optional<std::thread::id>& getRenderThreadID() const { return renderThreadID; }
    void setRenderThreadID(optional<std::thread::id> id) { renderThreadID = id; }

protected:
    Bucket() = default;
    std::atomic<bool> uploaded { false };

    optional<std::thread::id> renderThreadID;
};
