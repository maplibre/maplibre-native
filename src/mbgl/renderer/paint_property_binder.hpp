...
template <size_t N>
std::array<float, N * 2> zoomInterpolatedAttributeValue(
    const std::array<float, N>& min, const std::array<float, N>& max
) {
...
public:
    ...
    virtual void
    populateVertexVector(const GeometryTileFeature& feature, std::size_t length, std::size_t index, const ImagePositions&, const std::optional<PatternDependency>&, const CanonicalTileID& canonical, const style::expression::Value&) = 0;
    ...
    virtual void
    setPatternParameters(const std::optional<ImagePosition>&, const std::optional<ImagePosition>&, const CrossfadeParameters&) = 0;
    virtual std::tuple<ExpandToType<As, std::optional<gfx::AttributeBinding>>...> attributeBinding(
        const PossiblyEvaluatedType& currentValue
    ) const = 0;
    virtual std::tuple<ExpandToType<As, float>...> interpolationFactor(float currentZoom) const = 0;
    virtual std::tuple<ExpandToType<As, UniformValueType>...> uniformValue(const PossiblyEvaluatedType& currentValue
    ) const = 0;
...
public:
    ...
    void
    populateVertexVector(const GeometryTileFeature&, std::size_t, std::size_t, const ImagePositions&, const std::optional<PatternDependency>&, const CanonicalTileID&, const style::expression::Value&)
        override {}
    void updateVertexVector(std::size_t, std::size_t, const GeometryTileFeature&, const FeatureState&) override {}
    void upload(gfx::UploadPass&) override {}
    void
    setPatternParameters(const std::optional<ImagePosition>&, const std::optional<ImagePosition>&, const CrossfadeParameters&)
        override{};

    std::tuple<std::optional<gfx::AttributeBinding>> attributeBinding(const PossiblyEvaluatedPropertyValue<T>&)
        const override {
        return {};
    }
...
public:
    ...
    void
    setPatternParameters(const std::optional<ImagePosition>& posA, const std::optional<ImagePosition>& posB, const CrossfadeParameters&)
        override {
        if (!posA || !posB) {
            return;
        } else {
            constantPatternPositions = std::tuple<std::array<uint16_t, 4>, std::array<uint16_t, 4>>{
                posB->tlbr(), posA->tlbr()};
        }
    }
    ...
    std::tuple<std::array<uint16_t, 4>, std::array<uint16_t, 4>>
    uniformValue(const PossiblyEvaluatedPropertyValue<Faded<T>>&) const override {
...
public:
    ...
    void updateVertexVectors(const FeatureStates& states, const GeometryTileLayer& layer, const ImagePositions&)
        override {
    ...
    void updateVertexVector(
        std::size_t start, std::size_t end, const GeometryTileFeature& feature, const FeatureState& state
    ) override {
        using style::expression::EvaluationContext;
    ...
    std::tuple<std::optional<gfx::AttributeBinding>> attributeBinding(
        const PossiblyEvaluatedPropertyValue<T>& currentValue
    ) const override {
...
public:
    ...
    void populateVertexVector(
        ...
        Range<T> range = {
            expression.evaluate(
                EvaluationContext(zoomRange.min, &feature)
                    .withFormattedSection(&formattedSection)
                    .withCanonicalTileID(&canonical),
                defaultValue
            ),
            expression.evaluate(
                EvaluationContext(zoomRange.max, &feature)
                    .withFormattedSection(&formattedSection)
                    .withCanonicalTileID(&canonical),
                defaultValue
            ),
...
public:
    ...
    void setPatternParameters(
        const std::optional<ImagePosition>&, const std::optional<ImagePosition>&, const CrossfadeParameters& crossfade_
    ) override {
        crossfade = crossfade_;
    };

    void
    populateVertexVector(const GeometryTileFeature&, std::size_t length, std::size_t /* index */, const ImagePositions& patternPositions, const std::optional<PatternDependency>& patternDependencies, const CanonicalTileID&, const style::expression::Value&)
        override {
        if (!patternDependencies || patternDependencies->mid.empty()) {
            // Unlike other propperties with expressions that evaluate to null, the default value for `*-pattern`
            // properties is an empty string and will not have a valid entry in patternPositions. We still need to
            // populate the attribute buffers to avoid crashes when we try to draw the layer because we don't know at
            // draw time if all features were evaluated to valid pattern dependencies.
    ...
    std::tuple<std::optional<gfx::AttributeBinding>, std::optional<gfx::AttributeBinding>> attributeBinding(
        const PossiblyEvaluatedPropertyValue<Faded<T>>& currentValue
    ) const override {
        if (currentValue.isConstant()) {
            return {};
        } else {
...
PaintPropertyBinder<T, UniformValueType, PossiblyEvaluatedType, As...>::create(
    const PossiblyEvaluatedType& value, float zoom, T defaultValue
) {
...
public:
    ...
    void populateVertexVectors(
        ...
        util::ignore(
            {(binders.template get<Ps>()->populateVertexVector(
    ...
    void updateVertexVectors(
        const FeatureStates& states, const GeometryTileLayer& layer, const ImagePositions& imagePositions
    ) {
        util::ignore({(binders.template get<Ps>()->updateVertexVectors(states, layer, imagePositions), 0)...});
    }

    void setPatternParameters(
        const std::optional<ImagePosition>& posA,
