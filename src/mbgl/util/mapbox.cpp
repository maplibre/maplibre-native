...
std::map<std::string, std::string> createTokenMap(const std::string& urlTemplate, const std::string& url) {
    ...
    std::string templatePattern =
        std::regex_replace(urlTemplate, tokenPattern, "(.+)", std::regex_constants::match_any);
...
std::string makeQueryString(const TileServerOptions& tileServerOptions, const std::string& apiKey) {
    std::string queryString =
        (!tileServerOptions.requiresApiKey() || tileServerOptions.apiKeyParameterName().empty() || apiKey.empty())
            ? ""
