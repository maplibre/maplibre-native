...
void writeJSON(rapidjson::PrettyWriter<rapidjson::StringBuffer> &writer,
  ...
  value.match(
      ...
      [&writer](double d) {
        d == std::floor(d) ? writer.Int64(static_cast<int64_t>(d))
                           : writer.Double(d);
      },
...
std::string toJSON(const std::vector<mbgl::Feature> &features, unsigned indent,
                   bool singleLine) {
  ...
  for (const auto &feature : features) {
    ...
    if (!feature.id.is<mbgl::NullValue>()) {
      result.AddMember("id",
                       mapbox::geojson::identifier::visit(
                           feature.id, mapbox::geojson::to_value{allocator}),
                       allocator);
    }
    result.AddMember("geometry",
                     mapbox::geojson::convert(feature.geometry, allocator),
                     allocator);
    result.AddMember("properties",
                     mapbox::geojson::to_value{allocator}(feature.properties),
                     allocator);
    ...
    result.AddMember("state",
                     mapbox::geojson::to_value{allocator}(feature.state),
                     allocator);
...
readExpectedEntries(const std::regex &regex,
                    const mbgl::filesystem::path &base) {
...
std::vector<std::string>
readExpectedImageEntries(const mbgl::filesystem::path &base) {
  static const std::regex regex(".*/expected.*.png");
  return readExpectedEntries(regex, base);
}

std::vector<std::string>
readExpectedMetricEntries(const mbgl::filesystem::path &base) {
  static const std::regex regex(".*/metrics.json");
  return readExpectedEntries(regex, base);
}

std::vector<std::string>
readExpectedJSONEntries(const mbgl::filesystem::path &base) {
...
TestMetrics readExpectedMetrics(const mbgl::filesystem::path &path) {
  ...
  if (document.HasMember("file-size")) {
    ...
    for (auto &probeValue : fileSizeValue.GetArray()) {
      ...
      std::string mark{probeValue[0].GetString(),
                       probeValue[0].GetStringLength()};
      assert(!mark.empty());

      std::string filePath{probeValue[1].GetString(),
                           probeValue[1].GetStringLength()};
      assert(!filePath.empty());

      result.fileSize.emplace(
          std::piecewise_construct, std::forward_as_tuple(std::move(mark)),
  ...
  if (document.HasMember("memory")) {
    ...
    for (auto &probeValue : memoryValue.GetArray()) {
      ...
      result.memory.emplace(std::piecewise_construct,
                            ...
                            std::forward_as_tuple(probeValue[1].GetUint64(),
                                                  probeValue[2].GetUint64()));
    }
  }
  ...
  if (document.HasMember("fps")) {
    ...
    for (auto &probeValue : fpsValue.GetArray()) {
      ...
      const std::string mark{probeValue[0].GetString(),
                             probeValue[0].GetStringLength()};
      assert(!mark.empty());
      result.fps.insert({std::move(mark),
                         {probeValue[1].GetFloat(), probeValue[2].GetFloat(),
                          probeValue[3].GetFloat()}});
    }
  }
...
TestMetadata parseTestMetadata(const TestPaths &paths) {
  ...
  if (!maybeJson.is<mbgl::JSDocument>()) { // NOLINT
    metadata.errorMessage =
        std::string("Unable to parse: ") + metadata.paths.stylePath.string();
    return metadata;
  }
  ...
  if (testValue.HasMember("width")) {
    ...
    if (metadata.mapMode == mbgl::MapMode::Tile) {
      mbgl::Log::Warning(
          ...
          "The 'width' metadata field is ignored in tile map mode");
    } else {
  ...
  if (testValue.HasMember("height")) {
    ...
    if (metadata.mapMode == mbgl::MapMode::Tile) {
      mbgl::Log::Warning(
          mbgl::Event::ParseStyle,
          "The 'height' metadata field is ignored in tile map mode");
    } else {
  ...
  if (testValue.HasMember("description")) {
    ...
    metadata.description =
        std::string{testValue["description"].GetString(),
  ...
  if (testValue.HasMember("crossSourceCollisions")) {
    ...
    metadata.crossSourceCollisions =
        testValue["crossSourceCollisions"].GetBool();
  }
  ...
  if (testValue.HasMember("queryGeometry")) {
    ...
    if (testValue["queryGeometry"][0].IsNumber() &&
        testValue["queryGeometry"][1].IsNumber()) {
      metadata.queryGeometry.x = testValue["queryGeometry"][0].GetDouble();
      metadata.queryGeometry.y = testValue["queryGeometry"][1].GetDouble();
    } else if (testValue["queryGeometry"][0].IsArray() &&
               testValue["queryGeometry"][1].IsArray()) {
      metadata.queryGeometryBox.min.x =
          testValue["queryGeometry"][0][0].GetDouble();
      metadata.queryGeometryBox.min.y =
          testValue["queryGeometry"][0][1].GetDouble();
      metadata.queryGeometryBox.max.x =
          testValue["queryGeometry"][1][0].GetDouble();
      metadata.queryGeometryBox.max.y =
          testValue["queryGeometry"][1][1].GetDouble();
    }
    ...
    if (testValue["queryOptions"].HasMember("layers")) {
      ...
      for (uint32_t i = 0; i < layersArray.Size(); i++) {
        layersVec.emplace_back(
            testValue["queryOptions"]["layers"][i].GetString());
      }
...
TestOperations parseTestOperations(TestMetadata &metadata) {
  ...
  const auto &operationsArray =
      metadata.document["metadata"]["test"]["operations"].GetArray();
  ...
  for (auto &operationArray : operationsArray) {
    ...
    if (operationArray[0].GetString() == waitOp) {
    ...
    } else if (operationArray[0].GetString() == addImageOp ||
               operationArray[0].GetString() == updateImageOp) {
      ...
      result.emplace_back([imageName, imagePath, sdf,
        ...
        if (!maybeImage) {
          ctx.getMetadata().errorMessage +=
              std::string("Failed to load expected image ") + imagePath;
          return false;
        }

        ctx.getMap().getStyle().addImage(std::make_unique<mbgl::style::Image>(
            imageName, mbgl::decodeImage(*maybeImage), pixelRatio, sdf));
        return true;
      });
      ...
      std::string imageName{operationArray[1].GetString(),
                            operationArray[1].GetStringLength()};
      ...
      mbgl::LatLng center{centerArray[1].GetDouble(),
                          centerArray[0].GetDouble()};
      ...
      mbgl::style::conversion::Error error;
      auto converted = mbgl::style::conversion::convert<mbgl::style::Light>(
          operationArray[1], error);
      if (!converted) {
        metadata.errorMessage =
            std::string("Unable to convert light: ") + error.message;
        return {};
      }
      result.emplace_back([impl = converted->impl](TestContext &ctx) {
        ctx.getMap().getStyle().setLight(
            std::make_unique<mbgl::style::Light>(impl));
        return true;
      });
      ...
      assert(operationArray[1].IsObject());
      result.emplace_back(
          [json = serializeJsonValue(operationArray[1])](TestContext &ctx) {
            mbgl::style::conversion::Error error;
            auto converted = mbgl::style::conversion::convertJSON<
                std::unique_ptr<mbgl::style::Layer>>(json, error);
            if (!converted) {
              ctx.getMetadata().errorMessage =
                  std::string("Unable to convert layer: ") + error.message;
              return false;
            }
      ...
      result.emplace_back(
          [sourceName,
           json = serializeJsonValue(operationArray[2])](TestContext &ctx) {
            mbgl::style::conversion::Error error;
            auto converted = mbgl::style::conversion::convertJSON<
                std::unique_ptr<mbgl::style::Source>>(json, error, sourceName);
            if (!converted) {
              ctx.getMetadata().errorMessage =
                  std::string("Unable to convert source: ") + error.message;
              return false;
            }
      ...
      std::string layerName{operationArray[1].GetString(),
                            operationArray[1].GetStringLength()};
      std::string propertyName{operationArray[2].GetString(),
                               operationArray[2].GetStringLength()};
      result.emplace_back(
          [layerName, propertyName,
           json = serializeJsonValue(operationArray[3])](TestContext &ctx) {
            auto layer = ctx.getMap().getStyle().getLayer(layerName);
            if (!layer) {
              ctx.getMetadata().errorMessage =
                  std::string("Layer not found: ") + layerName;
      ...
      std::string mark = std::string(operationArray[1].GetString(),
                                     operationArray[1].GetStringLength());
      std::string path = std::string(operationArray[2].GetString(),
                                     operationArray[2].GetStringLength());
      assert(!path.empty());
      ...
      if (operationArray.Size() == 5) {
        ...
        assert(std::string(operationArray[4].GetString(),
                           operationArray[4].GetStringLength()) ==
               "compressed");
        compressed = true;
      }
      ...
      result.emplace_back([filePath, path, mark, tolerance,
                           compressed](TestContext &ctx) {
      ...
      result.emplace_back([mark, tolerance](TestContext &ctx) {
        ...
        if (tolerance >= 0.0f)
          emplaced.first->second.tolerance = tolerance;
        return true;
      });
      ...
      const std::function<std::optional<Error>(const std::string &,
        ...
        if (!valueParsed) {
          metadata.errorMessage =
              std::string("Could not get feature state value, state key: ") + k;
          return {};
        }
        stateKey = k;
      ...
      result.emplace_back(
          [sourceID, sourceLayer, featureID, parsedState](TestContext &ctx) {
            ...
            frontend.getRenderer()->setFeatureState(sourceID, sourceLayer,
                                                    featureID, parsedState);
            return true;
          });
      ...
      result.emplace_back([sourceID, sourceLayer, featureID](TestContext &ctx) {
        ...
        frontend.getRenderer()->getFeatureState(state, sourceID, sourceLayer,
                                                featureID);
        return true;
      });
      ...
      result.emplace_back(
          [sourceID, sourceLayer, featureID, stateKey](TestContext &ctx) {
            auto &frontend = ctx.getFrontend();
            try {
              frontend.render(ctx.getMap());
            } catch (const std::exception &) {
            ...
            frontend.getRenderer()->removeFeatureState(sourceID, sourceLayer,
                                                       featureID, stateKey);
            return true;
          });
      ...
      if (metadata.mapMode != mbgl::MapMode::Continuous) {
        metadata.errorMessage =
            "Map mode must be Continous for " + panGestureOp + " operation";
        return {};
      }
      ...
      std::tie(startPos, startZoom) =
          parsePosition(operationArray[3].GetArray());
      std::tie(endPos, endZoom) = parsePosition(operationArray[4].GetArray());

      result.emplace_back([mark, duration, startPos, endPos, startZoom,
                           endZoom](TestContext &ctx) {
        auto &map = ctx.getMap();
        auto &observer = ctx.getObserver();
        auto &frontend = ctx.getFrontend();
        std::vector<float> samples;
        // Jump to the starting point of the segment and make sure there's
        // something to render
        map.jumpTo(
            mbgl::CameraOptions().withCenter(startPos).withZoom(startZoom));
        ...
        for (auto it = samples.rbegin(); it != samples.rbegin() + sampleCount;
             it++)
          minFrameTime += *it;

        float minOnePcFps = sampleCount / minFrameTime;

        ctx.getMetadata().metrics.fps.insert(
            {mark, {averageFps, minOnePcFps, 0.0f}});
        return true;
      });
      ...
      result.emplace_back([mark](TestContext &ctx) {
        ...
        metricProbe.memIndexBuffers.peak -=
            ctx.baselineGfxProbe.memIndexBuffers.peak;
        metricProbe.memVertexBuffers.peak -=
            ctx.baselineGfxProbe.memVertexBuffers.peak;
        metricProbe.memTextures.peak -= ctx.baselineGfxProbe.memTextures.peak;
        ctx.getMetadata().metrics.gfx.insert({mark, metricProbe});
        return true;
      });
    } else {
      metadata.errorMessage = std::string("Unsupported operation: ") +
                              operationArray[0].GetString();
      return {};
    }
  }
...
std::string encodeBase64(const std::string &data) {
  ...
  std::stringstream os;
  std::copy(base64(data.c_str()), base64(data.c_str() + data.size()),
            ostream_iterator<char>(os));
  return os.str();
}

std::string createResultItem(const TestMetadata &metadata,
                             bool hasFailedTests) {
  const bool shouldHide =
      (hasFailedTests && metadata.status == "passed") ||
      (metadata.status.find("ignored") != std::string::npos);

  std::string html;
  html.append("<div class=\"test " + metadata.status +
              (shouldHide ? " hide" : "") + "\">\n");
  html.append(R"(<h2><span class="label" style="background: )" +
              metadata.color + "\">" + metadata.status + "</span> " +
              metadata.id + "</h2>\n");
  ...
  if (!metadata.renderErrored) {
    if (metadata.outputsImage) {
      if (metadata.renderTest) {
        ...
        html.append(" src=\"data:image/png;base64," +
                    encodeBase64(metadata.actual) + "\"");
        html.append(" data-alt-src=\"data:image/png;base64," +
                    encodeBase64(metadata.expected) + "\">\n");

        html.append("<img width=" + mbgl::util::toString(metadata.size.width));
        html.append(" height=" + mbgl::util::toString(metadata.size.height));
        html.append(" src=\"data:image/png;base64," +
                    encodeBase64(metadata.diff) + "\">\n");
      } else {
        html.append("<img width=" + mbgl::util::toString(metadata.size.width));
        html.append(" height=" + mbgl::util::toString(metadata.size.height));
        html.append(" src=\"data:image/png;base64," +
  ...
  if (metadata.difference != 0.0) {
    if (metadata.renderTest) {
      html.append("<p class=\"diff\"><strong>Diff:</strong> " +
                  mbgl::util::toString(metadata.difference) + "</p>\n");
    } else {
      html.append("<p class=\"diff\"><strong>Diff:</strong> " + metadata.diff +
                  "</p>\n");
    }
  }
  html.append("</div>\n");
...
std::string createResultPage(const TestStatistics &stats,
  ...
  if (stats.ignoreFailedTests) {
    resultsPage.append(mbgl::util::toString(stats.ignoreFailedTests) +
                       " ignored failed, ");
  }
  if (stats.ignorePassedTests) {
    resultsPage.append(mbgl::util::toString(stats.ignorePassedTests) +
                       " ignored passed, ");
  }
  ...
  // Test sequence
  {
    ...
    // Shuffle
    if (shuffle) {
      resultsPage.append("<p><strong>Shuffle seed</strong>: " +
                         mbgl::util::toString(seed) + "</p>\n");
    }
