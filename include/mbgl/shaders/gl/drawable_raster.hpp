...
struct ShaderSource<BuiltIn::RasterShader, gfx::Backend::Type::OpenGL> {
    ...
    static constexpr const char* vertexData = R"(layout (std140) uniform RasterDrawableUBO {
    highp mat4 u_matrix;
...
void main() {
    ...
    static constexpr const char* fragmentData = R"(layout (std140) uniform RasterDrawableUBO {
    highp mat4 u_matrix;
    highp vec3 u_spin_weights;
    highp vec2 u_tl_parent;
    highp float u_scale_parent;
...
void main() {
    ...
    static std::string vertex() {
        using Ty = ShaderSource<BuiltIn::RasterShader, gfx::Backend::Type::OpenGL>;
