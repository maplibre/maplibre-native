...
fn main(in: VertexInput) -> VertexOutput {
    ...
    out.frag_position = (vec2<f32>(f32(in.position.x), f32(in.position.y)) / 8192.0) * scale + epsilon;
...
fn getElevationStop(stop: i32, color_ramp_size: i32) -> f32 {
    // Elevation stops are stored in a 1D texture as normalized coordinates
