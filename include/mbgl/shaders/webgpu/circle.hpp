...
fn main(in: VertexInput) -> VertexOutput {
    ...
    let stroke_opacity = unpack_mix_float(in.stroke_opacity, drawable.stroke_opacity_t);
    ...
    let world_pos = circle_center + scaled_extrude * radius_with_stroke;

    out.position = drawable.matrix * vec4<f32>(world_pos, 0.0, 1.0);
    out.extrude = extrude;
    out.circle_data = vec4<f32>(1.0 / radius_with_stroke, radius, blur, opacity);
