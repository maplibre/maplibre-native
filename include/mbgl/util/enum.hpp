...
#define MBGL_DEFINE_ENUM(T, ...)                                                                                    \
    ...
    const char* Enum<T>::toString(T t) {                                                                            \
        auto it =                                                                                                   \
            std::find_if(std::begin(T##_names), std::end(T##_names), [&](const auto& v) { return t == v.first; });  \
    ...
    std::optional<T> Enum<T>::toEnum(const std::string& s) {                                                        \
        auto it =                                                                                                   \
            std::find_if(std::begin(T##_names), std::end(T##_names), [&](const auto& v) { return s == v.second; }); \
