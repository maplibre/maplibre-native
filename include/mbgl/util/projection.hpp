...
public:
    ...
    static double getMetersPerPixelAtLatitude(double lat, double zoom) {
        ...
        return std::cos(constrainedLatitude * util::DEG2RAD_D) * util::M2PI * util::EARTH_RADIUS_M /
    ...
    static LatLng latLngForProjectedMeters(const ProjectedMeters& projectedMeters) {
        double latitude = (2 * std::atan(std::exp(projectedMeters.northing() / util::EARTH_RADIUS_M)) - (M_PI / 2.0)) *
                          util::RAD2DEG_D;
...
private:
    static Point<double> project_(const LatLng& latLng, double worldSize) {
        ...
        return Point<double>{util::LONGITUDE_MAX + latLng.longitude(),
                             util::LONGITUDE_MAX -
                                 util::RAD2DEG_D * std::log(std::tan(M_PI / 4 + latitude * M_PI / util::DEGREES_MAX))} *
