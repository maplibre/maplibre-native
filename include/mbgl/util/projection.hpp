...
public:
    ...
    static double getMetersPerPixelAtLatitude(double lat, double zoom) {
        ...
        return std::cos(util::DEG2RAD_D(constrainedLatitude)) * util::M2PI * util::EARTH_RADIUS_M / worldSize(constrainedScale);
    }
    ...
    static ProjectedMeters projectedMetersForLatLng(const LatLng& latLng) {
        ...
        const double easting  = util::DEG2RAD_D(util::EARTH_RADIUS_M * constrainedLongitude);
    ...
    static LatLng latLngForProjectedMeters(const ProjectedMeters& projectedMeters) {
        double latitude = util::RAD2DEG_D(2 * std::atan(std::exp(projectedMeters.northing() / util::EARTH_RADIUS_M)) - (M_PI / 2.0));
        double longitude = util::RAD2DEG_D(projectedMeters.easting()) / util::EARTH_RADIUS_M;
    ...
    static LatLng unproject(const Point<double>& p, double scale, LatLng::WrapMode wrapMode = LatLng::Unwrapped) {
        ...
        return LatLng {
            util::DEGREES_MAX / M_PI * std::atan(std::exp(util::DEG2RAD_D(util::LONGITUDE_MAX - p2.y))) - 90.0,
