...
Both Bazel and CMake run `scripts/generate_metal_shader_code.ts` during a Metal
...
To inspect the output manually with Node.js 24 (see `.nvmrc`):

```sh
node scripts/generate_metal_shader_code.ts --out /tmp/metal-shader-source.cpp
```
