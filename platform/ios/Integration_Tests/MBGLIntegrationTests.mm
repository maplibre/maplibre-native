...
- (void)testOpenGLLayerDoesNotLeakWhenStyleChanged {
  ...
  // At this point the C++ CustomLayer will have been destroyed, and the rawLayer pointer has been
  // NULLed
...
- (void)testOpenGLLayerDoesNotLeakWhenMapViewDeallocs {
  ...
  @autoreleasepool {
    NSURL *styleURL = [[NSBundle bundleForClass:[self class]] URLForResource:@"one-liner"
                                                               withExtension:@"json"];
    MLNMapView *mapView2 = [[MLNMapView alloc] initWithFrame:UIScreen.mainScreen.bounds
                                                    styleURL:styleURL];
...
- (void)testMLNMapViewImplHasCorrectSize {
  ...
  CLLocationCoordinate2D coordinates[] = {CLLocationCoordinate2DMake(9.6315313, 52.4133574),
                                          CLLocationCoordinate2DMake(24.9410248, 60.1733244)};

  MLNPointCollectionFeature *points = [MLNPointCollectionFeature
      pointCollectionWithCoordinates:coordinates
                               count:sizeof(coordinates) / sizeof(coordinates[0])];
  MLNShapeSource *source = [[MLNShapeSource alloc] initWithIdentifier:@"heatmap"
                                                                shape:points
                                                              options:nil];
  [self.style addSource:source];

  MLNHeatmapStyleLayer *heatmapLayer = [[MLNHeatmapStyleLayer alloc] initWithIdentifier:@"lineLayer"
                                                                                 source:source];
