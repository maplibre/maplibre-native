...
#import "MLNTestLocationManager.h"
...
@interface MLNAnnotationViewIntegrationTests : MLNMapViewIntegrationTest
@end

@implementation MLNAnnotationViewIntegrationTests
...
- (void)internalTestOffscreenSelectionTitle:(NSString *)title
                               withTestData:(PanTestData)test
  ...
  CGSize annotationSize = CGSizeMake(floor(size.width * kAnnotationRelativeScale.x),
                                     floor(size.height * kAnnotationRelativeScale.y));
  ...
  self.viewForAnnotation = ^MLNAnnotationView *(MLNMapView *view, id<MLNAnnotation> annotation) {
    ...
    MLNAnnotationView *annotationView =
        [[MLNAnnotationView alloc] initWithAnnotation:annotation
                                      reuseIdentifier:MLNTestAnnotationReuseIdentifer];
  ...
  CGPoint annotationPoint = CGPointMake(floor(relativeCoordinate.x * size.width),
                                        floor(relativeCoordinate.y * size.height));
  CLLocationCoordinate2D coordinate = [self.mapView convertPoint:annotationPoint
  ...
  self.mapViewAnnotationCanShowCalloutForAnnotation =
      ^BOOL(MLNMapView *mapView, id<MLNAnnotation> annotation) {
        return showsCallout;
      };

  self.mapViewCalloutViewForAnnotation =
      ^id<MLNCalloutView>(MLNMapView *mapView, id<MLNAnnotation> annotation) {
    if (!showsCallout) return nil;
  ...
  // Animated selection takes MLNAnimationDuration (0.3 seconds), so wait a little
  ...
  BOOL (^CGRectContainsRectWithAccuracy)(CGRect, CGRect, CGFloat) =
      ^(CGRect rect1, CGRect rect2, CGFloat accuracy) {
  ...
  if (expectMapToHavePanned) {
    CLLocationDegrees latitudeDelta =
        fabs(mapCenterAfterSelection.latitude - mapCenterBeforeSelection.latitude);
    CLLocationDegrees longitudeDelta =
        fabs(mapCenterAfterSelection.longitude - mapCenterBeforeSelection.longitude);

    XCTAssert((latitudeDelta > epsilon) || (longitudeDelta > epsilon), @"Deltas: lat=%f, long=%f",
              latitudeDelta, longitudeDelta);  // One of them should have moved
    ...
    XCTAssert(
        CGRectContainsRectWithAccuracy(self.mapView.bounds, annotationFrameAfterSelection, 0.25),
        @"Mapview:%@ frame:%@", NSStringFromCGRect(self.mapView.bounds),
    ...
    if (showsCallout) {
      ...
      // This can fail if the callout view's width is < the annotations. This is really a warning,
      // so if you need this NOT to fail the tests, consider replacing with MLNTestWarning
      XCTAssert(expectCalloutToBeFullyOnscreen ==
                    CGRectContainsRectWithAccuracy(self.mapView.bounds, calloutView.frame, 0.25),
                @"Expect contains:%d, Mapview:%@ annotation:%@ callout:%@",
                expectCalloutToBeFullyOnscreen, NSStringFromCGRect(self.mapView.bounds),
                NSStringFromCGRect(annotationFrameAfterSelection),
    ...
    CGPoint annotationPoint2 = [self.mapView convertCoordinate:point.coordinate
    ...
    if (showsCallout) {
      ...
      if (annotationViewAfterSelection) {
        ...
        XCTAssert((expectCalloutToBeFullyOnscreen ==
                   CGRectContainsRectWithAccuracy(self.mapView.bounds, calloutView.frame, 0.25)),
                  @"Mapview:%@ annotation:%@ callout:%@", NSStringFromCGRect(self.mapView.bounds),
...
- (void)testBasicSelection {
  ...
  PanTestData tests[] = {
      //  Coord           showsCallout    impl margins?   moveIntoView    expectMapToPan
      //  calloutOnScreen
      //  Offscreen
...
- (void)testBasicSelectionWithCallout {
  ...
  PanTestData tests[] = {
      ...
      {{0.5f, 1.0f}, YES, YES, NO, NO, YES},  // Because annotation was off the bottom of screen,
                                              // and callout is above annotation
...
- (void)testSelectionMoveIntoView {
  ...
  //
  // | Annotation position | Has callout? | Callout implements `marginInsets...`? | Map pans when
  // selected with moveIntoView=YES? |
  // |---------------------|--------------|---------------------------------------|-----------------------------------------------|
...
- (void)testSelectionMoveIntoViewWithCallout {
  ...
  // selected with moveIntoView=YES? |
  // |---------------------|--------------|---------------------------------------|-----------------------------------------------|
  // | Offscreen           | Yes          | Yes                                   | Yes to ensure
  // callout is fully visible        | | Partially           | Yes          | Yes | Yes to ensure
  // callout is fully visible        | | Onscreen            | Yes          | Yes | Yes, but *only*
  // to ensure callout is fully visible |
  //
...
- (void)testSelectionMoveIntoViewWithBasicCallout {
  ...
  // selected with moveIntoView=YES? |
  // |---------------------|--------------|---------------------------------------|-----------------------------------------------|
  // | Offscreen           | Yes          | No                                    | Yes, but only to
  // show annotation (not callout) with no margins | | Partially           | Yes          | No | No
  // | | Onscreen            | Yes          | No                                    | No |
  //
  ...
  PanTestData tests[] = {
      ...
      {{0.5f, 2.0f}, YES, NO, YES, YES, YES},  // Because annotation was off the bottom of screen,
                                               // and callout is above annotation
      ...
      {{0.5f, 1.0f}, YES, NO, YES, NO, YES},  // Because annotation was off the bottom of screen,
                                              // and callout is above annotation
  ...
  for (CGFloat dx = -100.0; dx <= 100.0; dx += 100.0) {
    for (CGFloat dy = -100.0; dy <= 100.0; dy += 100.0) {
      ...
      UIEdgeInsets edgeInsets =
          UIEdgeInsetsMake(fmax(-dy, 0.0), fmax(-dy, 0.0), fmax(dy, 0.0), fmax(dx, 0.0));
...
- (void)internalTestSelectingAnnotationWithCenterOffsetWithOffset:(CGVector)offset
  ...
  CGPoint annotationPoint = [self.mapView convertCoordinate:point.coordinate
                                              toPointToView:self.mapView];
  XCTAssertEqualWithAccuracy(annotationPoint.x,
                             (size.width - edgeInsets.right + edgeInsets.left) / 2.0, 0.25);
  XCTAssertEqualWithAccuracy(annotationPoint.y,
                             (size.height - edgeInsets.bottom + edgeInsets.top) / 2.0, 0.25);

  // Now test taps around the annotation
  CGPoint tapPoint = CGPointMake(annotationPoint.x + offset.dx, annotationPoint.y + offset.dy);

  MLNAnnotationTag tagAtPoint = [self.mapView annotationTagAtPoint:tapPoint persistingResults:YES];
  XCTAssert(tagAtPoint != UINT32_MAX, @"Should have tapped on annotation");

  CGPoint testPoints[] = {{tapPoint.x - annotationSize.width, tapPoint.y},
...
- (void)testUserLocationWithOffsetAnchorPoint {
  ...
  MLNTestLocationManager *locationManager = [[MLNTestLocationManager alloc] init];
...
- (void)testSelectingAnnotationWhenMapIsRotated {
  CLLocationCoordinate2D coordinates[] = {{40.0, 40.0}, {NAN, NAN}};
  ...
  for (NSInteger iter = 0; iter < 10; iter++) {
    ...
    CGPoint anchor = CGPointMake(drand48() * CGRectGetWidth(self.mapView.bounds),
                                 drand48() * CGRectGetHeight(self.mapView.bounds));

    NSString *activityTitle =
        [NSString stringWithFormat:@"Rotate to: %0.1f from: %0.1f", angle, lastAngle];
    [XCTContext runActivityNamed:activityTitle
                           block:^(id<XCTActivity> _Nonnull activity) {
                             MLNMapCamera *toCamera =
                                 [self.mapView cameraByRotatingToDirection:angle
                                                         aroundAnchorPoint:anchor];
...
- (void)testSelectingAnnotationWhenMapIsScaled {
  CLLocationCoordinate2D coordinates[] = {{0.005, 0.005}, {NAN, NAN}};
  ...
  for (NSInteger iter = 0; iter < 10; iter++) {
    ...
    [XCTContext runActivityNamed:activityTitle
                           block:^(id<XCTActivity> _Nonnull activity) {
                             MLNMapCamera *toCamera =
                                 [self.mapView cameraByZoomingToZoomLevel:zoom
                                                        aroundAnchorPoint:anchor];
                             [self internalTestSelecting:annotation withCamera:toCamera];
                           }];
  }
...
- (void)internalTestShowingAnnotationsThenSelectingAnimated:(BOOL)animated {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    [self internalTestShowingAnnotationsThenSelectingAnimated:animated
...
- (void)internalTestShowingAnnotationsThenSelectingAnimated:(BOOL)animated
                                                edgePadding:(UIEdgeInsets)edgeInsets
...
- (NSArray *)internalAddAnnotationsAtCoordinates:(CLLocationCoordinate2D *)coordinates {
  ...
  [XCTContext runActivityNamed:@"Map setup"
                         block:^(id<XCTActivity> _Nonnull activity) {
                           NSString *const MLNTestAnnotationReuseIdentifer =
                               @"MLNTestAnnotationReuseIdentifer";

                           CGSize annotationSize = CGSizeMake(40.0, 40.0);

                           self.viewForAnnotation = ^MLNAnnotationView *(
                               MLNMapView *view, id<MLNAnnotation> annotation2) {
                             if (![annotation2 isKindOfClass:[MLNPointAnnotation class]]) {
                               return nil;
                             }

                             // No dequeue
                             MLNAnnotationView *annotationView = [[MLNAnnotationView alloc]
                                 initWithAnnotation:annotation2
                                    reuseIdentifier:MLNTestAnnotationReuseIdentifer];
                             annotationView.bounds =
                                 (CGRect){.origin = CGPointZero, .size = annotationSize};
                             annotationView.backgroundColor = UIColor.redColor;
                             annotationView.enabled = YES;
...
- (void)internalSelectDeselectAnnotation:(MLNPointAnnotation *)point {
  [XCTContext
      runActivityNamed:[NSString stringWithFormat:@"Select annotation: %@", point]
                 block:^(id<XCTActivity> _Nonnull activity) {
                   CGPoint annotationPoint = [self.mapView convertCoordinate:point.coordinate
                                                               toPointToView:self.mapView];

                   MLNAnnotationTag tagAtPoint = [self.mapView annotationTagAtPoint:annotationPoint
                   ...
                   if (tagAtPoint != UINT32_MAX) {
                     ...
                     // Select
                     XCTestExpectation *selectionCompleted =
                         [self expectationWithDescription:@"Selection completed"];
                     ...
                     XCTAssert(self.mapView.selectedAnnotations.count == 1,
                               @"There should only be 1 selected annotation");
                     XCTAssertEqualObjects(self.mapView.selectedAnnotations.firstObject, annotation,
                                           @"The annotation should be selected");

                     // Deselect
                     [self.mapView deselectAnnotation:annotation animated:NO];
                   } else {
                     XCTFail(@"Should be an annotation at this point: %@",
                             NSStringFromCGPoint(annotationPoint));
                   }
                 }];
}
...
- (void)waitForCollisionDetectionToRun {
  ...
  // Wait 1/2 second
  dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(NSEC_PER_SEC >> 1)),
