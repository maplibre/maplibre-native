...
- (void)testInterruptingAndResetNorthOnlyOnceInIsChanging {
  ...
  XCTAssertEqualWithAccuracy(self.mapView.direction, 0.0, 0.001,
                             @"Camera should have reset to north. %0.3f", self.mapView.direction);
}

- (void)testSetCenterCancelsTransitions {
  XCTestExpectation *cameraIsInDCExpectation =
      [self expectationWithDescription:@"camera reset to DC"];
  ...
  dispatch_after(
      dispatch_time(DISPATCH_TIME_NOW, 0.15 * NSEC_PER_SEC), dispatch_get_main_queue(), ^{
        MLNCameraTransitionTests *strongSelf = weakself;

        [strongSelf.mapView setCenterCoordinate:dc zoomLevel:zoomLevel animated:NO];
        MLNTestAssertEqualWithAccuracy(strongSelf, dc.latitude,
                                       strongSelf.mapView.centerCoordinate.latitude, 0.0005,
                                       @"setting center coordinate should cancel transitions");
        MLNTestAssertEqualWithAccuracy(strongSelf, dc.longitude,
                                       strongSelf.mapView.centerCoordinate.longitude, 0.0005,
                                       @"setting center coordinate should cancel transitions");
  ...
  self.regionDidChange = ^(MLNMapView *mapView, MLNCameraChangeReason reason, BOOL animated) {
    ...
    switch (delegateCallCount) {
      case 0: {
        ...
        MLNTestAssertEqualWithAccuracy(strongSelf, target.longitude, center.longitude, 0.0005,
                                       @"center coordinate longitude should be at target");

        MLNTestAssertEqualWithAccuracy(strongSelf, target.latitude, center.latitude, 0.0005,
      ...
      case 1: {
        // Our center coordinate should match our target (assuming we're not
        // constrained by zoom level)
        MLNTestAssertEqualWithAccuracy(strongSelf, target2.longitude, center.longitude, 0.0005,
                                       @"center coordinate longitude should be at target2");

        MLNTestAssertEqualWithAccuracy(strongSelf, target2.latitude, center.latitude, 0.0005,
...
- (void)testFlyToCameraInDelegateMethod {
  ...
  double altitude =
      MLNAltitudeForZoomLevel(zoomLevel, 0.0, target.latitude, self.mapView.frame.size);
    ...
    switch (delegateCallCount) {
      case 0: {
        ...
        [mapView flyToCamera:camera
            completionHandler:^{
              MLNTestAssert(strongSelf, !runloop,
                            @"Completion block should be called after delegate method");
              [expectation fulfill];
  ...
  XCTAssert(delegateCallCount == 2, @"Expecting 2 regionDidChange callbacks, got %ld",
            (long)delegateCallCount);  // Once for the setDirection and once for the reset north
}
