...
class UserMap: MLNMapView, MLNMapViewDelegate {
    ...
    @MainActor func animate(camera: MLNMapCamera, withDuration duration: TimeInterval) async {
        ...
        await animate(camera: camera, withDuration: duration, gate: gate)
        cameraGate = nil
    }
    ...
    func stop() {
        ...
        // release a pending style load or camera animation so the task can observe
        // the cancellation and stop holding on to this map view
        styleGate?.open()
        cameraGate?.open()
    }
