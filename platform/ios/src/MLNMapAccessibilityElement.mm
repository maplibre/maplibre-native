...
- (instancetype)initWithAccessibilityContainer:(id)container feature:(id<MLNFeature>)feature {
  if (self = [super initWithAccessibilityContainer:container]) {
    ...
    NSString *dominantScript =
        [NSOrthography mgl_dominantScriptForMapboxStreetsLanguage:languageCode];
...
- (instancetype)initWithAccessibilityContainer:(id)container feature:(id<MLNFeature>)feature {
  if (self = [super initWithAccessibilityContainer:container feature:feature]) {
    ...
    if (attributes[@"elevation_m"] ?: attributes[@"elevation_ft"]) {
      ...
      BOOL usesMetricSystem = ![[formatter.numberFormatter.locale
          objectForKey:NSLocaleMeasurementSystem] isEqualToString:@"U.S."];
      if (usesMetricSystem) {
    ...
    if (facts.count) {
      NSString *separator =
          NSLocalizedStringWithDefaultValue(@"LIST_SEPARATOR", nil, nil, @", ", @"List separator");
...
- (instancetype)initWithAccessibilityContainer:(id)container feature:(id<MLNFeature>)feature {
  if (self = [super initWithAccessibilityContainer:container feature:feature]) {
    ...
    if (attributes[@"ref"]) {
      ...
      NSString *ref = [NSString
          stringWithFormat:
              NSLocalizedStringWithDefaultValue(
                  @"ROAD_REF_A11Y_FMT", nil, nil, @"Route %@",
    ...
    if ([attributes[@"oneway"] isEqualToString:@"true"]) {
      [facts addObject:NSLocalizedStringWithDefaultValue(
                           @"ROAD_ONEWAY_A11Y_VALUE", nil, nil, @"One way",
    ...
    if ([feature isKindOfClass:[MLNMultiPolylineFeature class]]) {
      [facts addObject:NSLocalizedStringWithDefaultValue(
                           @"ROAD_DIVIDED_A11Y_VALUE", nil, nil, @"Divided road",
                           @"Accessibility value indicating that a road is a divided road (dual "
                           @"carriageway)")];
    ...
    if (polyline) {
      ...
      if (pointCount) {
        ...
        CLLocationDirection startDirection =
            MLNDirectionBetweenCoordinates(coordinates[pointCount - 1], coordinates[0]);
        CLLocationDirection endDirection =
            MLNDirectionBetweenCoordinates(coordinates[0], coordinates[pointCount - 1]);
        ...
        NSString *directionString = [NSString
            stringWithFormat:NSLocalizedStringWithDefaultValue(
                                 @"ROAD_DIRECTION_A11Y_FMT", nil, nil, @"%@ to %@",
                                 @"String format for accessibility value for road feature; "
                                 @"{starting compass direction}, {ending compass direction}"),
                             startDirectionString, endDirectionString];
...
- (instancetype)initWithAccessibilityContainer:(id)container {
  if (self = [super initWithAccessibilityContainer:container]) {
    ...
    self.accessibilityHint = NSLocalizedStringWithDefaultValue(
        @"CLOSE_CALLOUT_A11Y_HINT", nil, nil, @"Returns to the map",
        @"Accessibility hint for closing the selected annotation’s callout view and returning to "
        @"the map");
  }
  return self;
}
