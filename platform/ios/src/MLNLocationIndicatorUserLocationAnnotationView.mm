...
- (void)update {
  if (self.mapView.style && !self->location_layer) {
    ...
    self->location_layer = (MLNLocationIndicatorStyleLayer *)[self.mapView.style
        layerWithIdentifier:@"location_indicator_layer"];

    if (!self->location_layer) {
      self->_renderMode = MLNRenderModeNone;

      self->location_layer =
          [[MLNLocationIndicatorStyleLayer alloc] initWithIdentifier:@"location_indicator_layer"];

      [self.mapView.style setImage:[UIImage mgl_resourceImageNamed:@"user_bearing_icon"]
                           forName:@"user_bearing_icon"];
      [self.mapView.style setImage:[UIImage mgl_resourceImageNamed:@"user_icon_stale"]
                           forName:@"user_icon_stale"];
      [self.mapView.style setImage:[UIImage mgl_resourceImageNamed:@"user_icon"]
                           forName:@"user_icon"];
      [self.mapView.style setImage:[UIImage mgl_resourceImageNamed:@"user_puck_icon"]
                           forName:@"user_puck_icon"];
      [self.mapView.style setImage:[UIImage mgl_resourceImageNamed:@"user_stroke_icon"]
                           forName:@"user_stroke_icon"];
      [self.mapView.style setImage:[UIImage mgl_resourceImageNamed:@"user_icon_shadow"]
                           forName:@"user_icon_shadow"];

      NSDictionary *opacityStops =
          @{@(self.mapView.minimumZoomLevel) : @0.6f, @(self.mapView.maximumZoomLevel) : @1.0f};
      NSExpression *stops = [NSExpression expressionForConstantValue:opacityStops];
      NSExpression *styleScaling = [NSExpression
      ...
      NSDictionary *backgroundOpacityStops =
          @{@(self.mapView.minimumZoomLevel) : @1.2f, @(self.mapView.maximumZoomLevel) : @1.5f};
      NSExpression *backgroundStops =
          [NSExpression expressionForConstantValue:backgroundOpacityStops];
      NSExpression *backgroundStyleScaling = [NSExpression
          mgl_expressionForInterpolatingExpression:NSExpression.zoomLevelVariableExpression
                                     withCurveType:MLNExpressionInterpolationModeLinear
                                        parameters:nil
      ...
      self->location_layer.bearingTransition =
          MLNTransitionMake(1.0, 0, kCAMediaTimingFunctionLinear);
  ...
  if (self->location_layer) {
    if (self.mapView.userTrackingMode == MLNUserTrackingModeFollowWithCourse) {
      if (self.userLocation.location.course >= 0) {
        self->location_layer.bearing =
            [NSExpression expressionForConstantValue:@(self.userLocation.location.course)];
      }

      if (self->_renderMode != MLNRenderModeGps) {
        self->location_layer.bearingImage =
            [NSExpression expressionForConstantValue:@"user_puck_icon"];
        self->location_layer.topImage = [NSExpression expressionForConstantValue:@"user_puck_icon"];
        self->location_layer.shadowImage =
            [NSExpression expressionForConstantValue:@"user_stroke_icon"];

        self->_renderMode = MLNRenderModeGps;
      }
    } else {
      if (self->_renderMode != MLNRenderModeNormal &&
          self.mapView.userTrackingMode == MLNUserTrackingModeFollow) {
        self->location_layer.bearingImage =
            self.mapView.showsUserHeadingIndicator
                ? [NSExpression expressionForConstantValue:@"user_bearing_icon"]
                : [NSExpression expressionForConstantValue:@"user_icon"];
        self->location_layer.topImage = [NSExpression expressionForConstantValue:@"user_icon"];
        self->location_layer.shadowImage =
            [NSExpression expressionForConstantValue:@"user_stroke_icon"];

        self->_renderMode = MLNRenderModeNormal;
      } else if (self->_renderMode != MLNRenderModeCompass &&
                 self.mapView.userTrackingMode == MLNUserTrackingModeFollowWithHeading) {
        self->location_layer.bearingImage =
            [NSExpression expressionForConstantValue:@"user_bearing_icon"];
        self->location_layer.topImage = [NSExpression expressionForConstantValue:@"user_icon"];
        self->location_layer.shadowImage =
      ...
      bool showHeading = self.mapView.showsUserHeadingIndicator ||
                         self.mapView.userTrackingMode == MLNUserTrackingModeFollowWithHeading;
      if (showHeading) {
        CLLocationDirection headingDirection = (self.userLocation.heading.trueHeading >= 0
                                                    ? self.userLocation.heading.trueHeading
                                                    : self.userLocation.heading.magneticHeading);
        if (headingDirection >= 0) {
          self->location_layer.bearing =
              [NSExpression expressionForConstantValue:@(headingDirection)];
        }
      }
    }
    ...
    NSArray *location =
        @[ @(newLocation.coordinate.latitude), @(newLocation.coordinate.longitude), @0 ];
