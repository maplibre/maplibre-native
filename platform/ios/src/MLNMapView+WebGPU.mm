...
MLNMapViewWebGPUImpl::MLNMapViewWebGPUImpl(MLNMapView* nativeView_)
  ...
  if (features.featureCount > 0 && features.features) {
    for (size_t i = 0; i < features.featureCount; ++i) {
      if (static_cast<wgpu::FeatureName>(features.features[i]) ==
...
void MLNMapViewWebGPUImpl::createView() {
  ...
  impl->webGPUView.autoresizingMask =
  ...
  CGSize drawableSize =
      CGSizeMake(mapView.bounds.size.width * scaleFactor, mapView.bounds.size.height * scaleFactor);
...
void MLNMapViewWebGPUImpl::createSurface() {
  ...
  impl->surface =
      wgpuInstanceCreateSurface(static_cast<WGPUInstance>(impl->instance), &surfaceDesc);
#endif
...
void MLNMapViewWebGPUImpl::markNeedsPresent() { impl->needsPresent = true; }
...
void MLNMapViewWebGPUImpl::display() { render(); }
...
void MLNMapViewWebGPUImpl::layoutChanged() {
  ...
  CGSize drawableSize = CGSizeMake(screenSize.width * scaleFactor, screenSize.height * scaleFactor);
...
UIImage* MLNMapViewWebGPUImpl::snapshot() {
  ...
  const uint32_t alignedRowStride =
...
#if MLN_WEBGPU_IMPL_DAWN
  WGPUStringView bufferLabel = {"Snapshot Buffer", strlen("Snapshot Buffer")};
  ...
  if (mapContext.status == WGPUMapAsyncStatus_Success) {
    ...
    if (mappedData) {
      auto rgbaData = std::make_unique<uint8_t[]>(static_cast<size_t>(rowStride) * fbSize.height);
      ...
      for (uint32_t row = 0; row < fbSize.height; ++row) {
        ...
        auto* dst = rgbaData.get() + static_cast<size_t>(row) * rowStride;
        if (swizzle) {
      ...
      CGContextRef ctx =
          CGBitmapContextCreate(rgbaData.get(), fbSize.width, fbSize.height, 8, rowStride,
                                colorSpace, kCGImageAlphaPremultipliedLast);
      if (ctx) {
        CGImageRef cgImage = CGBitmapContextCreateImage(ctx);
        if (cgImage) {
          const CGFloat scale = impl->webGPUView.contentScaleFactor;
          result = [UIImage imageWithCGImage:cgImage scale:scale orientation:UIImageOrientationUp];
...
MLNBackendResource* MLNMapViewWebGPUImpl::getObject() { return [[MLNBackendResource alloc] init]; }
