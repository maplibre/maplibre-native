...
- (void)update {
  if (CGSizeEqualToSize(self.frame.size, CGSizeZero)) {
    CGFloat frameSize = (self.mapView.userTrackingMode == MLNUserTrackingModeFollowWithCourse)
...
- (void)drawPreciseLocationPuck {
  ...
  (self.mapView.userTrackingMode == MLNUserTrackingModeFollowWithCourse) ? [self drawPuck]
                                                                         : [self drawDot];
  [self updatePitch];
  _haloLayer.hidden = !CLLocationCoordinate2DIsValid(self.mapView.userLocation.coordinate) ||
                      self.mapView.userLocation.location.horizontalAccuracy > 10;
}
...
- (void)updatePitch {
  if (self.mapView.camera.pitch != _oldPitch) {
    ...
    CATransform3D t = CATransform3DRotate(
        CATransform3DIdentity, MLNRadiansFromDegrees(self.mapView.camera.pitch), 1.0, 0, 0);
...
- (void)drawPuck {
  ...
  if (!_puckArrow) {
    ...
    _puckArrow.bounds = CGRectMake(0, 0, round(MLNUserLocationAnnotationArrowSize),
                                   round(MLNUserLocationAnnotationArrowSize));
  ...
  if (self.userLocation.location.course >= 0) {
    _puckArrow.affineTransform = CGAffineTransformRotate(
        CGAffineTransformIdentity,
        -MLNRadiansFromDegrees(self.mapView.direction - self.userLocation.location.course));
  }
...
- (void)drawDot {
  ...
  //
  BOOL headingTrackingModeEnabled =
  ...
  if (showHeadingIndicator) {
    ...
    if (([_headingIndicatorLayer isMemberOfClass:[MLNUserLocationHeadingBeamLayer class]] &&
         !headingTrackingModeEnabled) ||
        ([_headingIndicatorLayer isMemberOfClass:[MLNUserLocationHeadingArrowLayer class]] &&
         headingTrackingModeEnabled)) {
    ...
    if (!_headingIndicatorLayer && headingAccuracy) {
      if (headingTrackingModeEnabled) {
        _headingIndicatorLayer =
            [[MLNUserLocationHeadingBeamLayer alloc] initWithUserLocationAnnotationView:self];
        ...
        _headingIndicatorLayer =
            [[MLNUserLocationHeadingArrowLayer alloc] initWithUserLocationAnnotationView:self];
    ...
    CLLocationDirection headingDirection =
        (self.userLocation.heading.trueHeading >= 0 ? self.userLocation.heading.trueHeading
    ...
    if (headingDirection >= 0) {
      ...
      if (fabs(rotation) > MLNUserLocationHeadingUpdateThreshold) {
        ...
        _headingIndicatorLayer.affineTransform =
            CGAffineTransformRotate(CGAffineTransformIdentity, rotation);
  ...
  if (_accuracyRingLayer &&
    ...
    if (accuracyRingSize > MLNUserLocationAnnotationDotSize + 15) {
      ...
      _haloLayer.bounds =
          CGRectMake(0, 0, MLNUserLocationAnnotationHaloSize, MLNUserLocationAnnotationHaloSize);
  ...
  if (!_haloLayer) {
    ...
    CAKeyframeAnimation *boundsAnimation =
        [CAKeyframeAnimation animationWithKeyPath:@"transform.scale.xy"];
  ...
  if (!_dotBorderLayer) {
    ...
    _dotBorderLayer.shadowPath =
        [[UIBezierPath bezierPathWithOvalInRect:_dotBorderLayer.bounds] CGPath];
...
- (void)drawApproximate {
  ...
  if (!_approximateLayer && self.userLocation.location.horizontalAccuracy) {
    CGFloat accuracyRingSize =
        [self calculateAccuracyRingSize:MAX(self.mapView.zoomLevel,
                                            MLNUserLocationApproximateZoomThreshold)];
...
- (CGFloat)calculateAccuracyRingSize:(double)zoomLevel {
  ...
  return round(self.userLocation.location.horizontalAccuracy /
               [self.mapView metersPerPointAtLatitude:self.userLocation.coordinate.latitude
                                            zoomLevel:zoomLevel] *
               2.0);
}
