...
static const MLNRow MLNMetricTable[] = {
    {.distance = 1, .numberOfBars = 2},      {.distance = 2, .numberOfBars = 2},
    {.distance = 4, .numberOfBars = 2},      {.distance = 10, .numberOfBars = 2},
    {.distance = 20, .numberOfBars = 2},     {.distance = 50, .numberOfBars = 2},
    {.distance = 75, .numberOfBars = 3},     {.distance = 100, .numberOfBars = 2},
    {.distance = 150, .numberOfBars = 2},    {.distance = 200, .numberOfBars = 2},
    {.distance = 300, .numberOfBars = 3},    {.distance = 500, .numberOfBars = 2},
    {.distance = 1000, .numberOfBars = 2},   {.distance = 1500, .numberOfBars = 2},
    {.distance = 3000, .numberOfBars = 3},   {.distance = 5000, .numberOfBars = 2},
    {.distance = 10000, .numberOfBars = 2},  {.distance = 20000, .numberOfBars = 2},
    {.distance = 30000, .numberOfBars = 3},  {.distance = 50000, .numberOfBars = 2},
    {.distance = 100000, .numberOfBars = 2}, {.distance = 200000, .numberOfBars = 2},
    {.distance = 300000, .numberOfBars = 3}, {.distance = 400000, .numberOfBars = 2},
    {.distance = 500000, .numberOfBars = 2}, {.distance = 600000, .numberOfBars = 3},
...
- (void)commonInit {
  ...
  _secondaryColor = [UIColor colorWithRed:247.0 / 255.0
                                    green:247.0 / 255.0
                                     blue:247.0 / 255.0
                                    alpha:1];
  _borderWidth = 1.0f;
...
- (BOOL)usesRightToLeftLayout {
  ...
  return
      [UIView
          userInterfaceLayoutDirectionForSemanticContentAttribute:self.superview
...
- (void)setUsesMetricSystem:(BOOL)usesMetricSystem {
  if (_usesMetricSystem != usesMetricSystem) {
    ...
    // Find the current locale for the system, then check if we need to override this. E.g. Locale
    // is in the United States but we want to see metric.
    NSLocale *locale = [NSLocale currentLocale];
    ...
    if (currentLocaleUsesMetric && !usesMetricSystem) {
      // Our current locale is metric, but for some reason we don't want to show metric here, so we
      // need to force a locale that isn't metric.
      ...
      self.formatter.numberFormatter.locale = [NSLocale localeWithLocaleIdentifier:@"en_CA"];
               ...
               (!currentLocaleUsesMetric && !usesMetricSystem)) {
...
- (void)updateVisibility {
  ...
  NSUInteger count = metric ? sizeof(MLNMetricTable) / sizeof(MLNMetricTable[0])
                            : sizeof(MLNImperialTable) / sizeof(MLNImperialTable[0]);

  CLLocationDistance maximumDistance = [self maximumWidth] * [self unitsPerPoint];
  CLLocationDistance allowedDistance =
      metric ? MLNMetricTable[count - 1].distance : MLNImperialTable[count - 1].distance;
...
- (void)layoutSubviews {
  ...
  self.containerView.frame =
      CGRectMake(barOffset, intrinsicContentHeight - MLNBarHeight, totalBarWidth, MLNBarHeight);
...
- (void)layoutLabelsWithOffset:(CGFloat)barOffset
                         delta:(CGFloat)barDelta
