...
const changelogPath = 'platform/macos/CHANGELOG.md';
...
*/
let currentVersion = execSync('git describe --tags --match=macos-v*.*.* --abbrev=0')
    .toString()
    .trim()
    .replace('macos-v', '');
...
if (outputMode.isJazzy) {
    ...
    templatedReleaseNotes = ejs.render(fs.readFileSync('platform/macos/scripts/release-notes-jazzy.md.ejs', 'utf8'), {
        'CURRENTVERSION': currentVersion,
