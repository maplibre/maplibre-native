 ...
 */
class CameraPositionActivity : FragmentActivity(), OnMapReadyCallback, View.OnClickListener,
    ...
    private val moveCanceledListener =
        OnCameraMoveCanceledListener { Timber.e("OnCameraMoveCanceled") }
    private val moveStartedListener: OnCameraMoveStartedListener =
        object : OnCameraMoveStartedListener {
            private val REASONS =
                arrayOf("REASON_API_GESTURE", "REASON_DEVELOPER_ANIMATION", "REASON_API_ANIMATION")
    ...
    private class DialogClickListener(
        ...
        override fun onClick(dialog: DialogInterface, which: Int) {
            val latitude =
                (dialogContent.findViewById<View>(R.id.value_lat) as TextView).text.toString()
                    .toDouble()
            val longitude =
                (dialogContent.findViewById<View>(R.id.value_lon) as TextView).text.toString()
                    .toDouble()
            val zoom =
                (dialogContent.findViewById<View>(R.id.value_zoom) as TextView).text.toString()
                    .toDouble()
            val bearing =
                (dialogContent.findViewById<View>(R.id.value_bearing) as TextView).text.toString()
                    .toDouble()
            val tilt =
                (dialogContent.findViewById<View>(R.id.value_tilt) as TextView).text.toString()
                    .toDouble()
            if (latitude < GeometryConstants.MIN_LATITUDE || latitude > GeometryConstants.MAX_LATITUDE) {
                Toast.makeText(
                    dialogContent.context, "latitude value must be set "
                            + " between " + GeometryConstants.MIN_LATITUDE + " and " + GeometryConstants.MAX_LATITUDE,
                    Toast.LENGTH_SHORT
                ).show()
                return
            }
