...
class CameraAnimationTypeActivity : AppCompatActivity(), OnMapReadyCallback {
  ...
  override fun onMapReady(map: MapboxMap) {
    ...
    moveButton?.setOnClickListener { view: View? ->
      val cameraPosition =
          CameraPosition.Builder().target(nextLatLng).zoom(14.0).tilt(30.0).tilt(0.0).build()
    ...
    easeButton?.setOnClickListener { view: View? ->
      val cameraPosition =
          CameraPosition.Builder().target(nextLatLng).zoom(15.0).bearing(180.0).tilt(30.0).build()
