...
mln::LineAnnotation Polyline::toAnnotation(jni::JNIEnv& env, const jni::Object<Polyline>& polyline) {
    ...
    mln::LineAnnotation annotation{MultiPoint::toGeometry<mln::LineString<double>>(env, polyline.Get(env, points))};

    annotation.opacity = polyline.Get(env, alpha);
    annotation.color = *conversion::convert<mln::Color>(env, polyline.Get(env, color));
