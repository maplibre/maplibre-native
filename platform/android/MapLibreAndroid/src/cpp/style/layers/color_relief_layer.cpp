...
namespace android {
    ...
    jni::Local<jni::Object<>> ColorReliefLayer::getResampling(jni::JNIEnv& env) {
        ...
        auto layer = layerPtr.get();
        if (!layer) {
            return std::move(*convert<jni::Local<jni::Object<>>>(env, style::ColorReliefLayer::getDefaultResampling()));
        }
        return std::move(*convert<jni::Local<jni::Object<>>>(env, toColorReliefLayer(*layer).getResampling()));
    }
