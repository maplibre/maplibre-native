...
jni::Local<jni::Object<>> RasterLayer::getResampling(jni::JNIEnv& env) {
    ...
    auto layer = layerPtr.get();
    if (!layer) {
        return std::move(*convert<jni::Local<jni::Object<>>>(env, style::RasterLayer::getDefaultResampling()));
    }
    return std::move(*convert<jni::Local<jni::Object<>>>(env, toRasterLayer(*layer).getResampling()));
}
