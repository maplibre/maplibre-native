...
public final class Transform implements MapView.OnCameraDidChangeListener {
  ...
  public final void moveCamera(@NonNull MapboxMap mapboxMap, CameraUpdate update,
    ...
    if (isValidCameraPosition(cameraPosition)) {
      ...
      nativeMap.jumpTo(cameraPosition.getTarget(), cameraPosition.getZoom(), cameraPosition.getTilt(), cameraPosition.getBearing(),
        cameraPosition.getPadding());
      invalidateCameraPosition();
      cameraChangeDispatcher.onCameraIdle();
      handler.post(new Runnable() {
        @Override
  ...
  final void easeCamera(@NonNull MapboxMap mapboxMap, CameraUpdate update, int durationMs, boolean easingInterpolator,
    ...
    if (isValidCameraPosition(cameraPosition)) {
      ...
      nativeMap.easeTo(cameraPosition.getTarget(), cameraPosition.getZoom(), cameraPosition.getBearing(), cameraPosition.getTilt(),
        cameraPosition.getPadding(), durationMs, easingInterpolator);
    } else if (callback != null) {
      callback.onFinish();
    }
  }
  ...
  public final void animateCamera(@NonNull MapboxMap mapboxMap, CameraUpdate update, int durationMs,
    ...
    if (isValidCameraPosition(cameraPosition)) {
      ...
      mapView.addOnCameraDidChangeListener(this);
      nativeMap.flyTo(cameraPosition.getTarget(), cameraPosition.getZoom(), cameraPosition.getBearing(),
        cameraPosition.getTilt(), cameraPosition.getPadding(), durationMs);
    } else if (callback != null) {
      callback.onFinish();
    }
  }
