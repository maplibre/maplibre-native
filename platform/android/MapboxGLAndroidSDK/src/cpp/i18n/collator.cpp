...
public:
    Impl(bool caseSensitive_, bool diacriticSensitive_, std::optional<std::string> locale_)
        : caseSensitive(caseSensitive_), diacriticSensitive(diacriticSensitive_), env(android::AttachEnv()) {
        ...
        if (!languageTag.language) {
            ...
            locale =
                jni::NewGlobal(*env, android::Locale::New(*env, jni::Make<jni::String>(*env, *languageTag.language)));
        } else {
