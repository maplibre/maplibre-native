...
void OfflineRegion::setOfflineRegionObserver(
    jni::JNIEnv& env_, const jni::Object<OfflineRegion::OfflineRegionObserver>& callback
) {
    // Define the observer
    ...
    public:
        ...
        void responseError(mbgl::Response::Error error) override {
            ...
            static auto& javaClass = jni::Class<OfflineRegion::OfflineRegionObserver>::Singleton(*env);
            static auto method = javaClass.GetMethod<void(jni::Object<mbgl::android::OfflineRegionError>)>(
                *env, "onError"
            );
...
void OfflineRegion::getOfflineRegionStatus(
    jni::JNIEnv& env_, const jni::Object<OfflineRegionStatusCallback>& callback_
) {
...
void OfflineRegion::deleteOfflineRegion(jni::JNIEnv& env_, const jni::Object<OfflineRegionDeleteCallback>& callback_) {
    ...
    fileSource->deleteOfflineRegion(
        *region,
        [
            // Ensure the object is not gc'd in the meanwhile
            callback = std::make_shared<decltype(globalCallback)>(std::move(globalCallback))](std::exception_ptr error
        ) mutable {
            // Reattach, the callback comes from a different thread
            android::UniqueEnv env = android::AttachEnv();
...
void OfflineRegion::invalidateOfflineRegion(
    jni::JNIEnv& env_, const jni::Object<OfflineRegionInvalidateCallback>& callback_
) {
    auto globalCallback = jni::NewGlobal<jni::EnvAttachingDeleter>(env_, callback_);

    fileSource->invalidateOfflineRegion(
        *region,
        [
            // Ensure the object is not gc'd in the meanwhile
            callback = std::make_shared<decltype(globalCallback)>(std::move(globalCallback))](std::exception_ptr error
...
void OfflineRegion::updateOfflineRegionMetadata(
    jni::JNIEnv& env_,
...
jni::Local<jni::Object<OfflineRegion>> OfflineRegion::New(
    jni::JNIEnv& env, const jni::Object<FileSource>& jFileSource, mbgl::OfflineRegion region
) {
    // Definition
    auto definition = region.getDefinition().match(
        ...
        [&](const mbgl::OfflineGeometryRegionDefinition def) { return OfflineGeometryRegionDefinition::New(env, def); }
    );
...
void OfflineRegion::OfflineRegionStatusCallback::onError(
    jni::JNIEnv& env, const jni::Object<OfflineRegion::OfflineRegionStatusCallback>& callback, std::exception_ptr error
) {
...
void OfflineRegion::OfflineRegionDeleteCallback::onError(
    jni::JNIEnv& env, const jni::Object<OfflineRegion::OfflineRegionDeleteCallback>& callback, std::exception_ptr error
) {
