...
static style::CustomGeometrySource::Options convertCustomGeometrySourceOptions(jni::JNIEnv& env,
    ...
    Error error;
    std::optional<style::CustomGeometrySource::Options> result =
        convert<style::CustomGeometrySource::Options>(Value(env, options), error);
    if (!result) {
...
bool CustomGeometrySource::isCancelled(jni::jint z, jni::jint x, jni::jint y) {
    android::UniqueEnv _env = android::AttachEnv();
