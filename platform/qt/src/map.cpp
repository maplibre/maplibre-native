...
*/
Map::Map(QObject *parent_, const Settings &settings, const QSize &size, qreal pixelRatio) : QObject(parent_) {
    assert(!size.isEmpty());
...
void Map::addSource(const QString &id, const QVariantMap &params) {
    ...
    std::optional<std::unique_ptr<Source>> source =
        convert<std::unique_ptr<Source>>(QVariant(params), error, id.toStdString());
    if (!source) {
