...
struct OnlineFileRequest {
  ...
  OnlineFileRequest(Resource resource_, Callback callback_,
  ...
  // We're using this to add a delay when making a new request so we don't keep
  // retrying immediately in case of a server serving expired tiles.
  uint32_t expiredRequests = 0;

  // Counts the number of subsequent failed requests. We're using this value for
  // exponential backoff when retrying requests.
  uint32_t failedRequests = 0;
  Response::Error::Reason failedRequestReason =
      Response::Error::Reason::Success;
  std::optional<Timestamp> retryAfter;
};

class OnlineFileSourceThread {
public:
  OnlineFileSourceThread(const ResourceOptions &resourceOptions_,
  ...
  void request(AsyncRequest *req, Resource resource,
               const ActorRef<FileSourceRequest> &ref) {
    auto callback = [ref](const Response &res) {
      ref.invoke(&FileSourceRequest::setResponse, res);
    };
    tasks[req] = std::make_unique<OnlineFileRequest>(
        std::move(resource), std::move(callback), *this);
  }
  ...
  void add(OnlineFileRequest *req) {
    ...
    if (resourceTransform) {
      // Request the ResourceTransform actor a new url and replace the resource
      // url with the transformed one before proceeding to schedule the request.
      resourceTransform.transform(
          req->resource.kind, req->resource.url,
          [ref = req->actor()](const std::string &url) {
...
private:
  ...
  struct PendingRequests {
    ...
    void insert(OnlineFileRequest *request) {
      if (request->resource.priority == Resource::Priority::Regular) {
        firstLowPriorityRequest =
            queue.insert(firstLowPriorityRequest, request);
        firstLowPriorityRequest++;
      } else {
   ...
   * 4. Back to #1
   *
   * Requests in any state are in `allRequests`. Requests in the pending state
   * are in `pendingRequests`. Requests in the active state are in
   * `activeRequests`.
   */
  ...
  util::AsyncTask reachability{
      std::bind(&OnlineFileSourceThread::networkIsReachableAgain, this)};
  std::map<AsyncRequest *, std::unique_ptr<OnlineFileRequest>> tasks;
};

class OnlineFileSource::Impl {
public:
  Impl(const ResourceOptions &resourceOptions,
      ...
      : cachedResourceOptions(resourceOptions.clone()),
        ...
        thread(std::make_unique<util::Thread<OnlineFileSourceThread>>(
            util::makeThreadPrioritySetter(
                platform::EXPERIMENTAL_THREAD_PRIORITY_NETWORK),
            "OnlineFileSource", resourceOptions.clone(),
            clientOptions.clone())) {}
  ...
  std::unique_ptr<AsyncRequest> request(Callback callback, Resource res) {
    ...
    thread->actor().invoke(&OnlineFileSourceThread::request, req.get(),
                           std::move(res), req->actor());
    return req;
  }
  ...
  void setResourceTransform(ResourceTransform transform) {
    thread->actor().invoke(&OnlineFileSourceThread::setResourceTransform,
                           std::move(transform));
  }

  void setResourceOptions(ResourceOptions options) {
    thread->actor().invoke(&OnlineFileSourceThread::setResourceOptions,
                           options.clone());
    {
  ...
  void setClientOptions(ClientOptions options) {
    thread->actor().invoke(&OnlineFileSourceThread::setClientOptions,
                           options.clone());
    {
  ...
  void setMaximumConcurrentRequests(const mapbox::base::Value &value) {
    if (auto *maximumConcurrentRequests = value.getUint()) {
      ...
      const auto maxConcurrentRequests =
          static_cast<uint32_t>(*maximumConcurrentRequests);
      thread->actor().invoke(
          &OnlineFileSourceThread::setMaximumConcurrentRequests,
      ...
      Log::Error(Event::General,
                 "Invalid max-concurrent-requests property value type.");
    }
  }
    ...
    if (auto *baseURL = value.getString()) {
      thread->actor().invoke(&OnlineFileSourceThread::setAPIBaseURL, *baseURL);
      {
        ...
        cachedResourceOptions.withTileServerOptions(
            cachedResourceOptions.tileServerOptions().clone().withBaseURL(
                *baseURL));
      }
    } else {
...
private:
  ...
  uint32_t cachedMaximumConcurrentRequests =
      util::DEFAULT_MAXIMUM_CONCURRENT_REQUESTS;
  const std::unique_ptr<util::Thread<OnlineFileSourceThread>> thread;
};

OnlineFileRequest::OnlineFileRequest(Resource resource_, Callback callback_,
                                     OnlineFileSourceThread &impl_)
    : impl(impl_), resource(std::move(resource_)),
...
Timestamp interpolateExpiration(const Timestamp &current,
                                std::optional<Timestamp> prior, bool &expired) {
  auto now = util::now();
  if (current > now) {
    return current;
  }
...
Duration
OnlineFileRequest::getUpdateInterval(std::optional<Timestamp> expires) const {
  // Calculate a timeout that depends on how many
  // consecutive errors we've encountered, and on the expiration time, if
  // present.
  Duration errorRetryTimeout =
      http::errorRetryTimeout(failedRequestReason, failedRequests, retryAfter);
  Duration expirationTimeout =
      std::max(http::expirationTimeout(std::move(expires), expiredRequests),
...
inline std::string sanitizeURL(std::string &url) {
  ...
  return (queryIndex != std::string::npos) ? std::string(url, 0, queryIndex)
                                           : url;
}
...
void OnlineFileRequest::completed(Response response) {
  // If we didn't get various caching headers in the response, continue using
  // the previous values. Otherwise, update the previous values to the new
  // values.
  ...
  if (response.notModified && resource.priorData) {
    // When the priorData field is set, it indicates that we had to revalidate
    // the request and that the requestor hasn't gotten data yet. If we get a
    // 304 response, this means that we have send the cached data to give the
    // requestor a chance to actually obtain the data.
  ...
  if (response.expires) {
    ...
    response.expires =
        interpolateExpiration(*response.expires, prior, isExpired);
  }
  ...
  // done last, and needs to make a local copy of the callback to ensure that it
...
void OnlineFileRequest::networkIsReachableAgain() {
  // We need all requests to fail at least once before we are going to start
  // retrying them, and we only immediately restart request that failed due to
  // connection issues.
...
OnlineFileSource::OnlineFileSource(const ResourceOptions &resourceOptions,
                                   const ClientOptions &clientOptions)
...
OnlineFileSource::request(const Resource &resource, Callback callback) {
  Resource res = resource;
  const TileServerOptions options =
      impl->getResourceOptions().tileServerOptions();
  ...
  case Resource::Kind::Style:
    res.url = mbgl::util::mapbox::normalizeStyleURL(options, resource.url,
                                                    impl->getApiKey());
    break;

  case Resource::Kind::Source:
    res.url = util::mapbox::normalizeSourceURL(options, resource.url,
                                               impl->getApiKey());
    break;

  case Resource::Kind::Glyphs:
    res.url = util::mapbox::normalizeGlyphsURL(options, resource.url,
                                               impl->getApiKey());
    break;

  case Resource::Kind::SpriteImage:
  case Resource::Kind::SpriteJSON:
    res.url = util::mapbox::normalizeSpriteURL(options, resource.url,
                                               impl->getApiKey());
    break;

  case Resource::Kind::Tile:
    res.url = util::mapbox::normalizeTileURL(options, resource.url,
                                             impl->getApiKey());
    break;
  }
...
bool OnlineFileSource::canRequest(const Resource &resource) const {
  return resource.hasLoadingMethod(Resource::LoadingMethod::Network) &&
         resource.url.rfind(mbgl::util::ASSET_PROTOCOL, 0) ==
             std::string::npos &&
         resource.url.rfind(mbgl::util::FILE_PROTOCOL, 0) == std::string::npos;
}

void OnlineFileSource::pause() { impl->pause(); }

void OnlineFileSource::resume() { impl->resume(); }

void OnlineFileSource::setProperty(const std::string &key,
                                   const mapbox::base::Value &value) {
  if (key == API_KEY_KEY) {
...
mapbox::base::Value
OnlineFileSource::getProperty(const std::string &key) const {
  if (key == API_KEY_KEY) {
