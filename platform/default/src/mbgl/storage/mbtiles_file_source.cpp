...
std::string url_to_path(const std::string &url) {
  return mbgl::util::percentDecode(
      url.substr(std::char_traits<char>::length(mbgl::util::MBTILES_PROTOCOL)));
}
} // namespace
...
public:
  explicit Impl(const ActorRef<Impl> &, const ResourceOptions &resourceOptions_,
  ...
  void request_tilejson(const Resource &resource,
    ...
    auto db =
        mapbox::sqlite::Database::open(path.c_str(), mapbox::sqlite::ReadOnly);
    ...
    std::string format =
        format_ptr == values.end() ? "png" : format_ptr->second;

    if (format != "pbf" && values.count("scale") == 0) {
      values["scale"] = "1";
    }

    // We use file location with appended parameter query parameter as URL for
    // actual tile data
    std::string tile_url =
        std::string(resource.url + "?file={x}/{y}/{z}." + format);
    ...
    if (minzoom_ptr == values.end() || maxzoom_ptr == values.end()) {
      mapbox::sqlite::Statement zoom_stmt(
          db, "SELECT MIN(zoom_level),MAX(zoom_level) from tiles");
    ...
    for (auto const &entry : values) {
      if (entry.first == "scale") {
        ...
        doc.AddMember(name, rapidjson::Value(entry.second, allocator),
                      allocator);
      }
    }
    ...
    for (mapbox::sqlite::Query q(stmt); q.run();) {
      ...
      if (data) {
        ...
        if (is_compressed(*response.data)) {
          response.data =
              std::make_shared<std::string>(util::decompress(*response.data));
        }
      }
    }
    req.invoke(&FileSourceRequest::setResponse, response);
...
private:
  ...
  // Multiple databases open simultaneoulsy, to effectively support multiple
  // .mbtiles maps
  mapbox::sqlite::Database &get_db(const std::string &path) {
    ...
    auto ptr2 =
        db_cache.insert(std::pair<std::string, mapbox::sqlite::Database>(
            path, mapbox::sqlite::Database::open(path.c_str(),
...
MBTilesFileSource::MBTilesFileSource(const ResourceOptions &resourceOptions,
                                     const ClientOptions &clientOptions)
    : thread(std::make_unique<util::Thread<Impl>>(
          util::makeThreadPrioritySetter(
              platform::EXPERIMENTAL_THREAD_PRIORITY_FILE),
          "MBTilesFileSource", resourceOptions.clone(),
          clientOptions.clone())) {}

std::unique_ptr<AsyncRequest>
MBTilesFileSource::request(const Resource &resource,
