...
- (void)testCoordinateBoundsIsEmpty {
  MLNCoordinateBounds emptyBounds =
      MLNCoordinateBoundsMake(CLLocationCoordinate2DMake(0, 0), CLLocationCoordinate2DMake(10, 0));
  XCTAssertTrue(MLNCoordinateBoundsIsEmpty(emptyBounds));
  XCTAssertFalse(MLNCoordinateSpanEqualToCoordinateSpan(
      MLNCoordinateSpanZero, MLNCoordinateBoundsGetCoordinateSpan(emptyBounds)));
}
...
- (void)testAltitudeConversions {
  ...
  XCTAssertEqualWithAccuracy(
      1800, MLNAltitudeForZoomLevel(MLNZoomLevelForAltitude(1800, 0, 0, midSize), 0, 0, midSize),
      1e-8);
  XCTAssertLessThan(MLNZoomLevelForAltitude(1800, 0, 0, midSize),
                    MLNZoomLevelForAltitude(1800, 0, 0, tallSize));
  XCTAssertGreaterThan(MLNZoomLevelForAltitude(1800, 0, 0, midSize),
                       MLNZoomLevelForAltitude(1800, 0, 0, shortSize));

  XCTAssertEqualWithAccuracy(
      0, MLNZoomLevelForAltitude(MLNAltitudeForZoomLevel(0, 0, 0, midSize), 0, 0, midSize), 1e-8);
  XCTAssertEqualWithAccuracy(
      18, MLNZoomLevelForAltitude(MLNAltitudeForZoomLevel(18, 0, 0, midSize), 0, 0, midSize), 1e-8);

  XCTAssertEqualWithAccuracy(
      0, MLNZoomLevelForAltitude(MLNAltitudeForZoomLevel(0, 0, 40, midSize), 0, 40, midSize), 1e-8);
  XCTAssertEqualWithAccuracy(
      18, MLNZoomLevelForAltitude(MLNAltitudeForZoomLevel(18, 0, 40, midSize), 0, 40, midSize),
      1e-8);

  XCTAssertEqualWithAccuracy(
      0, MLNZoomLevelForAltitude(MLNAltitudeForZoomLevel(0, 60, 40, midSize), 60, 40, midSize),
      1e-8);
  XCTAssertEqualWithAccuracy(
      18, MLNZoomLevelForAltitude(MLNAltitudeForZoomLevel(18, 60, 40, midSize), 60, 40, midSize),
      1e-8);
}

- (void)testGeometryBoxing {
  CLLocationCoordinate2D coordinate = CLLocationCoordinate2DMake(38.9131982, -77.0325453144239);
  CLLocationCoordinate2D roundTrippedCoordinate =
      [NSValue valueWithMLNCoordinate:coordinate].MLNCoordinateValue;

  XCTAssertEqual(coordinate.latitude, roundTrippedCoordinate.latitude,
                 @"Latitude should round-trip.");
  XCTAssertEqual(coordinate.longitude, roundTrippedCoordinate.longitude,
                 @"Longitude should round-trip.");

  MLNCoordinateSpan span = MLNCoordinateSpanMake(4.383333333333335, -4.299999999999997);
  MLNCoordinateSpan roundTrippedSpan =
      [NSValue valueWithMLNCoordinateSpan:span].MLNCoordinateSpanValue;

  XCTAssertEqual(span.latitudeDelta, roundTrippedSpan.latitudeDelta,
                 @"Latitude delta should round-trip.");
  XCTAssertEqual(span.longitudeDelta, roundTrippedSpan.longitudeDelta,
                 @"Longitude delta should round-trip.");

  MLNCoordinateBounds bounds =
      MLNCoordinateBoundsMake(CLLocationCoordinate2DMake(38.9131982, -77.0325453144239),
                              CLLocationCoordinate2DMake(37.7757368, -122.4135302));
  MLNCoordinateBounds roundTrippedBounds =
      [NSValue valueWithMLNCoordinateBounds:bounds].MLNCoordinateBoundsValue;
...
- (void)testGeoJSONDeserialization {
  NSData *data = [@"{\"type\": \"Feature\", \"geometry\": {\"type\": \"Point\", \"coordinates\": "
                  @"[0, 0]}, \"properties\": {}}" dataUsingEncoding:NSUTF8StringEncoding];
  NSError *error;
  ...
  XCTAssertTrue([feature isKindOfClass:[MLNPointFeature class]],
                @"Valid GeoJSON point feature data should produce an MLNPointFeature.");
  ...
  data = [@"{\"type\": \"Feature\", \"feature\": {\"type\": \"Point\", \"coordinates\": [0, 0]}}"
...
- (void)testGeoJSONSerialization {
  ...
  NSError *error;
  NSDictionary *serializedGeoJSON = [NSJSONSerialization JSONObjectWithData:data
                                                                    options:0
                                                                      error:&error];
  ...
  XCTAssertTrue([serializedGeoJSON isKindOfClass:[NSDictionary class]],
                @"Serialized GeoJSON data should be a JSON object.");
  ...
  XCTAssertEqualObjects(serializedGeoJSON, geoJSON,
                        @"MLNPointFeature should serialize as a GeoJSON point feature.");
}
...
- (void)testMLNCoordinateBoundsToMLNCoordinateQuad {
  ...
  XCTAssertEqualObjects(
      [NSValue valueWithMLNCoordinate:CLLocationCoordinate2DMake(46.437, -80.425)],
      [NSValue valueWithMLNCoordinate:quad.topLeft],
      @"Quad top left should be computed correctly.");
  XCTAssertEqualObjects(
      [NSValue valueWithMLNCoordinate:CLLocationCoordinate2DMake(37.936, -71.516)],
