...
- (void)testProperties {
  ...
  MLNRasterStyleLayer *layer = [[MLNRasterStyleLayer alloc] initWithIdentifier:@"layerID"
                                                                        source:source];
  ...
  // raster-brightness-max
  {
    ...
    XCTAssertEqual(rawLayer->getRasterBrightnessMax(), propertyValue,
                   @"Setting maximumRasterBrightness to a constant value expression should update "
                   @"raster-brightness-max.");
    ...
    XCTAssertEqual(rawLayer->getRasterBrightnessMax(), propertyValue,
                   @"Setting maximumRasterBrightness to a camera expression should update "
                   @"raster-brightness-max.");
    ...
    XCTAssertTrue(rawLayer->getRasterBrightnessMax().isUndefined(),
                  @"Unsetting maximumRasterBrightness should return raster-brightness-max to the "
                  @"default value.");
    XCTAssertEqualObjects(
        layer.maximumRasterBrightness, defaultExpression,
        @"maximumRasterBrightness should return the default value after being unset.");

    functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
    XCTAssertThrowsSpecificNamed(
        layer.maximumRasterBrightness = functionExpression, NSException, NSInvalidArgumentException,
        @"MLNRasterLayer should raise an exception if a camera-data expression is applied to a "
  ...
  // raster-brightness-min
  {
    ...
    XCTAssertEqual(rawLayer->getRasterBrightnessMin(), propertyValue,
                   @"Setting minimumRasterBrightness to a constant value expression should update "
                   @"raster-brightness-min.");
    ...
    XCTAssertEqual(rawLayer->getRasterBrightnessMin(), propertyValue,
                   @"Setting minimumRasterBrightness to a camera expression should update "
                   @"raster-brightness-min.");
    ...
    XCTAssertTrue(rawLayer->getRasterBrightnessMin().isUndefined(),
                  @"Unsetting minimumRasterBrightness should return raster-brightness-min to the "
                  @"default value.");
    XCTAssertEqualObjects(
        layer.minimumRasterBrightness, defaultExpression,
        @"minimumRasterBrightness should return the default value after being unset.");

    functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
    XCTAssertThrowsSpecificNamed(
        layer.minimumRasterBrightness = functionExpression, NSException, NSInvalidArgumentException,
        @"MLNRasterLayer should raise an exception if a camera-data expression is applied to a "
        @"property that does not support key paths to feature attributes.");
    functionExpression = [NSExpression expressionWithFormat:@"mgl_step:from:stops:(bogus, %@, %@)",
                                                            constantExpression,
  ...
  // raster-contrast
  {
    ...
    mbgl::style::PropertyValue<float> propertyValue = {1.0};
    XCTAssertEqual(
        rawLayer->getRasterContrast(), propertyValue,
    ...
    functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
    XCTAssertThrowsSpecificNamed(
        layer.rasterContrast = functionExpression, NSException, NSInvalidArgumentException,
        @"MLNRasterLayer should raise an exception if a camera-data expression is applied to a "
        @"property that does not support key paths to feature attributes.");
    functionExpression = [NSExpression expressionWithFormat:@"mgl_step:from:stops:(bogus, %@, %@)",
                                                            constantExpression,
  ...
  // raster-fade-duration
  {
    ...
    XCTAssertEqual(rawLayer->getRasterFadeDuration(), propertyValue,
                   @"Setting rasterFadeDuration to a constant value expression should update "
                   @"raster-fade-duration.");
    ...
    XCTAssertEqual(
        rawLayer->getRasterFadeDuration(), propertyValue,
    ...
    XCTAssertTrue(
        rawLayer->getRasterFadeDuration().isUndefined(),
    ...
    functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
    XCTAssertThrowsSpecificNamed(
        layer.rasterFadeDuration = functionExpression, NSException, NSInvalidArgumentException,
        @"MLNRasterLayer should raise an exception if a camera-data expression is applied to a "
        @"property that does not support key paths to feature attributes.");
    functionExpression = [NSExpression expressionWithFormat:@"mgl_step:from:stops:(bogus, %@, %@)",
                                                            constantExpression,
  ...
  // raster-hue-rotate
  {
    ...
    XCTAssertEqual(rawLayer->getRasterHueRotate(), propertyValue,
                   @"Setting rasterHueRotation to a constant value expression should update "
                   @"raster-hue-rotate.");
    ...
    XCTAssertEqual(
        rawLayer->getRasterHueRotate(), propertyValue,
    ...
    XCTAssertTrue(
        rawLayer->getRasterHueRotate().isUndefined(),
    ...
    functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
    XCTAssertThrowsSpecificNamed(
        layer.rasterHueRotation = functionExpression, NSException, NSInvalidArgumentException,
        @"MLNRasterLayer should raise an exception if a camera-data expression is applied to a "
        @"property that does not support key paths to feature attributes.");
    functionExpression = [NSExpression expressionWithFormat:@"mgl_step:from:stops:(bogus, %@, %@)",
                                                            constantExpression,
  ...
  // raster-opacity
  {
    ...
    mbgl::style::PropertyValue<float> propertyValue = {1.0};
    XCTAssertEqual(
        rawLayer->getRasterOpacity(), propertyValue,
    ...
    functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
    XCTAssertThrowsSpecificNamed(
        layer.rasterOpacity = functionExpression, NSException, NSInvalidArgumentException,
        @"MLNRasterLayer should raise an exception if a camera-data expression is applied to a "
        @"property that does not support key paths to feature attributes.");
    functionExpression = [NSExpression expressionWithFormat:@"mgl_step:from:stops:(bogus, %@, %@)",
                                                            constantExpression,
  ...
  // raster-resampling
  {
    ...
    mbgl::style::PropertyValue<mbgl::style::RasterResamplingType> propertyValue = {
        mbgl::style::RasterResamplingType::Nearest};
    XCTAssertEqual(rawLayer->getRasterResampling(), propertyValue,
                   @"Setting rasterResamplingMode to a constant value expression should update "
                   @"raster-resampling.");
    ...
    XCTAssertEqual(
        rawLayer->getRasterResampling(), propertyValue,
    ...
    XCTAssertTrue(
        rawLayer->getRasterResampling().isUndefined(),
        @"Unsetting rasterResamplingMode should return raster-resampling to the default value.");
    XCTAssertEqualObjects(
        layer.rasterResamplingMode, defaultExpression,
        @"rasterResamplingMode should return the default value after being unset.");

    functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
    XCTAssertThrowsSpecificNamed(
        layer.rasterResamplingMode = functionExpression, NSException, NSInvalidArgumentException,
        @"MLNRasterLayer should raise an exception if a camera-data expression is applied to a "
        @"property that does not support key paths to feature attributes.");
    functionExpression = [NSExpression expressionWithFormat:@"mgl_step:from:stops:(bogus, %@, %@)",
                                                            constantExpression,
  ...
  // raster-saturation
  {
    ...
    XCTAssertEqual(rawLayer->getRasterSaturation(), propertyValue,
                   @"Setting rasterSaturation to a constant value expression should update "
                   @"raster-saturation.");
    ...
    XCTAssertEqual(
        rawLayer->getRasterSaturation(), propertyValue,
    ...
    XCTAssertTrue(
        rawLayer->getRasterSaturation().isUndefined(),
    ...
    functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
    XCTAssertThrowsSpecificNamed(
        layer.rasterSaturation = functionExpression, NSException, NSInvalidArgumentException,
        @"MLNRasterLayer should raise an exception if a camera-data expression is applied to a "
        @"property that does not support key paths to feature attributes.");
    functionExpression = [NSExpression expressionWithFormat:@"mgl_step:from:stops:(bogus, %@, %@)",
                                                            constantExpression,
...
- (void)testValueAdditions {
  XCTAssertEqual([NSValue valueWithMLNRasterResamplingMode:MLNRasterResamplingModeLinear]
                     .MLNRasterResamplingModeValue,
                 MLNRasterResamplingModeLinear);
  XCTAssertEqual([NSValue valueWithMLNRasterResamplingMode:MLNRasterResamplingModeNearest]
                     .MLNRasterResamplingModeValue,
