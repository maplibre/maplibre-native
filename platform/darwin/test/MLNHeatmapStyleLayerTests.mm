...
- (void)testProperties {
  ...
  // heatmap-intensity
  {
    ...
    XCTAssertEqual(rawLayer->getHeatmapIntensity(), propertyValue,
                   @"Setting heatmapIntensity to a constant value expression should update "
                   @"heatmap-intensity.");
    ...
    XCTAssertEqual(
        rawLayer->getHeatmapIntensity(), propertyValue,
    ...
    XCTAssertTrue(
        rawLayer->getHeatmapIntensity().isUndefined(),
    ...
    XCTAssertThrowsSpecificNamed(
        layer.heatmapIntensity = functionExpression, NSException, NSInvalidArgumentException,
  ...
  // heatmap-opacity
  {
    ...
    mbgl::style::PropertyValue<float> propertyValue = {1.0};
    XCTAssertEqual(
        rawLayer->getHeatmapOpacity(), propertyValue,
    ...
    functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
    XCTAssertThrowsSpecificNamed(
        layer.heatmapOpacity = functionExpression, NSException, NSInvalidArgumentException,
        @"MLNHeatmapLayer should raise an exception if a camera-data expression is applied to a "
        @"property that does not support key paths to feature attributes.");
    functionExpression = [NSExpression expressionWithFormat:@"mgl_step:from:stops:(bogus, %@, %@)",
                                                            constantExpression,
  ...
  // heatmap-radius
  {
    ...
    mbgl::style::PropertyValue<float> propertyValue = {1.0};
    XCTAssertEqual(
        rawLayer->getHeatmapRadius(), propertyValue,
  ...
  // heatmap-weight
  {
    ...
    mbgl::style::PropertyValue<float> propertyValue = {1.0};
    XCTAssertEqual(
        rawLayer->getHeatmapWeight(), propertyValue,
