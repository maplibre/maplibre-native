...
- (void)testProperties {
  ...
  // background-color
  {
    ...
    XCTAssertEqual(
        rawLayer->getBackgroundColor(), propertyValue,
    ...
    XCTAssertTrue(
        rawLayer->getBackgroundColor().isUndefined(),
    ...
    XCTAssertThrowsSpecificNamed(
        layer.backgroundColor = functionExpression, NSException, NSInvalidArgumentException,
        @"MLNBackgroundLayer should raise an exception if a camera-data expression is applied to a "
  ...
  // background-opacity
  {
    ...
    XCTAssertEqual(rawLayer->getBackgroundOpacity(), propertyValue,
                   @"Setting backgroundOpacity to a constant value expression should update "
                   @"background-opacity.");
    ...
    XCTAssertEqual(
        rawLayer->getBackgroundOpacity(), propertyValue,
    ...
    XCTAssertTrue(
        rawLayer->getBackgroundOpacity().isUndefined(),
    ...
    functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
    XCTAssertThrowsSpecificNamed(
        layer.backgroundOpacity = functionExpression, NSException, NSInvalidArgumentException,
        @"MLNBackgroundLayer should raise an exception if a camera-data expression is applied to a "
        @"property that does not support key paths to feature attributes.");
    functionExpression = [NSExpression expressionWithFormat:@"mgl_step:from:stops:(bogus, %@, %@)",
                                                            constantExpression,
  ...
  // background-pattern
  {
    ...
    NSExpression *constantExpression =
        [NSExpression expressionForConstantValue:@"Background Pattern"];
    ...
    XCTAssertEqual(rawLayer->getBackgroundPattern(), propertyValue,
                   @"Setting backgroundPattern to a constant value expression should update "
                   @"background-pattern.");
    ...
    constantExpression =
        [NSExpression expressionWithFormat:@"MLN_FUNCTION('image', 'Background Pattern')"];
    XCTExpectFailure(
        @"Awaiting unit test refactoring https://github.com/maplibre/maplibre-native/issues/421");
    NSExpression *functionExpression = [NSExpression
        expressionWithFormat:@"mgl_step:from:stops:($zoomLevel, %@, %@)", constantExpression,
...

    {
      ...
      propertyValue = mbgl::style::PropertyExpression<mbgl::style::expression::Image>(
          step(zoom(), image(literal("Background Pattern")), 18.0,
               image(literal("Background Pattern"))));
    }

    XCTAssertEqual(
        rawLayer->getBackgroundPattern(), propertyValue,
    ...
    XCTAssertTrue(
        rawLayer->getBackgroundPattern().isUndefined(),
    ...
    functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
    XCTAssertThrowsSpecificNamed(
        layer.backgroundPattern = functionExpression, NSException, NSInvalidArgumentException,
        @"MLNBackgroundLayer should raise an exception if a camera-data expression is applied to a "
        @"property that does not support key paths to feature attributes.");
    functionExpression = [NSExpression expressionWithFormat:@"mgl_step:from:stops:(bogus, %@, %@)",
                                                            constantExpression,
