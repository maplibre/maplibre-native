...
- (void)testProperties {
  ...
  mbgl::style::TransitionOptions transitionOptions{{MLNDurationFromTimeInterval(6)},
                                                   {MLNDurationFromTimeInterval(3)}};
  ...
  // anchor
  {
    ...
    XCTAssertEqual(mglLight.anchor.expressionType, NSConstantValueExpressionType,
                   @"mglLight.anchor isn’t a constant value expression.");
    XCTAssertEqualObjects(mglLight.anchor.constantValue, @"viewport");

    mbgl::style::PropertyValue<mbgl::style::LightAnchorType> propertyValue = {
  ...
  // position
  {
    ...
    XCTAssert(positionTransition.delay &&
              MLNTimeIntervalFromDuration(*positionTransition.delay) == defaultTransition.delay);
    XCTAssert(positionTransition.duration &&
              MLNTimeIntervalFromDuration(*positionTransition.duration) ==
    ...
    XCTAssert(positionTransition.delay &&
              MLNTimeIntervalFromDuration(*positionTransition.delay) == transition.delay);
    XCTAssert(positionTransition.duration &&
              MLNTimeIntervalFromDuration(*positionTransition.duration) == transition.duration);
  }
  ...
  // color
  {
    ...
    XCTAssert(colorTransition.delay &&
              MLNTimeIntervalFromDuration(*colorTransition.delay) == defaultTransition.delay);
    XCTAssert(colorTransition.duration &&
              MLNTimeIntervalFromDuration(*colorTransition.duration) == defaultTransition.duration);
    ...
    XCTAssert(colorTransition.delay &&
              MLNTimeIntervalFromDuration(*colorTransition.delay) == transition.delay);
    XCTAssert(colorTransition.duration &&
              MLNTimeIntervalFromDuration(*colorTransition.duration) == transition.duration);
  }
  ...
  // intensity
  {
    ...
    XCTAssert(intensityTransition.delay &&
              MLNTimeIntervalFromDuration(*intensityTransition.delay) == defaultTransition.delay);
    XCTAssert(intensityTransition.duration &&
              MLNTimeIntervalFromDuration(*intensityTransition.duration) ==
                  defaultTransition.duration);
    ...
    XCTAssert(intensityTransition.delay &&
              MLNTimeIntervalFromDuration(*intensityTransition.delay) == transition.delay);
    XCTAssert(intensityTransition.duration &&
              MLNTimeIntervalFromDuration(*intensityTransition.duration) == transition.duration);
  }
}
...
- (void)testValueAdditions {
  ...
  XCTAssertEqual([NSValue valueWithMLNSphericalPosition:position].MLNSphericalPositionValue.radial,
                 position.radial);
  XCTAssertEqual(
      [NSValue valueWithMLNSphericalPosition:position].MLNSphericalPositionValue.azimuthal,
      position.azimuthal);
  XCTAssertEqual([NSValue valueWithMLNSphericalPosition:position].MLNSphericalPositionValue.polar,
                 position.polar);
  XCTAssertEqual([NSValue valueWithMLNLightAnchor:MLNLightAnchorMap].MLNLightAnchorValue,
                 MLNLightAnchorMap);
  XCTAssertEqual([NSValue valueWithMLNLightAnchor:MLNLightAnchorViewport].MLNLightAnchorValue,
