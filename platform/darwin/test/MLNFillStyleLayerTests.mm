...
- (void)testPredicates {
  ...
  MLNShapeSource *source = [[MLNShapeSource alloc] initWithIdentifier:@"sourceID"
                                                                shape:feature
                                                              options:nil];
  MLNFillStyleLayer *layer = [[MLNFillStyleLayer alloc] initWithIdentifier:@"layerID"
                                                                    source:source];
...
- (void)testProperties {
  ...
  // fill-sort-key
  {
    ...
    XCTAssertEqual(
        rawLayer->getFillSortKey(), propertyValue,
  ...
  // fill-antialias
  {
    ...
    XCTAssertEqual(
        rawLayer->getFillAntialias(), propertyValue,
    ...
    XCTAssertThrowsSpecificNamed(
        layer.fillAntialiased = functionExpression, NSException, NSInvalidArgumentException,
        @"MLNFillLayer should raise an exception if a camera-data expression is applied to a "
  ...
  // fill-color
  {
    XCTAssertTrue(rawLayer->getFillColor().isUndefined(), @"fill-color should be unset initially.");
  ...
  // fill-opacity
  {
    ...
    mbgl::style::PropertyValue<float> propertyValue = {1.0};
    XCTAssertEqual(
        rawLayer->getFillOpacity(), propertyValue,
  ...
  // fill-outline-color
  {
    ...
    XCTAssertEqual(rawLayer->getFillOutlineColor(), propertyValue,
                   @"Setting fillOutlineColor to a constant value expression should update "
                   @"fill-outline-color.");
    ...
    XCTAssertEqual(
        rawLayer->getFillOutlineColor(), propertyValue,
    ...
    XCTAssertTrue(
        rawLayer->getFillOutlineColor().isUndefined(),
  ...
  // fill-pattern
  {
    ...
    XCTAssertEqual(
        rawLayer->getFillPattern(), propertyValue,
    ...
    constantExpression =
        [NSExpression expressionWithFormat:@"MLN_FUNCTION('image', 'Fill Pattern')"];
    XCTExpectFailure(
        @"Awaiting unit test refactoring https://github.com/maplibre/maplibre-native/issues/421");
    NSExpression *functionExpression = [NSExpression
        expressionWithFormat:@"mgl_step:from:stops:($zoomLevel, %@, %@)", constantExpression,
  ...
  // fill-translate
  {
    ...
    XCTAssertEqual(
        rawLayer->getFillTranslate(), propertyValue,
    ...
    functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
    XCTAssertThrowsSpecificNamed(
        layer.fillTranslation = functionExpression, NSException, NSInvalidArgumentException,
        @"MLNFillLayer should raise an exception if a camera-data expression is applied to a "
        @"property that does not support key paths to feature attributes.");
    functionExpression = [NSExpression expressionWithFormat:@"mgl_step:from:stops:(bogus, %@, %@)",
                                                            constantExpression,
  ...
  // fill-translate-anchor
  {
    ...
    XCTAssertEqual(rawLayer->getFillTranslateAnchor(), propertyValue,
                   @"Setting fillTranslationAnchor to a constant value expression should update "
                   @"fill-translate-anchor.");
    ...
    XCTAssertEqual(rawLayer->getFillTranslateAnchor(), propertyValue,
                   @"Setting fillTranslationAnchor to a camera expression should update "
                   @"fill-translate-anchor.");
    ...
    XCTAssertTrue(rawLayer->getFillTranslateAnchor().isUndefined(),
                  @"Unsetting fillTranslationAnchor should return fill-translate-anchor to the "
                  @"default value.");
    ...
    functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
    XCTAssertThrowsSpecificNamed(
        layer.fillTranslationAnchor = functionExpression, NSException, NSInvalidArgumentException,
        @"MLNFillLayer should raise an exception if a camera-data expression is applied to a "
        @"property that does not support key paths to feature attributes.");
    functionExpression = [NSExpression expressionWithFormat:@"mgl_step:from:stops:(bogus, %@, %@)",
                                                            constantExpression,
...
- (void)testValueAdditions {
  XCTAssertEqual([NSValue valueWithMLNFillTranslationAnchor:MLNFillTranslationAnchorMap]
                     .MLNFillTranslationAnchorValue,
                 MLNFillTranslationAnchorMap);
  XCTAssertEqual([NSValue valueWithMLNFillTranslationAnchor:MLNFillTranslationAnchorViewport]
                     .MLNFillTranslationAnchorValue,
