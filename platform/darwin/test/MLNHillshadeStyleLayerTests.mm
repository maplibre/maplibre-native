...
- (void)testProperties {
    ...
    // hillshade-highlight-color
    {
        ...
        NSExpression *constantExpression = [NSExpression expressionWithFormat:@"%@", @[[MLNColor redColor], [MLNColor blueColor]]];
        layer.hillshadeHighlightColor = constantExpression;
        mbgl::style::PropertyValue<std::vector<mbgl::Color>> propertyValue = { { { 1, 0, 0, 1 }, { 0, 0, 1, 1 } } };
        ...
        constantExpression = [NSExpression expressionWithFormat:@"%@", @[[MLNColor redColor], [MLNColor blueColor]]];
        XCTExpectFailure(@"Awaiting unit test refactoring https://github.com/maplibre/maplibre-native/issues/421");
        NSExpression *functionExpression = [NSExpression expressionWithFormat:@"mgl_step:from:stops:($zoomLevel, %@, %@)", constantExpression, @{@18: constantExpression}];
...

        {
            ...
            propertyValue = mbgl::style::PropertyExpression<std::vector<mbgl::Color>>(
                step(zoom(), literal({ { 1, 0, 0, 1 }, { 0, 0, 1, 1 } }), 18.0, literal({ { 1, 0, 0, 1 }, { 0, 0, 1, 1 } }))
            );
        }
    ...
    // hillshade-illumination-altitude
    {
        ...
        XCTAssertEqual(rawLayer->getHillshadeIlluminationAltitude(), propertyValue,
                       @"Setting hillshadeIlluminationAltitude to a constant value expression should update hillshade-illumination-altitude.");
        XCTAssertEqualObjects(layer.hillshadeIlluminationAltitude, constantExpression,
        ...
        XCTAssertEqual(rawLayer->getHillshadeIlluminationAltitude(), propertyValue,
                       @"Setting hillshadeIlluminationAltitude to a camera expression should update hillshade-illumination-altitude.");
        ...
        XCTAssertTrue(rawLayer->getHillshadeIlluminationAltitude().isUndefined(),
                      @"Unsetting hillshadeIlluminationAltitude should return hillshade-illumination-altitude to the default value.");
        XCTAssertEqualObjects(layer.hillshadeIlluminationAltitude, defaultExpression,
                              @"hillshadeIlluminationAltitude should return the default value after being unset.");

        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.hillshadeIlluminationAltitude = functionExpression, NSException, NSInvalidArgumentException, @"MLNHillshadeLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"mgl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"mgl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.hillshadeIlluminationAltitude = functionExpression, NSException, NSInvalidArgumentException, @"MLNHillshadeLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // hillshade-method
    {
        ...
        mbgl::style::PropertyValue<mbgl::style::HillshadeMethodType> propertyValue = { mbgl::style::HillshadeMethodType::Multidirectional };
        XCTAssertEqual(rawLayer->getHillshadeMethod(), propertyValue,
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.hillshadeMethod = functionExpression, NSException, NSInvalidArgumentException, @"MLNHillshadeLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"mgl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"mgl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.hillshadeMethod = functionExpression, NSException, NSInvalidArgumentException, @"MLNHillshadeLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
...
- (void)testValueAdditions {
    ...
    XCTAssertEqual([NSValue valueWithMLNHillshadeMethod:MLNHillshadeMethodStandard].MLNHillshadeMethodValue, MLNHillshadeMethodStandard);
    XCTAssertEqual([NSValue valueWithMLNHillshadeMethod:MLNHillshadeMethodBasic].MLNHillshadeMethodValue, MLNHillshadeMethodBasic);
    XCTAssertEqual([NSValue valueWithMLNHillshadeMethod:MLNHillshadeMethodCombined].MLNHillshadeMethodValue, MLNHillshadeMethodCombined);
    XCTAssertEqual([NSValue valueWithMLNHillshadeMethod:MLNHillshadeMethodIgor].MLNHillshadeMethodValue, MLNHillshadeMethodIgor);
    XCTAssertEqual([NSValue valueWithMLNHillshadeMethod:MLNHillshadeMethodMultidirectional].MLNHillshadeMethodValue, MLNHillshadeMethodMultidirectional);
}
