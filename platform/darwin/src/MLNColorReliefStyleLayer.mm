...
- (instancetype)initWithIdentifier:(NSString *)identifier source:(MLNSource *)source
{
    ...
    auto layer = std::make_unique<mbgl::style::ColorReliefLayer>(identifier.UTF8String, source.identifier.UTF8String);
...
- (void)setColorReliefColor:(NSExpression *)colorReliefColor {
    ...
    auto mbglValue = MLNStyleValueTransformer<mbgl::Color, MLNColor *>().toPropertyValue<mbgl::style::ColorRampPropertyValue>(colorReliefColor);
...
- (void)setColorReliefOpacity:(NSExpression *)colorReliefOpacity {
    ...
    auto mbglValue = MLNStyleValueTransformer<float, NSNumber *>().toPropertyValue<mbgl::style::PropertyValue<float>>(colorReliefOpacity, false);
...
- (MLNTransition)colorReliefOpacityTransition {
    ...
    mbgl::style::TransitionOptions transitionOptions = self.rawLayer->getColorReliefOpacityTransition();
