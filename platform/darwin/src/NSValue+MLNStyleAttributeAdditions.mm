...
+ (instancetype)mgl_valueWithLocationArray:(std::array<double, 3>)locationArray {
  CLLocation* location = [[CLLocation alloc] initWithLatitude:locationArray[0]
...
- (std::array<float, 4>)mgl_paddingArrayValue {
  MLNAssert(strcmp(self.objCType, @encode(MLNEdgeInsets)) == 0,
            @"Value does not represent an NSEdgeInsets/UIEdgeInsets");
  MLNEdgeInsets insets;
...
- (std::array<double, 3>)mgl_locationArrayValue {
  MLNAssert(strcmp(self.objCType, @encode(CLLocation)) == 0,
            @"Value does not represent an CLLocation");
  CLLocation* location;
...
- (std::array<float, 3>)mgl_lightPositionArrayValue {
  MLNAssert(strcmp(self.objCType, @encode(MLNSphericalPosition)) == 0,
            @"Value does not represent an MLNSphericalPosition");
