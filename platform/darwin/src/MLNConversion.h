...
public:
  ...
  static std::optional<Holder> objectMember(const Holder& holder, const char* key) {
    const id value = holder.value;
    NSCAssert([value isKindOfClass:[NSDictionary class]],
              @"Value must be an NSDictionary for get(string).");
...
#pragma clang diagnostic pop
  ...
  static std::optional<GeoJSON> toGeoJSON(const Holder& holder, Error& error) {
    ...
    if ([object isKindOfClass:[NSDictionary class]]) {
      ...
      error = {std::string(
          [NSString stringWithFormat:@"%@ is not a GeoJSON object.", object].UTF8String)};
      return {};
    }
  }
...
private:
  ...
  static bool _isString(const id value) { return [value isKindOfClass:[NSString class]]; }
};

inline Convertible makeConvertible(const id value) { return Convertible(Holder(value)); }
