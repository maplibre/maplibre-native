...
//        NSMutableArray *_assets = nil;
        ...
        void drawRenderList(NSArray<GLTFMTLRenderItem *> *renderList,
                            id<MTLRenderCommandEncoder> renderEncoder);

        id<MTLRenderPipelineState> renderPipelineStateForSubmesh(GLTFSubmesh *submesh);

        void bindTexturesForMaterial(GLTFMaterial *material,
                                     id<MTLRenderCommandEncoder> renderEncoder);

        id<MTLBuffer> dequeueReusableBufferOfLength(size_t length);

        void computeJointsForSubmesh(GLTFSubmesh *submesh,
                                                    GLTFNode *node,
