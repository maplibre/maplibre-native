...
NS_INLINE NSString *MLNStringFromBOOL(BOOL value) { return value ? @"YES" : @"NO"; }
...
#define MLNLogInfo(message, ...) \
  MLNLogWithType(MLNLoggingLevelInfo, __PRETTY_FUNCTION__, __LINE__, message, ##__VA_ARGS__)
#define MLNLogWarning(message, ...) \
  MLNLogWithType(MLNLoggingLevelWarning, __PRETTY_FUNCTION__, __LINE__, message, ##__VA_ARGS__)
#define MLNLogError(message, ...) \
  MLNLogWithType(MLNLoggingLevelError, __PRETTY_FUNCTION__, __LINE__, message, ##__VA_ARGS__)
#define MLNLogFault(message, ...) \
  MLNLogWithType(MLNLoggingLevelFault, __PRETTY_FUNCTION__, __LINE__, message, ##__VA_ARGS__)
#define MLNLogErrorMessage(message) \
  MLNLogWithType(MLNLoggingLevelError, __PRETTY_FUNCTION__, __LINE__, message)
...
#define MLNLogWithType(type, function, line, message, ...)                                         \
  {                                                                                                \
    if ([MLNLoggingConfiguration sharedConfiguration].loggingLevel != MLNLoggingLevelNone &&       \
        type <= [MLNLoggingConfiguration sharedConfiguration].loggingLevel) {                      \
      [[MLNLoggingConfiguration sharedConfiguration] logCallingFunction:function                   \
                                                           functionLine:line                       \
                                                            messageType:type                       \
