...
- (NSString *)mgl_keyPath {
  ...
  [NSException raise:NSInvalidArgumentException
              format:@"Comparison predicate must compare an attribute (as a key path) to a "
...
- (id)mgl_jsonExpressionObject {
      ...
      if (self.predicateOperatorType == NSNotEqualToPredicateOperatorType) {
        ...
        if (self.options) {
          [NSException
              ...
              format:@"NSComparisonPredicateOptions not supported for “ALL … !=” comparisons."];
        }

        NSPredicate *directPredicate =
        ...
        NSPredicate *invertedPredicate =
            [NSCompoundPredicate notPredicateWithSubpredicate:directPredicate];
        return invertedPredicate.mgl_jsonExpressionObject;
      } else {
        [NSException raise:NSInvalidArgumentException
                    format:@"“ALL” is only supported for the “!=” operator."];
      }
    ...
    case NSAnyPredicateModifier:
      ...
      if (self.predicateOperatorType == NSEqualToPredicateOperatorType) {
        ...
        if (self.options) {
          [NSException
               raise:NSInvalidArgumentException
              format:@"NSComparisonPredicateOptions not supported for “ANY … =” comparisons."];
        }
        ...
        [NSException raise:NSInvalidArgumentException
                    format:@"“ANY” or “SOME” is only supported for the “=” operator."];
      }

    default:
      [NSException raise:NSInvalidArgumentException
                  format:@"NSComparisonPredicateModifier:%lu is not supported.",
                         (unsigned long)self.comparisonPredicateModifier];
  }
  ...
  switch (self.predicateOperatorType) {
    ...
    case NSBetweenPredicateOperatorType: {
      ...
      NSPredicate *leftHandPredicate = [NSComparisonPredicate
          predicateWithLeftExpression:limits[0]
                      rightExpression:self.leftExpression
                             modifier:NSDirectPredicateModifier
                                 type:NSLessThanOrEqualToPredicateOperatorType
                              options:0];
      NSPredicate *rightHandPredicate = [NSComparisonPredicate
          predicateWithLeftExpression:self.leftExpression
                      rightExpression:limits[1]
                             modifier:NSDirectPredicateModifier
                              ...
                              options:0];
      return @[
        op, leftHandPredicate.mgl_jsonExpressionObject, rightHandPredicate.mgl_jsonExpressionObject
      ];
    }
    case NSInPredicateOperatorType: {
      ...
      if (self.leftExpression.expressionType == NSConstantValueExpressionType &&
        ...
        NSExpression *rightLiteralExpression = [NSExpression
            expressionWithFormat:@"MLN_FUNCTION('literal', %@)", self.rightExpression];
        NSPredicate *literalPredicate =
            [NSComparisonPredicate predicateWithLeftExpression:self.leftExpression
    ...
    case NSCustomSelectorPredicateOperatorType:
      [NSException raise:NSInvalidArgumentException
                  format:@"NSPredicateOperatorType:%lu is not supported.",
                         (unsigned long)self.predicateOperatorType];
  }
  if (!op) {
    return nil;
  }
  NSArray *comparisonArray = @[
    op, self.leftExpression.mgl_jsonExpressionObject, self.rightExpression.mgl_jsonExpressionObject
  ];
  if (self.options) {
