...
- (instancetype)initWithMBGLLight:(const mbgl::style::Light *)mbglLight {
  if (self = [super init]) {
    ...
    if (anchor.isUndefined()) {
      mbgl::style::PropertyValue<mbgl::style::LightAnchorType> defaultAnchor =
          mbglLight->getDefaultAnchor();
      anchorExpression = MLNStyleValueTransformer<mbgl::style::LightAnchorType, NSValue *,
                                                  mbgl::style::LightAnchorType, MLNLightAnchor>()
                             .toExpression(defaultAnchor);
    } else {
      anchorExpression = MLNStyleValueTransformer<mbgl::style::LightAnchorType, NSValue *,
                                                  mbgl::style::LightAnchorType, MLNLightAnchor>()
                             .toExpression(anchor);
    ...
    if (positionValue.isUndefined()) {
      _position = MLNStyleValueTransformer<mbgl::style::Position, NSValue *>().toExpression(
          mbglLight->getDefaultPosition());
    } else {
      _position =
          MLNStyleValueTransformer<mbgl::style::Position, NSValue *>().toExpression(positionValue);
    }
    _positionTransition = MLNTransitionFromOptions(mbglLight->getPositionTransition());
    auto colorValue = mbglLight->getColor();
    if (colorValue.isUndefined()) {
      _color = MLNStyleValueTransformer<mbgl::Color, MLNColor *>().toExpression(
          mbglLight->getDefaultColor());
    } else {
    ...
    if (intensityValue.isUndefined()) {
      _intensity = MLNStyleValueTransformer<float, NSNumber *>().toExpression(
          mbglLight->getDefaultIntensity());
    } else {
...
- (mbgl::style::Light)mbglLight {
  mbgl::style::Light mbglLight;
  auto anchor = MLNStyleValueTransformer<mbgl::style::LightAnchorType, NSValue *,
                                         mbgl::style::LightAnchorType, MLNLightAnchor>()
                    .toPropertyValue<mbgl::style::PropertyValue<mbgl::style::LightAnchorType>>(
                        self.anchor, false);
  mbglLight.setAnchor(anchor);

  auto position =
      MLNStyleValueTransformer<mbgl::style::Position, NSValue *>()
          .toPropertyValue<mbgl::style::PropertyValue<mbgl::style::Position>>(self.position, false);
  mbglLight.setPosition(position);

  mbglLight.setPositionTransition(MLNOptionsFromTransition(self.positionTransition));

  auto color = MLNStyleValueTransformer<mbgl::Color, MLNColor *>()
                   .toPropertyValue<mbgl::style::PropertyValue<mbgl::Color>>(self.color, false);
  mbglLight.setColor(color);

  mbglLight.setColorTransition(MLNOptionsFromTransition(self.colorTransition));

  auto intensity = MLNStyleValueTransformer<float, NSNumber *>()
                       .toPropertyValue<mbgl::style::PropertyValue<float>>(self.intensity, false);
