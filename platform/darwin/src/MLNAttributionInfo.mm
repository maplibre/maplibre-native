...
+ (NSArray<MLNAttributionInfo *> *)attributionInfosFromHTMLString:(nullable NSString *)htmlString
  ...
  NSMutableString *css = [NSMutableString
      stringWithString:@"html { font-family: -apple-system, -apple-system-font, sans-serif; }"
  ...
  if (linkColor) {
    ...
    [css appendFormat:@"a:link { color: rgba(%f%%, %f%%, %f%%, %f); }", red * 100, green * 100,
                      blue * 100, alpha];
  }
  NSString *styledHTML =
      [NSString stringWithFormat:@"<style type='text/css'>%@</style>%@", css, htmlString];
  ...
  dispatch_block_t initialization = ^{
    // This initializer should be called from a global or main queue.
  ...
  [attributedString
      enumerateAttribute:NSLinkAttributeName
              ...
              usingBlock:^(id _Nullable value, NSRange range, BOOL *_Nonnull stop) {
                MLNCAssert(!value || [value isKindOfClass:[NSURL class]],
                           @"If present, URL attribute must be an NSURL.");
                ...
                // Clean up strings by stripping punctuation and whitespace (often present for the
                // web).
                ...
                [unlinkedTitle removeAttribute:NSLinkAttributeName
                                         range:unlinkedTitle.mgl_wholeRange];

                MLNAttributionInfo *info = [[MLNAttributionInfo alloc] initWithTitle:unlinkedTitle
                                                                                 URL:value];
...
- (id)copyWithZone:(nullable NSZone *)zone {
  MLNAttributionInfo *info = [[[self class] allocWithZone:zone] initWithTitle:_title URL:_URL];
...
- (NSAttributedString *)titleWithStyle:(MLNAttributionInfoStyle)style {
  NSString *openStreetMap =
      NSLocalizedStringWithDefaultValue(@"OSM_FULL_NAME", @"Foundation", nil, @"OpenStreetMap",
                                        @"OpenStreetMap full name attribution");
  NSString *OSM = NSLocalizedStringWithDefaultValue(@"OSM_SHORT_NAME", @"Foundation", nil, @"OSM",
                                                    @"OpenStreetMap short name attribution");

  NSMutableAttributedString *title =
      [[NSMutableAttributedString alloc] initWithAttributedString:self.title];
  ...
  if ([title.string rangeOfString:@"OpenStreetMap"].location != NSNotFound) {
    [title.mutableString replaceOccurrencesOfString:@"OpenStreetMap"
                                         withString:isAbbreviated ? OSM : openStreetMap
...
- (BOOL)isEqual:(id)object {
  return [object isKindOfClass:[self class]] && [[object title] isEqual:self.title] &&
         [[object URL] isEqual:self.URL];
}
...
- (void)growArrayByAddingAttributionInfo:(MLNAttributionInfo *)info {
  ...
  [self enumerateObjectsUsingBlock:^(MLNAttributionInfo *_Nonnull existingInfo, NSUInteger idx,
                                     BOOL *_Nonnull stop) {
