...
- (NSString *)description {
  return [NSString stringWithFormat:@"<%@: %p; identifier = %@, attributes = %@>",
                                    ...
                                    self.identifier
                                        ? [NSString stringWithFormat:@"\"%@\"", self.identifier]
                                        : self.identifier,
...
- (NSDictionary *)geoJSONDictionary {
  return NSDictionaryFeatureForGeometry([super geoJSONDictionary], self.attributes,
                                        self.identifier);
}
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@: %p; identifier = %@, coordinate = %f, %f, attributes = %@>",
                       NSStringFromClass([self class]), (void *)self,
                       self.identifier ? [NSString stringWithFormat:@"\"%@\"", self.identifier]
                                       : self.identifier,
...
- (NSUInteger)clusterIdentifier {
  NSNumber *clusterNumber =
      MLN_OBJC_DYNAMIC_CAST([self attributeForKey:MLNClusterIdentifierKey], NSNumber);
...
- (NSString *)description {
  return [NSString
      stringWithFormat:@"<%@: %p; identifier = %@, count = %lu, bounds = %@, attributes = %@>",
                       NSStringFromClass([self class]), (void *)self,
                       self.identifier ? [NSString stringWithFormat:@"\"%@\"", self.identifier]
                                       : self.identifier,
...
public:
  ...
  MLNShape *operator()(const mbgl::MultiLineString<T> &geometry) const {
    ...
    for (auto &lineString : geometry) {
      ...
      MLNPolyline *polyline = [MLNPolyline polylineWithCoordinates:&coordinates[0]
  ...
  MLNShape *operator()(const mapbox::geometry::geometry_collection<T> &collection) const {
    ...
    for (auto &geometry : collection) {
      // This is very much like the transformation that happens in MLNFeaturesFromMBGLFeatures(),
      // but these are raw geometries with no associated feature IDs or attributes.
      MLNShape *shape = mapbox::geometry::geometry<T>::visit(geometry, *this);
      [shapes addObject:shape];
    }
    Class shapeClass =
        is_in_feature ? [MLNShapeCollectionFeature class] : [MLNShapeCollection class];
  ...
  static U *toShape(const mbgl::Polygon<T> &geometry, const bool isInFeature) {
    ...
    if (geometry.size() > 1) {
      ...
      for (auto iter = geometry.begin() + 1; iter != geometry.end(); iter++) {
        ...
        MLNPolygon *innerPolygon = [MLNPolygon polygonWithCoordinates:&innerCoordinates[0]
                                                                count:innerCoordinates.size()];
        [innerPolygons addObject:innerPolygon];
      }
    }

    Class shapeClass = isInFeature ? [V class] : [U class];
    return [shapeClass polygonWithCoordinates:&coordinates[0]
                                        count:coordinates.size()
...
NSDictionary<NSString *, id> *NSDictionaryFeatureForGeometry(NSDictionary *geometry,
  ...
  NSMutableDictionary *feature =
      [@{@"type" : @"Feature", @"properties" : attributes, @"geometry" : geometry} mutableCopy];
