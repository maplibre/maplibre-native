...
+ (instancetype)cameraLookingAtCenterCoordinate:(CLLocationCoordinate2D)centerCoordinate
  ...
  if (CLLocationCoordinate2DIsValid(centerCoordinate) &&
      CLLocationCoordinate2DIsValid(eyeCoordinate)) {
...
+ (instancetype)cameraLookingAtCenterCoordinate:(CLLocationCoordinate2D)centerCoordinate
                                   fromDistance:(CLLocationDistance)distance
                                          pitch:(CGFloat)pitch
                                        heading:(CLLocationDirection)heading {
  // TODO: Remove this compatibility shim in favor of
...
- (instancetype)initWithCenterCoordinate:(CLLocationCoordinate2D)centerCoordinate
                                altitude:(CLLocationDistance)altitude
                                   pitch:(CGFloat)pitch
                                 heading:(CLLocationDirection)heading {
  MLNLogDebug(@"Initializing withCenterCoordinate: %@ altitude: %.0fm pitch: %f° heading: %f°",
              MLNStringFromCLLocationCoordinate2D(centerCoordinate), altitude, pitch, heading);
  if (self = [super init]) {
...
- (NSString *)description {
  return [NSString stringWithFormat:@"<%@: %p; centerCoordinate = %f, %f; altitude = %.0fm; "
                                    @"heading = %.0f°; pitch = %.0f°; roll = %.0f°>",
                                    NSStringFromClass([self class]), (void *)self,
                                    _centerCoordinate.latitude, _centerCoordinate.longitude,
                                    _altitude, _heading, _pitch, _roll];
}
...
- (BOOL)isEqual:(id)other {
  ...
  return (_centerCoordinate.latitude == otherCamera.centerCoordinate.latitude &&
          ...
          _altitude == otherCamera.altitude && _pitch == otherCamera.pitch &&
          _heading == otherCamera.heading && _roll == otherCamera.roll);
}
...
- (BOOL)isEqualToMapCamera:(MLNMapCamera *)otherCamera {
  ...
  return (MLNEqualFloatWithAccuracy(_centerCoordinate.latitude,
                                    otherCamera.centerCoordinate.latitude, 1e-8) &&
          MLNEqualFloatWithAccuracy(_centerCoordinate.longitude,
                                    otherCamera.centerCoordinate.longitude, 1e-8) &&
