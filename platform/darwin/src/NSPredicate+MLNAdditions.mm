...
+ (instancetype)predicateWithMLNJSONObject:(id)object {
  ...
  MLNAssert([object isKindOfClass:[NSArray class]],
            @"Condition for case expression should be an expression.");
  ...
  if (operatorTypeNumber) {
    NSPredicateOperatorType operatorType =
        (NSPredicateOperatorType)[operatorTypeNumber unsignedIntegerValue];

    NSComparisonPredicateOptions options = 0;
    if (objects.count > 3) {
      NSArray *collatorExpression = objects[3];
      MLNCAssert([collatorExpression isKindOfClass:[NSArray class]],
                 @"Collators must be dictionaries.");
      MLNCAssert(collatorExpression.count == 2, @"Malformed collator expression");
      NSDictionary *collator = collatorExpression[1];
      MLNCAssert([collator isKindOfClass:[NSDictionary class]],
                 @"Malformed collator in collator expression");
    ...
    NSArray *subexpressions = MLNSubexpressionsWithJSONObjects(
        [objects subarrayWithRange:NSMakeRange(1, objects.count - 1)]);
  ...
  if ([op isEqualToString:@"!"]) {
    NSArray *subpredicates = MLNSubpredicatesWithJSONObjects(
        [objects subarrayWithRange:NSMakeRange(1, objects.count - 1)]);
  ...
  if ([op isEqualToString:@"all"]) {
    NSArray<NSPredicate *> *subpredicates = MLNSubpredicatesWithJSONObjects(
        [objects subarrayWithRange:NSMakeRange(1, objects.count - 1)]);
    if (subpredicates.count == 2) {
      ...
      if ([subpredicates[0] isKindOfClass:[NSComparisonPredicate class]] &&
        ...
        } else if (leftCondition.predicateOperatorType ==
                       NSLessThanOrEqualToPredicateOperatorType &&
        ...
        if (limits && leftConditionExpression) {
          return [NSPredicate predicateWithFormat:@"%@ BETWEEN %@", leftConditionExpression,
  ...
  return [NSComparisonPredicate
      predicateWithLeftExpression:expression
...
- (id)mgl_jsonExpressionObject {
  ...
  [NSException raise:NSInvalidArgumentException format:@"Unrecognized predicate type."];
  return nil;
}
