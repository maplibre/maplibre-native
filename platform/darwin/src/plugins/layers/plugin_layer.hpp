...
public:
    virtual ~PluginLayerHost() = default;
    /**
...
class PluginLayer : public Layer {
public:
    PluginLayer(const std::string& id, std::unique_ptr<PluginLayerHost> host);

    PluginLayer(const PluginLayer&) = delete;
    ~PluginLayer() final;
    class Impl;
