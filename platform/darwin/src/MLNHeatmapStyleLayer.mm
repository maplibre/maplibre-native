...
- (instancetype)initWithIdentifier:(NSString *)identifier source:(MLNSource *)source {
  MLNLogDebug(@"Initializing %@ with identifier: %@ source: %@", NSStringFromClass([self class]),
              identifier, source);
  auto layer = std::make_unique<mbgl::style::HeatmapLayer>(identifier.UTF8String,
...
- (void)setHeatmapColor:(NSExpression *)heatmapColor {
  ...
  auto mbglValue = MLNStyleValueTransformer<mbgl::Color, MLNColor *>()
                       .toPropertyValue<mbgl::style::ColorRampPropertyValue>(heatmapColor);
...
- (void)setHeatmapIntensity:(NSExpression *)heatmapIntensity {
  ...
  auto mbglValue = MLNStyleValueTransformer<float, NSNumber *>()
                       .toPropertyValue<mbgl::style::PropertyValue<float>>(heatmapIntensity, false);
...
- (void)setHeatmapOpacity:(NSExpression *)heatmapOpacity {
  ...
  auto mbglValue = MLNStyleValueTransformer<float, NSNumber *>()
                       .toPropertyValue<mbgl::style::PropertyValue<float>>(heatmapOpacity, false);
...
- (void)setHeatmapRadius:(NSExpression *)heatmapRadius {
  ...
  auto mbglValue = MLNStyleValueTransformer<float, NSNumber *>()
                       .toPropertyValue<mbgl::style::PropertyValue<float>>(heatmapRadius, true);
...
- (void)setHeatmapWeight:(NSExpression *)heatmapWeight {
  ...
  auto mbglValue = MLNStyleValueTransformer<float, NSNumber *>()
                       .toPropertyValue<mbgl::style::PropertyValue<float>>(heatmapWeight, true);
