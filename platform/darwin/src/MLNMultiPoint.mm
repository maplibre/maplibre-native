...
- (BOOL)isEqual:(id)other {
  ...
  if (![other isKindOfClass:[MLNMultiPoint class]]) return NO;

  MLNMultiPoint *otherMultipoint = other;
  return ([super isEqual:otherMultipoint] && _coordinates == otherMultipoint->_coordinates);
}
...
- (void)getCoordinates:(CLLocationCoordinate2D *)coords range:(NSRange)range {
  ...
  std::copy(_coordinates.begin() + range.location, _coordinates.begin() + NSMaxRange(range),
            coords);
}

- (void)setCoordinates:(CLLocationCoordinate2D *)coords count:(NSUInteger)count {
  MLNLogDebug(@"Setting: %lu coordinates.", (unsigned long)count);
  if (!count) {
    [NSException raise:NSInvalidArgumentException
                format:@"A multipoint must have at least one vertex."];
  }
...
- (void)insertCoordinates:(const CLLocationCoordinate2D *)coords
  ...
  MLNLogDebug(@"Inserting %lu coordinates at index %lu.", (unsigned long)count,
              (unsigned long)index);
  if (!count) {
    return;
  }
  ...
  if (index > _coordinates.size()) {
    [NSException raise:NSRangeException
                format:@"Invalid index %lu for existing coordinate count %ld", (unsigned long)index,
...
- (void)appendCoordinates:(const CLLocationCoordinate2D *)coords count:(NSUInteger)count {
  MLNLogDebug(@"Appending %lu coordinates.", (unsigned long)count);
...
- (void)replaceCoordinatesInRange:(NSRange)range
  ...
  MLNLogDebug(@"Replacing coordinates in range: %@", NSStringFromRange(range));
...
- (void)replaceCoordinatesInRange:(NSRange)range
                  withCoordinates:(const CLLocationCoordinate2D *)coords
                            count:(NSUInteger)count {
  MLNLogDebug(@"Replacing %lu coordinates in range %@.", (unsigned long)count,
  ...
  if (count >= range.length) {
    _coordinates.insert(_coordinates.begin() + NSMaxRange(range), coords,
                        coords + count - range.length);
  } else {
    _coordinates.erase(_coordinates.begin() + range.location + count,
                       _coordinates.begin() + NSMaxRange(range));
  }
  _bounds = {};
  [self didChangeValueForKey:@"coordinates"];
}
...
- (void)removeCoordinatesInRange:(NSRange)range {
  MLNLogDebug(@"Removing coordinatesInRange: %@", NSStringFromRange(range));
