...
CGImageRef CGImageCreateWithMLNPremultipliedImage(mbgl::PremultipliedImage&& src) {
  ...
  return CGImageCreate(src.size.width, src.size.height, bitsPerComponent, bitsPerPixel, bytesPerRow,
...
mbgl::PremultipliedImage MLNPremultipliedImageFromCGImage(CGImageRef src) {
  ...
  CGContextHandle context(CGBitmapContextCreate(image.data.get(), width, height, bitsPerComponent,
