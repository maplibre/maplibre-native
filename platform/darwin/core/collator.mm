...
public:
  Impl(bool caseSensitive, bool diacriticSensitive, const std::optional<std::string>& locale_)
      : options((caseSensitive ? 0 : NSCaseInsensitiveSearch) |
        ...
        locale(locale_ ? [[NSLocale alloc] initWithLocaleIdentifier:@((*locale_).c_str())]
  ...
  std::string resolvedLocale() const {
    ...
    // may append the region tag with an "_". Changing that to a "-" makes the identifier BCP 47
    // compliant. Experimentally, "zh-Hans-HK" and "zh-Hans_HK" both round trip -- if the second is
    // used by
...
Collator::Collator(bool caseSensitive, bool diacriticSensitive,
                   const std::optional<std::string>& locale_)
    : impl(std::make_shared<Impl>(caseSensitive, diacriticSensitive, locale_)) {}

bool Collator::operator==(const Collator& other) const { return *impl == *(other.impl); }
