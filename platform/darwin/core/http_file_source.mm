...
public:
  HTTPRequestShared(Response &response_, util::AsyncTask &async_)
      : response(response_), async(async_) {}
...
public:
  HTTPRequest(FileSource::Callback callback_)
      : shared(std::make_shared<HTTPRequestShared>(response, async)), callback(callback_) {}
...
ResourceOptions HTTPFileSource::Impl::getResourceOptions() { return resourceOptions.clone(); }

void HTTPFileSource::Impl::setClientOptions(ClientOptions options) { clientOptions = options; }

ClientOptions HTTPFileSource::Impl::getClientOptions() { return clientOptions.clone(); }
...
NSString *HTTPFileSource::Impl::getUserAgent() const {
    ...
    [userAgentComponents
        addObject:[NSString
                      stringWithFormat:@"%@/%@",
                                       appName.length
                                           ? appName
                                           : appBundle.infoDictionary[@"CFBundleIdentifier"],
  ...
  if (sdkBundle) {
    ...
    if (versionString) {
      [userAgentComponents
          addObject:[NSString stringWithFormat:@"%@/%@", sdkBundle.infoDictionary[@"CFBundleName"],
                                               versionString]];
    }
  }
...
BOOL isValidMapboxEndpoint(NSURL *url) {
  return ([url.host isEqualToString:@"mapbox.com"] || [url.host hasSuffix:@".mapbox.com"] ||
          [url.host isEqualToString:@"mapbox.cn"] || [url.host hasSuffix:@".mapbox.cn"]);
}
...
std::unique_ptr<AsyncRequest> HTTPFileSource::request(const Resource &resource,
  ...
  auto shared = request->shared;  // Explicit copy so that it also gets copied into the completion
                                  // handler block below.
  ...
  @autoreleasepool {
    ...
    if (resource.priorEtag) {
      [req addValue:@(resource.priorEtag->c_str()) forHTTPHeaderField:@"If-None-Match"];
    ...
    if (resource.dataRange) {
      NSString *rangeHeader = [NSString
          stringWithFormat:@"bytes=%lld-%lld", static_cast<long long>(resource.dataRange->first),
        ...
        dataTaskWithRequest:req
          completionHandler:^(NSData *data, NSURLResponse *res, NSError *error) {
            ...
            if ([networkManager.delegate respondsToSelector:@selector(didReceiveResponse:)]) {
              MLNInternalNetworkResponse *networkResponse =
                  [MLNInternalNetworkResponse responseWithData:data urlResponse:res error:error];
            ...
            if (error) {
              [MLNNativeNetworkManager.sharedManager
                  errorLog:@"Requesting: %@ failed with error: %@", req.URL,
              ...
              [MLNNativeNetworkManager.sharedManager
                  debugLog:@"Requesting %@ returned responseCode: %lu", res.URL.relativePath,
                           responseCode];
...
ResourceOptions HTTPFileSource::getResourceOptions() { return impl->getResourceOptions(); }
