...
bool MetalPluginLayerType::requiresPass3D() { return true; }
...
std::vector<std::shared_ptr<mbgl::plugin::LayerProperty>>
MetalPluginLayerType::getLayerProperties() {
  std::vector<std::shared_ptr<mbgl::plugin::LayerProperty>> tempResult;

  {
    std::shared_ptr<mbgl::plugin::LayerProperty> lp =
        std::make_shared<mbgl::plugin::LayerProperty>();
...
#if METAL
  auto metalRenderingContext =
      static_cast<const mbgl::plugin::RenderingContextMetal *>(renderingContext);
...
void MetalPluginLayer::createMetalShaders(const mbgl::plugin::RenderingContext *renderingContext) {
  @autoreleasepool {
    ...
    depthStencilDescriptor.depthCompareFunction =
        MTLCompareFunctionAlways;  // Or another value as needed
    depthStencilDescriptor.depthWriteEnabled = NO;

    _depthStencilStateWithoutStencil =
...
void VulkanPluginLayer::createVulkanShaders(
    const mbgl::plugin::RenderingContext *renderingContext) {
