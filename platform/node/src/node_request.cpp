...
NodeRequest::NodeRequest(std::function<void(mbgl::Response)> callback_, NodeAsyncRequest* asyncRequest_)
...
void NodeRequest::New(const Nan::FunctionCallbackInfo<v8::Value>& info) {
    ...
    auto callback = reinterpret_cast<std::function<void(mbgl::Response)>*>(info[1].As<v8::External>()->Value());
