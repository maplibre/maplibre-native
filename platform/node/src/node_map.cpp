...
void NodeMapObserver::onDidFailLoadingMap(mbgl::MapLoadError error,
...
ParseError)JS")
                                                .ToLocalChecked())
                    .ToLocalChecked();
  parseError.Reset(Nan::To<v8::Object>(Nan::RunScript(script).ToLocalChecked())
                       .ToLocalChecked());
  Nan::Set(target, Nan::New("ParseError").ToLocalChecked(),
           Nan::New(parseError));
...
void NodeMap::New(const Nan::FunctionCallbackInfo<v8::Value> &info) {
  ...
  if (!Nan::Has(options, Nan::New("request").ToLocalChecked()).FromJust() ||
      !Nan::Get(options, Nan::New("request").ToLocalChecked())
           .ToLocalChecked()
           ->IsFunction()) {
  ...
  if (Nan::Has(options, Nan::New("cancel").ToLocalChecked()).FromJust() &&
      !Nan::Get(options, Nan::New("cancel").ToLocalChecked())
           .ToLocalChecked()
           ->IsFunction()) {
  ...
  mbgl::FileSourceManager::get()->registerFileSourceFactory(
      ...
      [](const mbgl::ResourceOptions &resourceOptions,
        ...
        return std::make_unique<node_mbgl::NodeFileSource>(
            reinterpret_cast<node_mbgl::NodeMap *>(
                resourceOptions.platformContext()));
      });
...
void NodeMap::Load(const Nan::FunctionCallbackInfo<v8::Value> &info) {
  ...
  if (info[0]->IsObject()) {
    Nan::JSON JSON;
    style = *Nan::Utf8String(
        JSON.Stringify(info[0]->ToObject(context).ToLocalChecked())
            .ToLocalChecked());
  ...
  if (info.Length() == 2) {
    ...
    if (Nan::Has(options, Nan::New("defaultStyleCamera").ToLocalChecked())
            .FromJust()) {
      if (!Nan::Get(options, Nan::New("defaultStyleCamera").ToLocalChecked())
               .ToLocalChecked()
               ->IsBoolean()) {
        return Nan::ThrowError(
            "Options object 'defaultStyleCamera' property must be a boolean");
      }
      if (Nan::To<bool>(
              Nan::Get(options, Nan::New("cameraMutated").ToLocalChecked())
                  .ToLocalChecked())
              .ToChecked()) {
...
NodeMap::RenderOptions NodeMap::ParseOptions(v8::Local<v8::Object> obj) {
  ...
  if (Nan::Has(obj, Nan::New("zoom").ToLocalChecked()).FromJust()) {
    options.zoom =
        Nan::To<double>(
            Nan::Get(obj, Nan::New("zoom").ToLocalChecked()).ToLocalChecked())
            .ToChecked();
  }

  if (Nan::Has(obj, Nan::New("bearing").ToLocalChecked()).FromJust()) {
    options.bearing =
        Nan::To<double>(Nan::Get(obj, Nan::New("bearing").ToLocalChecked())
                            .ToLocalChecked())
            .ToChecked();
  }
  ...
  if (Nan::Has(obj, Nan::New("pitch").ToLocalChecked()).FromJust()) {
    options.pitch =
        Nan::To<double>(
            Nan::Get(obj, Nan::New("pitch").ToLocalChecked()).ToLocalChecked())
            .ToChecked();
  }

  if (Nan::Has(obj, Nan::New("light").ToLocalChecked()).FromJust()) {
    auto lightObj =
        Nan::Get(obj, Nan::New("light").ToLocalChecked()).ToLocalChecked();
    mbgl::style::conversion::Error conversionError;
    if (auto light = mbgl::style::conversion::convert<mbgl::style::Light>(
            lightObj, conversionError)) {
      options.light = *light;
    } else {
  ...
  if (Nan::Has(obj, Nan::New("axonometric").ToLocalChecked()).FromJust()) {
    options.axonometric =
        Nan::To<bool>(Nan::Get(obj, Nan::New("axonometric").ToLocalChecked())
                          .ToLocalChecked())
            .ToChecked();
  }
  ...
  if (Nan::Has(obj, Nan::New("skew").ToLocalChecked()).FromJust()) {
    auto skewObj =
        Nan::Get(obj, Nan::New("skew").ToLocalChecked()).ToLocalChecked();
    if (skewObj->IsArray()) {
      auto skew = skewObj.As<v8::Array>();
      if (skew->Length() > 0) {
        options.xSkew =
            Nan::To<double>(Nan::Get(skew, 0).ToLocalChecked()).ToChecked();
      }
      if (skew->Length() > 1) {
        options.ySkew =
            Nan::To<double>(Nan::Get(skew, 1).ToLocalChecked()).ToChecked();
      }
    }
  }
  ...
  if (Nan::Has(obj, Nan::New("center").ToLocalChecked()).FromJust()) {
    auto centerObj =
        Nan::Get(obj, Nan::New("center").ToLocalChecked()).ToLocalChecked();
    if (centerObj->IsArray()) {
      auto center = centerObj.As<v8::Array>();
      if (center->Length() > 0) {
        options.longitude =
            Nan::To<double>(Nan::Get(center, 0).ToLocalChecked()).ToChecked();
      }
      if (center->Length() > 1) {
        options.latitude =
            Nan::To<double>(Nan::Get(center, 1).ToLocalChecked()).ToChecked();
      }
    }
  }
  ...
  if (Nan::Has(obj, Nan::New("width").ToLocalChecked()).FromJust()) {
    options.size.width = static_cast<uint32_t>(
        Nan::To<int64_t>(
            Nan::Get(obj, Nan::New("width").ToLocalChecked()).ToLocalChecked())
            .ToChecked());
  }
  ...
  if (Nan::Has(obj, Nan::New("height").ToLocalChecked()).FromJust()) {
    options.size.height = static_cast<uint32_t>(
        Nan::To<int64_t>(
            Nan::Get(obj, Nan::New("height").ToLocalChecked()).ToLocalChecked())
            .ToChecked());
  }

  if (Nan::Has(obj, Nan::New("classes").ToLocalChecked()).FromJust()) {
    auto classes =
        Nan::To<v8::Object>(Nan::Get(obj, Nan::New("classes").ToLocalChecked())
                                .ToLocalChecked())
            .ToLocalChecked()
            .As<v8::Array>();
    ...
    for (int i = 0; i < length; i++) {
      options.classes.emplace_back(std::string{*Nan::Utf8String(
          Nan::To<v8::String>(Nan::Get(classes, i).ToLocalChecked())
              .ToLocalChecked())});
    }
  }
  ...
  if (Nan::Has(obj, Nan::New("debug").ToLocalChecked()).FromJust()) {
    auto debug =
        Nan::To<v8::Object>(
            Nan::Get(obj, Nan::New("debug").ToLocalChecked()).ToLocalChecked())
            .ToLocalChecked();
    if (Nan::Has(debug, Nan::New("tileBorders").ToLocalChecked()).FromJust()) {
      if (Nan::To<bool>(
              Nan::Get(debug, Nan::New("tileBorders").ToLocalChecked())
                  .ToLocalChecked())
              .ToChecked()) {
        options.debugOptions =
            options.debugOptions | mbgl::MapDebugOptions::TileBorders;
      }
    }
    if (Nan::Has(debug, Nan::New("parseStatus").ToLocalChecked()).FromJust()) {
      if (Nan::To<bool>(
              Nan::Get(debug, Nan::New("parseStatus").ToLocalChecked())
                  .ToLocalChecked())
              .ToChecked()) {
        options.debugOptions =
            options.debugOptions | mbgl::MapDebugOptions::ParseStatus;
      }
    }
    if (Nan::Has(debug, Nan::New("timestamps").ToLocalChecked()).FromJust()) {
      if (Nan::To<bool>(Nan::Get(debug, Nan::New("timestamps").ToLocalChecked())
                            .ToLocalChecked())
              .ToChecked()) {
        options.debugOptions =
            options.debugOptions | mbgl::MapDebugOptions::Timestamps;
      }
    }
    if (Nan::Has(debug, Nan::New("collision").ToLocalChecked()).FromJust()) {
      if (Nan::To<bool>(Nan::Get(debug, Nan::New("collision").ToLocalChecked())
                            .ToLocalChecked())
              .ToChecked()) {
        options.debugOptions =
            options.debugOptions | mbgl::MapDebugOptions::Collision;
      }
    }
    if (Nan::Has(debug, Nan::New("overdraw").ToLocalChecked()).FromJust()) {
      if (Nan::To<bool>(Nan::Get(debug, Nan::New("overdraw").ToLocalChecked())
                            .ToLocalChecked())
              .ToChecked()) {
...
void NodeMap::Render(const Nan::FunctionCallbackInfo<v8::Value> &info) {
  ...
  if ((info.Length() <= 1 && !info[0]->IsFunction()) ||
      (info.Length() > 1 && !info[1]->IsFunction())) {
  ...
  try {
    if (info[0]->IsFunction()) {
      ...
      nodeMap->req = std::make_unique<RenderRequest>(
          Nan::To<v8::Function>(info[0]).ToLocalChecked());

      nodeMap->startRender();
    } else {
      auto options =
          ParseOptions(Nan::To<v8::Object>(info[0]).ToLocalChecked());
      assert(!nodeMap->req);
      assert(!nodeMap->image.data);
      nodeMap->req = std::make_unique<RenderRequest>(
...
void NodeMap::startRender() {
  ...
  // Similarly, we're now waiting for the async to be called, so we need to make
  // sure that it keeps the loop alive.
...
void NodeMap::startRender(const NodeMap::RenderOptions &options) {
  ...
  map->renderStill(camera, options.debugOptions,
...
void NodeMap::renderFinished() {
  ...
  // We're done with this render call, so we're unrefing so that the loop could
  // close.
  uv_unref(reinterpret_cast<uv_handle_t *>(async));

  // Move the callback and image out of the way so that the callback can start a
  // new render call.
  ...
  // There is no render pending anymore, we the GC could now delete this object
...
void NodeMap::release() {
  if (!map)
    throw mbgl::util::Exception(releasedMessage());
...
void NodeMap::Cancel(const Nan::FunctionCallbackInfo<v8::Value> &info) {
  ...
  if (!nodeMap->map)
    return Nan::ThrowError(releasedMessage());
  if (!nodeMap->req)
    return Nan::ThrowError("No render in progress");
...
void NodeMap::cancel() {
  ...
  // Remove the existing async handle to flush any scheduled calls to
  // renderFinished.
  uv_unref(reinterpret_cast<uv_handle_t *>(async));
  uv_close(reinterpret_cast<uv_handle_t *>(async),
           [](uv_handle_t *h) { delete reinterpret_cast<uv_async_t *>(h); });
  ...
  frontend = std::make_unique<mbgl::HeadlessFrontend>(mbgl::Size{512, 512},
                                                      pixelRatio);
  map = std::make_unique<mbgl::Map>(
      *frontend, mapObserver,
      mbgl::MapOptions()
          .withSize(frontend->getSize())
          .withPixelRatio(pixelRatio)
          .withMapMode(mode)
          .withCrossSourceCollisions(crossSourceCollisions),
      mbgl::ResourceOptions().withPlatformContext(
...
void NodeMap::AddImage(const Nan::FunctionCallbackInfo<v8::Value> &info) {
  ...
  if (!Nan::Get(optionObject, Nan::New("height").ToLocalChecked())
           .ToLocalChecked()
           ->IsUint32()) {
    return Nan::ThrowTypeError("height parameter required");
  }

  if (!Nan::Get(optionObject, Nan::New("width").ToLocalChecked())
           .ToLocalChecked()
           ->IsUint32()) {
    return Nan::ThrowTypeError("width parameter required");
  }

  if (!Nan::Get(optionObject, Nan::New("pixelRatio").ToLocalChecked())
           .ToLocalChecked()
           ->IsNumber()) {
  ...
  uint32_t imageHeight =
      Nan::To<uint32_t>(
          Nan::Get(optionObject, Nan::New("height").ToLocalChecked())
              .ToLocalChecked())
          .ToChecked();
  uint32_t imageWidth =
      Nan::To<uint32_t>(
          Nan::Get(optionObject, Nan::New("width").ToLocalChecked())
              .ToLocalChecked())
          .ToChecked();
  ...
  bool sdf = false;
  if (Nan::Get(optionObject, Nan::New("sdf").ToLocalChecked())
          .ToLocalChecked()
          ->IsBoolean()) {
    sdf = Nan::To<bool>(Nan::Get(optionObject, Nan::New("sdf").ToLocalChecked())
                            .ToLocalChecked())
              .ToChecked();
  }
  ...
  float pixelRatio =
      (float)Nan::To<double>(
          Nan::Get(optionObject, Nan::New("pixelRatio").ToLocalChecked())
              .ToLocalChecked())
          .ToChecked();
  auto imageBuffer = Nan::To<v8::Object>(info[1])
                         .ToLocalChecked()
                         ->ToObject(context)
                         .ToLocalChecked();
  ...
  mbgl::PremultipliedImage cPremultipliedImage =
      mbgl::util::premultiply(std::move(cImage));
  nodeMap->map->getStyle().addImage(std::make_unique<mbgl::style::Image>(
      *Nan::Utf8String(info[0]), std::move(cPremultipliedImage), pixelRatio,
      sdf));
}
...
void NodeMap::SetLayerZoomRange(
  ...
  mbgl::style::Layer *layer =
      nodeMap->map->getStyle().getLayer(*Nan::Utf8String(info[0]));
  if (!layer) {
...
void NodeMap::SetCenter(const Nan::FunctionCallbackInfo<v8::Value> &info) {
  ...
  try {
    nodeMap->map->jumpTo(
        mbgl::CameraOptions().withCenter(mbgl::LatLng{latitude, longitude}));
  } catch (const std::exception &ex) {
    return Nan::ThrowError(ex.what());
  }
...
void NodeMap::SetZoom(const Nan::FunctionCallbackInfo<v8::Value> &info) {
  ...
  try {
    nodeMap->map->jumpTo(
        mbgl::CameraOptions().withZoom(Nan::To<double>(info[0]).ToChecked()));
  } catch (const std::exception &ex) {
    return Nan::ThrowError(ex.what());
  }
...
void NodeMap::SetBearing(const Nan::FunctionCallbackInfo<v8::Value> &info) {
  ...
  try {
    nodeMap->map->jumpTo(mbgl::CameraOptions().withBearing(
...
void NodeMap::SetPitch(const Nan::FunctionCallbackInfo<v8::Value> &info) {
  ...
  try {
    nodeMap->map->jumpTo(
        mbgl::CameraOptions().withPitch(Nan::To<double>(info[0]).ToChecked()));
  } catch (const std::exception &ex) {
    return Nan::ThrowError(ex.what());
  }
...
void NodeMap::SetAxonometric(const Nan::FunctionCallbackInfo<v8::Value> &info) {
  ...
  try {
    nodeMap->map->setProjectionMode(mbgl::ProjectionMode().withAxonometric(
...
void NodeMap::SetXSkew(const Nan::FunctionCallbackInfo<v8::Value> &info) {
  ...
  try {
    nodeMap->map->setProjectionMode(
        mbgl::ProjectionMode().withXSkew(Nan::To<double>(info[0]).ToChecked()));
  } catch (const std::exception &ex) {
    return Nan::ThrowError(ex.what());
  }
...
void NodeMap::SetYSkew(const Nan::FunctionCallbackInfo<v8::Value> &info) {
  ...
  try {
    nodeMap->map->setProjectionMode(
        mbgl::ProjectionMode().withYSkew(Nan::To<double>(info[0]).ToChecked()));
  } catch (const std::exception &ex) {
    return Nan::ThrowError(ex.what());
  }
...
void NodeMap::SetFeatureState(
  ...
  if (Nan::Has(feature, Nan::New("source").ToLocalChecked()).FromJust()) {
    auto sourceOption =
        Nan::Get(feature, Nan::New("source").ToLocalChecked()).ToLocalChecked();
    if (!sourceOption->IsString()) {
      return Nan::ThrowTypeError(
          "Requires feature.source property to be a string");
    }
    sourceID = *Nan::Utf8String(sourceOption);
  } else {
    return Nan::ThrowTypeError(
        "SetFeatureState: Requires feature.source property");
  }

  if (Nan::Has(feature, Nan::New("sourceLayer").ToLocalChecked()).FromJust()) {
    auto sourceLayerOption =
        Nan::Get(feature, Nan::New("sourceLayer").ToLocalChecked())
            .ToLocalChecked();
    if (!sourceLayerOption->IsString()) {
      return Nan::ThrowTypeError("SetFeatureState: Requires "
                                 "feature.sourceLayer property to be a string");
    }
  ...
  if (Nan::Has(feature, Nan::New("id").ToLocalChecked()).FromJust()) {
    auto idOption =
        Nan::Get(feature, Nan::New("id").ToLocalChecked()).ToLocalChecked();
    if (!idOption->IsString() &&
        !(idOption->IsNumber() || idOption->IsString())) {
      return Nan::ThrowTypeError(
          "Requires feature.id property to be a string or a number");
    }
  ...
  const std::function<std::optional<Error>(const std::string &,
                                           const Convertible &)>
      convertFn = [&](const std::string &k,
                      const Convertible &v) -> std::optional<Error> {
  ...
  try {
    nodeMap->frontend->getRenderer()->setFeatureState(sourceID, sourceLayerID,
                                                      featureID, newState);
  } catch (const std::exception &ex) {
    return Nan::ThrowError(ex.what());
  }
...
void NodeMap::GetFeatureState(
  ...
  if (Nan::Has(feature, Nan::New("source").ToLocalChecked()).FromJust()) {
    ...
    return Nan::ThrowTypeError(
        "GetFeatureState: Requires feature.source property");
  }
  ...
  if (Nan::Has(feature, Nan::New("sourceLayer").ToLocalChecked()).FromJust()) {
    auto sourceLayerOption =
        Nan::Get(feature, Nan::New("sourceLayer").ToLocalChecked())
            .ToLocalChecked();
    if (!sourceLayerOption->IsString()) {
      return Nan::ThrowTypeError("GetFeatureState: Requires "
                                 "feature.sourceLayer property to be a string");
    }
    sourceLayerID = {*Nan::Utf8String(sourceLayerOption)};
  }
  ...
  try {
    nodeMap->frontend->getRenderer()->getFeatureState(state, sourceID,
                                                      sourceLayerID, featureID);
  } catch (const std::exception &ex) {
    return Nan::ThrowError(ex.what());
  }
...
void NodeMap::RemoveFeatureState(
    const Nan::FunctionCallbackInfo<v8::Value> &info) {
  auto nodeMap = Nan::ObjectWrap::Unwrap<NodeMap>(info.Holder());
  if (!nodeMap->map)
    return Nan::ThrowError(releasedMessage());
  ...
  if (Nan::Has(feature, Nan::New("source").ToLocalChecked()).FromJust()) {
    ...
    return Nan::ThrowTypeError(
        "RemoveFeatureState: Requires feature.source property");
  }
  ...
  if (Nan::Has(feature, Nan::New("sourceLayer").ToLocalChecked()).FromJust()) {
    auto sourceLayerOption =
        Nan::Get(feature, Nan::New("sourceLayer").ToLocalChecked())
            .ToLocalChecked();
    if (!sourceLayerOption->IsString()) {
      return Nan::ThrowTypeError("RemoveFeatureState: Requires "
                                 "feature.sourceLayer property to be a string");
    }
    sourceLayerID = {*Nan::Utf8String(sourceLayerOption)};
  }
  ...
  if (info.Length() == 2) {
    ...
    if (!keyParam->IsString()) {
      return Nan::ThrowTypeError(
          "RemoveFeatureState: Requires feature key property to be a string");
    }
    stateKey = {*Nan::Utf8String(keyParam)};
  }

  try {
    nodeMap->frontend->getRenderer()->removeFeatureState(
...
void NodeMap::QueryRenderedFeatures(
    const Nan::FunctionCallbackInfo<v8::Value> &info) {
  using namespace mbgl::style;
  using namespace mbgl::style::conversion;
  ...
  if (!info[1]->IsNull() && !info[1]->IsUndefined()) {
    ...
    if (Nan::Has(options, Nan::New("layers").ToLocalChecked()).FromJust()) {
      auto layersOption = Nan::Get(options, Nan::New("layers").ToLocalChecked())
                              .ToLocalChecked();
      if (!layersOption->IsArray()) {
        return Nan::ThrowTypeError(
            "Requires options.layers property to be an array");
      }
      auto layers = layersOption.As<v8::Array>();
      std::vector<std::string> layersVec;
      for (uint32_t i = 0; i < layers->Length(); i++) {
        layersVec.emplace_back(
            *Nan::Utf8String(Nan::Get(layers, i).ToLocalChecked()));
      }
    ...
    if (Nan::Get(posOrBox, 0).ToLocalChecked()->IsArray()) {
      ...
      optional = nodeMap->frontend->getRenderer()->queryRenderedFeatures(
          mbgl::ScreenBox{
              ...
              {Nan::To<double>(Nan::Get(pos1, 0).ToLocalChecked()).ToChecked(),
               Nan::To<double>(Nan::Get(pos1, 1).ToLocalChecked())
                   .ToChecked()}},
          queryOptions);
      ...
      optional = nodeMap->frontend->getRenderer()->queryRenderedFeatures(
          mbgl::ScreenCoordinate{
              Nan::To<double>(Nan::Get(posOrBox, 0).ToLocalChecked())
                  .ToChecked(),
              Nan::To<double>(Nan::Get(posOrBox, 1).ToLocalChecked())
                  .ToChecked()},
          queryOptions);
    }
    ...
    : pixelRatio([&] {
      ...
      mode([&] {
        ...
        if (std::string(*v8::String::Utf8Value(
                ...
                Nan::Get(options, Nan::New("mode").ToLocalChecked())
                    .ToLocalChecked())) == "tile") {
      ...
      crossSourceCollisions([&] {
        Nan::HandleScope scope;
        return Nan::Has(options,
                        Nan::New("crossSourceCollisions").ToLocalChecked())
                       .FromJust()
                   ? Nan::To<bool>(
                         Nan::Get(
                             options,
...
NodeMap::~NodeMap() {
  try {
    ...
    mbgl::Log::Error(mbgl::Event::General,
                     "Error release the map object when destroying NodeMap");
  }
}

std::unique_ptr<mbgl::AsyncRequest>
NodeFileSource::request(const mbgl::Resource &resource,
                        mbgl::FileSource::Callback callback_) {
  assert(nodeMap);

  Nan::HandleScope scope;
  // Because this method may be called while this NodeMap is already eligible
  // for garbage collection, we need to explicitly hold onto our own handle here
  // so that GC during a v8 call doesn't destroy *this while we're still
  // executing code.
  nodeMap->handle();
  ...
  Nan::NewInstance(Nan::New(node_mbgl::NodeRequest::constructor), 5, argv)
      .ToLocalChecked();

  return asyncRequest;
}

bool NodeFileSource::canRequest(const mbgl::Resource &) const { return true; }
