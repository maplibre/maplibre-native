# We need CMake 3.9 because the Xcode generator doesn't know about systems include paths before 3.9. See
...
function(_node_module_unpack_tar_gz _TYPE _URL _PATH _DEST)
    ...
    execute_process(
        COMMAND ${CMAKE_COMMAND} -E tar xfz "${_TMP}.tar.gz"
...
function(add_node_module NAME)
    ...
    if(NOT EXISTS "${_CACHE_DIR}/node/index.tab")
        _node_module_download("Node.js version list" "https://nodejs.org/dist/index.tab" "${_CACHE_DIR}/node/index.tab")
    endif()
    ...
    foreach(_LINE IN LISTS _INDEX_FILE)
        ...
        if((_ABI GREATER _MINIMUM_NODE_ABI OR _ABI EQUAL _MINIMUM_NODE_ABI)
           AND NOT _ABI IN_LIST _EXCLUDE_NODE_ABIS
           AND NOT DEFINED _NODE_ABI_${_ABI}_VERSION
        )
    ...
    if(_NAN_VERSION AND NOT EXISTS "${_CACHE_DIR}/nan/${_NAN_VERSION}/nan.h")
        _node_module_unpack_tar_gz(
            "Nan ${_NAN_VERSION}" "https://registry.npmjs.org/nan/-/nan-${_NAN_VERSION}.tgz" "package" "${_CACHE_DIR}/nan/${_NAN_VERSION}"
        )
    endif()
    ...
    foreach(_ABI IN LISTS _ABIS)
        ...
        if(NOT EXISTS "${_CACHE_DIR}/node/${_NODE_VERSION}/node.h")
            _node_module_unpack_tar_gz(
                ...
                "node-${_NODE_VERSION}/include/node" "${_CACHE_DIR}/node/${_NODE_VERSION}"
            )
        endif()
        ...
        set_target_properties(
            ...
            PROPERTIES
                ...
                "${_TARGET}"
                SOURCES
                "" # Removes the fake empty.cpp again
                PREFIX
                ""
                SUFFIX
                ".node"
        ...
        target_compile_definitions(
            ${_TARGET} PRIVATE "MODULE_NAME=${NAME_IDENTIFIER}" "BUILDING_NODE_EXTENSION" "_LARGEFILE_SOURCE" "_FILE_OFFSET_BITS=64"
        )

        target_include_directories(${_TARGET} SYSTEM PRIVATE "${_CACHE_DIR}/node/${_NODE_VERSION}")

        if(_NAN_VERSION)
            target_include_directories(${_TARGET} SYSTEM PRIVATE "${_CACHE_DIR}/nan/${_NAN_VERSION}")
        endif()
        ...
        if(APPLE)
            # Ensures that linked symbols are loaded when the module is loaded instead of causing unresolved symbol errors later during
            # runtime.
            set_target_properties(${_TARGET} PROPERTIES LINK_FLAGS "-undefined dynamic_lookup -bind_at_load")
            target_compile_definitions(${_TARGET} PRIVATE "_DARWIN_USE_64_BIT_INODE=1")
        elseif(WIN32)
            # Do nothing here
        else()
            # Ensures that linked symbols are loaded when the module is loaded instead of causing unresolved symbol errors later during
            # runtime.
            set_target_properties(${_TARGET} PROPERTIES LINK_FLAGS "-z now")
        endif()
        ...
        if(WIN32)
            if(MLN_WITH_OSMESA)
                ...
                add_custom_command(
                    TARGET ${_TARGET}
                    POST_BUILD
                    COMMAND ${CMAKE_COMMAND} -E copy "${PROJECT_SOURCE_DIR}/platform/windows/vendor/mesa3d/${_ARCH}/libglapi.dll"
                            "${_OUTPUT_PATH}"
                    COMMAND ${CMAKE_COMMAND} -E copy "${PROJECT_SOURCE_DIR}/platform/windows/vendor/mesa3d/${_ARCH}/libGLESv2.dll"
                            "${_OUTPUT_PATH}"
                    COMMAND ${CMAKE_COMMAND} -E copy "${PROJECT_SOURCE_DIR}/platform/windows/vendor/mesa3d/${_ARCH}/osmesa.dll"
                            "${_OUTPUT_PATH}"
                )
            endif()
