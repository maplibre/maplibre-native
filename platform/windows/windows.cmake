...
execute_process(
    COMMAND powershell -ExecutionPolicy Bypass -File ${CMAKE_CURRENT_LIST_DIR}/Get-VendorPackages.ps1 -Triplet ${VCPKG_TARGET_TRIPLET}
            -Renderer ${_RENDERER}
)
unset(_RENDERER)
...
target_compile_options(mbgl-compiler-options INTERFACE $<$<CXX_COMPILER_ID:MSVC>:/MP>)
...
target_compile_definitions(mbgl-core PRIVATE CURL_STATICLIB USE_STD_FILESYSTEM)

if(MLN_WITH_EGL)
    find_package(unofficial-angle CONFIG REQUIRED)
    target_sources(
        mbgl-core PRIVATE ${PROJECT_SOURCE_DIR}/platform/windows/src/headless_backend_egl.cpp
                          ${PROJECT_SOURCE_DIR}/platform/windows/src/gl_functions.cpp
    )
    target_compile_definitions(mbgl-core PRIVATE KHRONOS_STATIC)
    target_link_libraries(mbgl-core PRIVATE unofficial::angle::libEGL unofficial::angle::libGLESv2)
elseif(MLN_WITH_OSMESA)
    ...
    target_sources(
        mbgl-core PRIVATE ${PROJECT_SOURCE_DIR}/platform/windows/src/headless_backend_osmesa.cpp
                          ${PROJECT_SOURCE_DIR}/platform/windows/src/gl_functions.cpp
    )

    set_property(
        SOURCE ${PROJECT_SOURCE_DIR}/platform/windows/src/headless_backend_osmesa.cpp PROPERTY INCLUDE_DIRECTORIES ${OSMesa_INCLUDE_DIRS}
    )

    target_link_libraries(mbgl-core PRIVATE OSMesa::osmesa OSMesa::libGLESv2)
else()
    find_package(OpenGL REQUIRED)
    target_sources(mbgl-core PRIVATE ${PROJECT_SOURCE_DIR}/platform/windows/src/headless_backend_wgl.cpp)
    target_compile_definitions(mbgl-core PRIVATE KHRONOS_STATIC)
    target_link_libraries(mbgl-core PRIVATE OpenGL::GL)
endif()
...
target_include_directories(
    ...
    PRIVATE ${PROJECT_SOURCE_DIR}/platform/windows/include ${CURL_INCLUDE_DIRS} ${DLFCN_INCLUDE_DIRS} ${JPEG_INCLUDE_DIRS}
            ${LIBUV_INCLUDE_DIRS} ${WEBP_INCLUDE_DIRS}
)
...
target_compile_definitions(mbgl-test-runner PRIVATE MBGL_BUILDING_LIB WORK_DIRECTORY=${PROJECT_SOURCE_DIR})

target_include_directories(mbgl-test-runner PRIVATE ${PROJECT_SOURCE_DIR}/platform/windows/include)
...
target_link_libraries(
    mbgl-benchmark-runner PRIVATE mbgl-compiler-options mbgl-benchmark -WHOLEARCHIVE:mbgl-benchmark
                                  $<IF:$<TARGET_EXISTS:libuv::uv_a>,libuv::uv_a,libuv::uv> shlwapi
)

add_executable(mbgl-render-test-runner ${PROJECT_SOURCE_DIR}/platform/default/src/mbgl/render-test/main.cpp)

target_compile_definitions(mbgl-render-test-runner PRIVATE MBGL_BUILDING_LIB)

target_link_libraries(
    mbgl-render-test-runner PRIVATE mbgl-compiler-options mbgl-render-test $<IF:$<TARGET_EXISTS:libuv::uv_a>,libuv::uv_a,libuv::uv>
)
