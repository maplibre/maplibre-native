...
JavaContainer::JavaContainer(JNIEnv* const env,
...
JavaHashMapAdder::JavaHashMapAdder(JNIEnv* const env) noexcept
    : m_java_container(env, "java/util/HashMap"),
      m_put_method_id(env->GetMethodID(
          m_java_container.container_class.get(), "put", "(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;")) {}
...
namespace {
std::pair<JniReference<jclass>, JniReference<jobject>> make_enum_set(JNIEnv* const env,
    ...
    auto enum_set = make_local_ref(
        env, env->CallStaticObjectMethod(enum_set_class.get(), none_of_method_id, element_class.get()));

    return {std::move(enum_set_class), std::move(enum_set)};
}
} // namespace

JavaEnumSetAdder::JavaEnumSetAdder(JNIEnv* const env, const JniReference<jclass>& element_class) noexcept
