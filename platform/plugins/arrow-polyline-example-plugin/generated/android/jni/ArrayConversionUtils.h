...
std::unordered_map<K, V, Hash> convert_from_jni(JNIEnv* const env,
                                                const JniReference<jobject>& java_map,
                                                TypeId<std::unordered_map<K, V, Hash>>);

template <typename K, typename V, typename Hash>
std::optional<std::unordered_map<K, V, Hash>> convert_from_jni(JNIEnv* const env,
                                                               const JniReference<jobject>& _jMap,
                                                               TypeId<std::optional<std::unordered_map<K, V, Hash>>>);

template <typename K, typename V, typename Hash>
JniReference<jobject> convert_to_jni(JNIEnv* const env, const std::unordered_map<K, V, Hash>& input);

template <typename K, typename V, typename Hash>
JniReference<jobject> convert_to_jni(JNIEnv* const env, const std::optional<std::unordered_map<K, V, Hash>>& input);
...
JniReference<jobject> convert_to_jni(JNIEnv* const env, const std::unordered_set<T, Hash>& input);

template <typename T, typename Hash, typename std::enable_if<std::is_enum<T>::value, int>::type = 0>
JniReference<jobject> convert_to_jni(JNIEnv* const env, const std::unordered_set<T, Hash>& input);

template <typename T, typename Hash>
JniReference<jobject> convert_to_jni(JNIEnv* const env, const std::optional<std::unordered_set<T, Hash>>& input);

template <typename T, typename Hash>
std::unordered_set<T, Hash> convert_from_jni(JNIEnv* const env,
                                             const JniReference<jobject>& java_set,
                                             TypeId<std::unordered_set<T, Hash>>);

template <typename T, typename Hash>
std::optional<std::unordered_set<T, Hash>> convert_from_jni(JNIEnv* const env,
                                                            const JniReference<jobject>& java_set,
                                                            TypeId<std::optional<std::unordered_set<T, Hash>>>);

template <typename Container>
std::optional<Container> convert_optional_container_from_jni(JNIEnv* const env,
                                                             const JniReference<jobject>& java_container,
                                                             TypeId<std::optional<Container>>);

template <typename Container>
JniReference<jobject> convert_optional_container_to_jni(JNIEnv* const env,
                                                        const std::optional<Container>& optional_container);

// Templated functions to create ArrayLists from C++ vectors and vice versa

template <typename T>
JniReference<jobject> convert_to_jni(JNIEnv* const env, const std::vector<T>& input) {
...
template <typename T>
std::vector<T> convert_from_jni(JNIEnv* const env, const JniReference<jobject>& array_list, TypeId<std::vector<T>>) {
    std::vector<T> result;
...
template <typename T>
JniReference<jobject> convert_to_jni_optimized(JNIEnv* const env,
                                               const std::vector<std::shared_ptr<T>>& input,
                                               const char* class_name) {
    auto vector_ptr = new (std::nothrow)
        std::shared_ptr<std::vector<std::shared_ptr<T>>>(new (std::nothrow) std::vector<std::shared_ptr<T>>(input));
...
template <typename T, typename Hash, typename std::enable_if<std::is_enum<T>::value, int>::type>
JniReference<jobject> convert_to_jni(JNIEnv* env, const std::unordered_set<T, Hash>& input) {
...
template <typename T>
std::optional<std::vector<T>> convert_from_jni(JNIEnv* const env,
...
template <typename T>
JniReference<jobject> convert_to_jni(JNIEnv* const env, const std::optional<std::vector<T>>& input) {
    return convert_optional_container_to_jni(env, input);
}
