...
convert_from_jni(JNIEnv* const env, const JniReference<jobject>& jvalue, TypeId<::glue_internal::Locale>)
{
    if (!jvalue)
    {
        ::glue_internal::jni::throw_new_null_pointer_exception(env);
    ...
    const auto languageCode = call_java_method<jstring>(env, locale_class, jvalue, "getLanguage", "()Ljava/lang/String;");
    ...
    const auto languageTag = call_java_method<jstring>(env, locale_class, jvalue, "toLanguageTag", "()Ljava/lang/String;");
...
convert_from_jni(JNIEnv* const env, const JniReference<jobject>& jvalue, TypeId<std::optional<::glue_internal::Locale>>)
{
    return jvalue
        ? std::optional<::glue_internal::Locale>(
...
{
    const auto& java_duration_class = get_cached_duration_class();
    const auto factory_method_id = env->GetStaticMethodID(java_duration_class.get(), "ofSeconds", "(JJ)Lcom/example/time/Duration;");
    return make_local_ref(env, env->CallStaticObjectMethod(java_duration_class.get(), factory_method_id, seconds, nanos));
}

JniReference<jobject>
convert_to_jni(JNIEnv* const env, const ::glue_internal::Locale& nvalue) noexcept {
    ...
    if (nvalue.language_tag) {
        ...
        call_java_method<jobject>(
            env, locale_builder_class, builder, "setLanguageTag", set_method_signature,
        ...
        call_java_method<jobject>(
            env, locale_builder_class, builder, "setLanguage", set_method_signature,
            convert_to_jni(env, nvalue.language_code));
        if (env->ExceptionOccurred())
            return {};

        call_java_method<jobject>(
            env, locale_builder_class, builder, "setRegion", set_method_signature,
            convert_to_jni(env, nvalue.country_code));
        if (env->ExceptionOccurred())
            return {};

        call_java_method<jobject>(
            env, locale_builder_class, builder, "setScript", set_method_signature,
...
JniReference<jobject>
convert_to_jni(JNIEnv* const env, const std::optional<::glue_internal::Locale>& nvalue) noexcept {
