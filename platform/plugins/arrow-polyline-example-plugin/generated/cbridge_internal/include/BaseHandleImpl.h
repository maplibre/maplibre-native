...
double
duration_to_seconds_double(const std::chrono::duration<Rep, Period>& d) {
    ...
    return static_cast<double>(division_results.quot) +
        static_cast<double>(division_results.rem) / Period::den;
}

template<class Rep, class Period>
std::chrono::duration<Rep, Period>
seconds_double_to_duration(double seconds_double) {
    // Double can represent an integer up to 52 bits without a loss of precision. For big 64-bit
    ...
    auto tick_count =
        static_cast<int64_t>(integralPart) * Period::den +
...
{
    static _baseRef
    toBaseRef( std::optional< T > ptr )
    {
        return !ptr ? 0
                    : reinterpret_cast< _baseRef >(
                          new std::optional< T >( ::std::move( ptr ) ) );
    }

    static _baseRef
    referenceBaseRef( const std::optional< T >& t )
    {
        return reinterpret_cast< _baseRef >( &t );
    }
...
struct Conversion<std::chrono::time_point<Clock, Duration>> {
    ...
    static std::chrono::time_point<Clock, Duration>
    toCppReturn(double seconds_value) {
        using namespace ::std::chrono;
        return time_point<Clock, Duration>(
            seconds_double_to_duration<typename Duration::rep, typename Duration::period>(seconds_value)
        );
...
struct Conversion<std::optional<std::chrono::time_point<Clock, Duration>>> {
    static _baseRef
    toBaseRef(std::optional<std::chrono::time_point<Clock, Duration>> ptr) {
        if (!ptr) return 0;
    ...
    static _baseRef
    referenceBaseRef(const std::optional<std::chrono::time_point<Clock, Duration>>& ptr) {
    ...
    toCpp(_baseRef ref) {
        ...
        return std::optional<std::chrono::time_point<Clock, Duration>>(
...
{
    static double
    toBaseRef(const std::chrono::duration<Rep, Period>& d) {
    ...
    static double
    referenceBaseRef(const std::chrono::duration<Rep, Period>& d) {
        return duration_to_seconds_double(d);
    }

    static std::chrono::duration<Rep, Period>
    toCpp(double seconds_double) {
        return seconds_double_to_duration<Rep, Period>(seconds_double);
    }

    static std::chrono::duration<Rep, Period>
    toCppReturn(double seconds_double) {
        return seconds_double_to_duration<Rep, Period>(seconds_double);
    }
};
...
struct Conversion<std::optional<std::chrono::duration<Rep, Period>>> {
    static _baseRef
    toBaseRef(std::optional<std::chrono::duration<Rep, Period>> ptr) {
        if (!ptr) return 0;
        return reinterpret_cast<_baseRef>(
            new std::optional<double>(duration_to_seconds_double(*ptr))
        );
    }

    static _baseRef
    referenceBaseRef(const std::optional<std::chrono::duration<Rep, Period>>& ptr) {
        return toBaseRef(ptr);
    }
