...
internal func copyToCType(_ swiftType: LatLng) -> RefHolder {
...
internal func moveToCType(_ swiftType: LatLng) -> RefHolder {
    return RefHolder(ref: copyToCType(swiftType).ref, release: GluecodiumPlugin_LatLng_release_handle)
}
internal func copyFromCType(_ handle: _baseRef) -> LatLng? {
    guard handle != 0 else {
        return nil
...
internal func moveToCType(_ swiftType: LatLng?) -> RefHolder {
    return RefHolder(ref: copyToCType(swiftType).ref, release: GluecodiumPlugin_LatLng_release_optional_handle)
}
