...
- (nullable instancetype)initWithMLNStyleImage:(const mbgl::style::Image &)styleImage {
  ...
  if (self = [self initWithSize:size]) {
    ...
    if (auto content = styleImage.getContent()) {
      self.capInsets = NSEdgeInsetsMake(content->top / scale, content->left / scale,
...
- (std::unique_ptr<mbgl::style::Image>)mgl_styleImageWithIdentifier:(NSString *)identifier {
  ...
  if (self.resizingMode == NSImageResizingModeStretch) {
    stretchX.push_back({
        ...
        (self.size.width - self.capInsets.right) * scale,
    });
    stretchY.push_back({
        self.capInsets.top * scale,
        (self.size.height - self.capInsets.bottom) * scale,
    });
  }
