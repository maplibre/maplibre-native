...
TEST(Transform, IsPanning) {
    ...
    AnimationOptions easeOptions(Seconds(1));
    easeOptions.transitionFrameFn = [&transform](double) { ASSERT_TRUE(transform.getState().isPanning()); };
...
TEST(Transform, LatLngBounds) {
    ...
    easeOptions.transitionFinishFn = [&]() { ASSERT_NEAR(transform.getLatLng().longitude(), 120.0, 1e-4); };
