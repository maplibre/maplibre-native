...
TEST(Map, TEST_REQUIRES_SERVER(StyleNotFound)) {
    ...
    test.observer.didFinishLoadingStyleCallback = [&]() { FAIL() << "Should not retry on not found!"; };
...
TEST(Map, DisabledSources) {
    ...
    test.fileSource->response = [](const Resource& res) -> std::optional<Response> {
        if (res.url == "asset://tile.png") {
            Response response;
            response.data =
                std::make_shared<std::string>(util::read_file("test/fixtures/map/disabled_layers/tile.png"));
...
TEST(Map, ObserveShaderRegistration) {
    ...
    test.observer.onRegisterShadersCallback = [&observedRegistry](gfx::ShaderRegistry&) { observedRegistry = true; };
