...
public:
  ...
  MapTest(float pixelRatio = 1, MapMode mode = MapMode::Static)
      : fileSource(std::make_shared<FileSource>(ResourceOptions::Default(),
                                                ClientOptions())),
        frontend(pixelRatio), map(frontend, observer, fileSource,
                                  MapOptions()
                                      .withMapMode(mode)
  ...
  explicit MapTest(MapOptions options)
      : fileSource(std::make_shared<FileSource>()),
        ...
        map(frontend, observer, fileSource,
            options.withSize(frontend.getSize())) {}

  template <typename T = FileSource>
  MapTest(const std::string &cachePath, const std::string &assetPath,
          float pixelRatio = 1, MapMode mode = MapMode::Static,
          typename std::enable_if_t<std::is_same_v<T, MainResourceLoader>> * =
              nullptr)
      : fileSource(std::make_shared<T>(
            ResourceOptions().withCachePath(cachePath).withAssetPath(assetPath),
            ClientOptions())),
        frontend(pixelRatio), map(frontend, observer, fileSource,
                                  MapOptions()
                                      .withMapMode(mode)
...
TEST(Map, RendererState) {
  ...
  test.map.jumpTo(CameraOptions()
                      .withCenter(coordinate)
                      .withZoom(zoom)
                      .withPitch(pitchInDegrees)
                      .withBearing(bearingInDegrees));
  ...
  test.map.getStyle().addImage(std::make_unique<style::Image>(
      "default_marker",
      decodeImage(util::read_file("test/fixtures/sprites/default_marker.png")),
      1.0f));
  ...
  test.map.getStyle().addLayer(
      std::make_unique<SymbolLayer>("SymbolLayer", "GeoJSONSource"));
...
TEST(Map, LatLngBoundsToCamera) {
  ...
  test.map.jumpTo(
      CameraOptions().withCenter(LatLng{40.712730, -74.005953}).withZoom(16.0));

  LatLngBounds bounds =
      LatLngBounds::hull({15.68169, 73.499857}, {53.560711, 134.77281});
...
TEST(Map, LatLngBoundsToCameraWithExcessivePadding) {
  ...
  CameraOptions virtualCamera =
      test.map.cameraForLatLngBounds(bounds, {500, 0, 1200, 0});
  ASSERT_TRUE(bounds.contains(*virtualCamera.center));
...
TEST(Map, LatLngBoundsToCameraWithBearing) {
  ...
  CameraOptions virtualCamera =
      test.map.cameraForLatLngBounds(bounds, {}, 35.0);
  ASSERT_TRUE(bounds.contains(*virtualCamera.center));
...
TEST(Map, LatLngBoundsToCameraWithBearingPitchAndPadding) {
  ...
  CameraOptions virtualCamera =
      test.map.cameraForLatLngBounds(bounds, {}, 35, 20);
  ASSERT_TRUE(bounds.contains(*virtualCamera.center));
  ...
  const CameraOptions virtualCameraPadded =
      test.map.cameraForLatLngBounds(bounds, padding, 35, 20);
  ASSERT_TRUE(bounds.contains(*virtualCameraPadded.center));
  ASSERT_DOUBLE_EQ(virtualCameraPadded.center->latitude(),
                   virtualCamera.center->latitude());
  ASSERT_DOUBLE_EQ(virtualCameraPadded.center->longitude(),
                   virtualCamera.center->longitude());

  const Size size = test.map.getMapOptions().size();
  const auto scaleChange =
      std::min((size.width - padding.left() - padding.right()) / size.width,
               (size.height - padding.top() - padding.bottom()) / size.height);
  ASSERT_DOUBLE_EQ(*virtualCameraPadded.zoom,
                   *virtualCamera.zoom + util::log2(scaleChange));
...
TEST(Map, LatLngsToCamera) {
  ...
  std::vector<LatLng> latLngs{
      {40.712730, 74.005953}, {15.68169, 73.499857}, {30.82678, 83.4082}};
...
TEST(Map, CameraToLatLngBounds) {
  ...
  ASSERT_NEAR(camera.center->latitude(), virtualCamera.center->latitude(),
              1e-7);
  ASSERT_NEAR(camera.center->longitude(), virtualCamera.center->longitude(),
              1e-7);
}
...
TEST(Map, CameraToLatLngBoundsUnwrappedWithRotation) {
  ...
  ASSERT_TRUE(test.map.latLngBoundsForCameraUnwrapped(camera).contains(
      test.map.latLngForPixel({})));
  ASSERT_TRUE(test.map.latLngBoundsForCameraUnwrapped(camera).contains(
      test.map.latLngForPixel({0.0, double(size.height)})));
  ASSERT_TRUE(test.map.latLngBoundsForCameraUnwrapped(camera).contains(
      test.map.latLngForPixel({double(size.width), 0.0})));
  ASSERT_TRUE(test.map.latLngBoundsForCameraUnwrapped(camera).contains(
      test.map.latLngForPixel({double(size.width), double(size.height)})));
  ASSERT_TRUE(test.map.latLngBoundsForCameraUnwrapped(camera).contains(
      test.map.latLngForPixel(
          {double(size.width) / 2, double(size.height) / 2})));
}
...
TEST(Map, CameraToLatLngBoundsUnwrappedCrossDateLine) {
  ...
  ASSERT_TRUE(test.map.latLngBoundsForCameraUnwrapped(camera).contains(
      test.map.latLngForPixel({}), LatLng::Wrapped));
  ASSERT_TRUE(test.map.latLngBoundsForCameraUnwrapped(camera).contains(
  ...
  ASSERT_TRUE(
      test.map.latLngBoundsForCameraUnwrapped(camera).crossesAntimeridian());
}
...
TEST(Map, Offline) {
  ...
  auto expiredItem = [](const std::string &path) {
    ...
    response.data = std::make_shared<std::string>(
        util::read_file("test/fixtures/map/offline/"s + path));
  ...
  dbfs->forward(Resource::style(prefix + "style.json"),
                expiredItem("style.json"), [] {});
  dbfs->forward(Resource::source(prefix + "streets.json"),
                expiredItem("streets.json"), [] {});
  dbfs->forward(Resource::spriteJSON(prefix + "sprite", 1.0),
                expiredItem("sprite.json"), [] {});
  dbfs->forward(Resource::spriteImage(prefix + "sprite", 1.0),
                expiredItem("sprite.png"), [] {});
  dbfs->forward(Resource::tile(prefix + "{z}-{x}-{y}.vector.pbf", 1.0, 0, 0, 0,
                               Tileset::Scheme::XYZ),
                expiredItem("0-0-0.vector.pbf"), [] {});
  dbfs->forward(Resource::glyphs(prefix + "{fontstack}/{range}.pbf",
                                 {{"Helvetica"}}, {0, 255}),
  ...
  test::checkImage("test/fixtures/map/offline",
                   test.frontend.render(test.map).image, 0.0046, 0.1);
#else
  test::checkImage("test/fixtures/map/offline",
                   test.frontend.render(test.map).image, 0.0015, 0.1);
#endif
...
TEST(Map, SetStyleDefaultCamera) {
  ...
  test.map.getStyle().loadJSON(
      util::read_file("test/fixtures/api/empty-zoomed.json"));
...
TEST(Map, ProjectionMode) {
  ...
  test.map.setProjectionMode(
      ProjectionMode().withAxonometric(true).withXSkew(1.0).withYSkew(0.0));
...
TEST(Map, BoundOptions) {
  ...
  test.map.setBounds(
      BoundOptions().withMinZoom(4).withMaxZoom(10).withLatLngBounds(llb));
...
TEST(Map, SetStyleInvalidJSON) {
  ...
  {
    MapTest<> test;
    test.observer.didFailLoadingMapCallback = [&]() { fail = true; };
  ...
  EXPECT_EQ(1u,
            flo->count({EventSeverity::Error, Event::ParseStyle, -1,
...
TEST(Map, SetStyleInvalidURL) {
  ...
  test.fileSource->styleResponse = [](const Resource &) {
    ...
    response.error = std::make_unique<Response::Error>(
        Response::Error::Reason::Other, "Failed by the test case");
    return response;
  };

  test.observer.didFailLoadingMapCallback = [&]() { test.runLoop.stop(); };
...
TEST(Map, StyleFresh) {
  ...
  Response response;
  response.data = std::make_shared<std::string>(
      util::read_file("test/fixtures/api/empty.json"));
...
TEST(Map, StyleExpired) {
  // The map should allow expired styles to be revalidated until we get a fresh
  // style.
  ...
  // Mutate layer. From now on, sending a response to the style won't overwrite
  // it anymore, but we should continue to wait for a fresh response.
  ...
  // Send a fresh response, and confirm that we didn't overwrite the style, but
  // continue to wait for a fresh response.
  response.expires = util::now() + 1h;
  test.fileSource->respond(Resource::Style, response);
  EXPECT_EQ(1u, test.fileSource->requests.size());
  EXPECT_NE(nullptr, test.map.getStyle().getLayer("bg"));
...
TEST(Map, StyleExpiredWithAnnotations) {
  ...
  test.map.addAnnotation(
      LineAnnotation{LineString<double>{{{0, 0}, {10, 10}}}});
  EXPECT_EQ(1u, test.fileSource->requests.size());
...
TEST(Map, StyleEarlyMutation) {
  ...
  Response response;
  response.data = std::make_shared<std::string>(
      util::read_file("test/fixtures/api/water.json"));
  test.fileSource->respond(Resource::Style, response);
...
TEST(Map, MapLoadingSignal) {
  ...
  bool emitted = false;
  test.observer.willStartLoadingMapCallback = [&]() { emitted = true; };
  test.map.getStyle().loadJSON(util::read_file("test/fixtures/api/empty.json"));
  EXPECT_TRUE(emitted);
}
...
TEST(Map, StyleLoadedSignal) {
  ...
  bool emitted = false;
  test.observer.didFinishLoadingStyleCallback = [&]() { emitted = true; };
  test.map.getStyle().loadJSON(util::read_file("test/fixtures/api/empty.json"));
  EXPECT_TRUE(emitted);
...
TEST(Map, TEST_REQUIRES_SERVER(StyleNetworkErrorRetry)) {
  ...
  test.observer.didFinishLoadingStyleCallback = [&]() { test.runLoop.stop(); };
...
TEST(Map, TEST_REQUIRES_SERVER(StyleNotFound)) {
  ...
  test.map.getStyle().loadURL(
      "http://127.0.0.1:3000/style-fail-once-404-cache");
  test.runLoop.run();
}
...
TEST(Map, AddLayer) {
  ...
  test::checkImage("test/fixtures/map/add_layer",
...
TEST(Map, WithoutVAOExtension) {
  ...
  static_cast<gl::Context &>(test.frontend.getBackend()->getContext())
      .disableVAOExtension = true;

  test.map.getStyle().loadJSON(util::read_file("test/fixtures/api/water.json"));

  test::checkImage("test/fixtures/map/no_vao",
                   test.frontend.render(test.map).image, 0.002);
}
...
TEST(Map, RemoveLayer) {
  ...
  test::checkImage("test/fixtures/map/remove_layer",
                   test.frontend.render(test.map).image);
}
...
TEST(Map, DisabledSources) {
  ...
  test.fileSource->response =
      [](const Resource &res) -> std::optional<Response> {
  ...
  // This stylesheet has two raster layers, one that starts at zoom 1, the other
  // at zoom 0. We first render a map at zoom level 1, which should show both
  // layers (both are "visible" due to an opacity of 0.5). Then, we are zooming
  // back out to a zoom level of 0.5 and rerender. The "raster1" layer should
  // not be visible anymore since it has minzoom 1, while "raster2" should still
  // be there. Both layers have a distinct color through "raster-hue-rotate".
  test.map.getStyle().loadJSON(R"STYLE(
{
  ...
  test::checkImage("test/fixtures/map/disabled_layers/first",
                   test.frontend.render(test.map).image);
  test.map.jumpTo(CameraOptions().withZoom(0.5));
  test::checkImage("test/fixtures/map/disabled_layers/second",
                   test.frontend.render(test.map).image);
}
...
TEST(Map, DontLoadUnneededTiles) {
  ...
  for (unsigned zoom = 0; zoom <= 30; zoom++) { // times 10
    // Note: using z += 0.1 in the loop doesn't produce accurate floating point
    // numbers.
...
TEST(Map, TEST_DISABLED_ON_CI(ContinuousRendering)) {
  ...
  StubMapObserver observer;
  observer.didFinishRenderingFrameCallback =
...
TEST(Map, NoContentTiles) {
  ...
  std::shared_ptr<FileSource> dbfs = FileSourceManager::get()->getFileSource(
      FileSourceType::Database, ResourceOptions{});
  dbfs->forward(Resource::tile("http://example.com/{z}-{x}-{y}.vector.pbf", 1.0,
...
TEST(Map, Issue12432) {
  ...
  test.fileSource->tileResponse = [&](const Resource &) {
    ...
    result.data = std::make_shared<std::string>(
        util::read_file("test/fixtures/map/issue12432/0-0-0.mvt"));
    return result;
  };
...
TEST(Map, Issue15216) {
  ...
  test.map.getStyle().addSource(
      std::make_unique<ImageSource>("ImageSource", std::array<LatLng, 4>()));
  test.map.getStyle().addLayer(
      std::make_unique<RasterLayer>("RasterLayer", "ImageSource"));
...
TEST(Map, Issue15342) {
  ...
  test.observer.didFinishLoadingMapCallback = [&]() {
    ...
    test.observer.didFinishRenderingFrameCallback =
        [&](MapObserver::RenderFrameStatus status) {
...
TEST(Map, UniversalStyleGetter) {
  ...
  StyleProperty lineOpacityTransition =
      lineLayer->getProperty("line-opacity-transition");
  ...
  StyleProperty lineColorTransition =
      lineLayer->getProperty("line-color-transition");
...
TEST(Map, PrefetchDeltaOverride) {
  ...
  // Vector source
  auto vectorSource =
      std::make_unique<VectorSource>("vector", Tileset{{"a/{z}/{x}/{y}"}});
  ...
  options.fetchTileFunction = [&requestedTiles,
                               &test](const CanonicalTileID &tileID) {
    ++requestedTiles;
    auto *customSrc = static_cast<CustomGeometrySource *>(
        test.map.getStyle().getSource("custom"));
    if (customSrc) {
      customSrc->setTileData(tileID, {});
    }
  };
  auto customSource =
      std::make_unique<CustomGeometrySource>("custom", std::move(options));
...
TEST(Map, PrefetchDeltaOverrideCustomSource) {
  ...
  auto makeCustomSource = [&requestedTiles, &test] {
    ...
    options.fetchTileFunction = [&requestedTiles,
                                 &test](const CanonicalTileID &tileID) {
      ++requestedTiles;
      auto *source = static_cast<CustomGeometrySource *>(
          test.map.getStyle().getSource("custom"));
      if (source) {
...
TEST(Map, TEST_REQUIRES_SERVER(ExpiredSpriteSheet)) {
  ...
  MapTest<MainResourceLoader, ForwardingHeadlessFrontend> test{
      ":memory:", ".", 1, MapMode::Continuous};

  auto makeResponse = [](const std::string &path, bool expires = false) {
    Response response;
    response.data = std::make_shared<std::string>(
        util::read_file("test/fixtures/map/online/"s + path));
  ...
  test.observer.didFinishLoadingMapCallback = [&test] {
    test.frontend.checkParams = [](std::shared_ptr<UpdateParameters> params) {
      EXPECT_TRUE(
          std::is_sorted(params->images->begin(), params->images->end()));
  ...
  std::shared_ptr<FileSource> dbfs = FileSourceManager::get()->getFileSource(
      FileSourceType::Database, ResourceOptions{});
  dbfs->forward(Resource::style(prefix + "style.json"),
                makeResponse("style.json"), [] {});
  dbfs->forward(Resource::source(prefix + "streets.json"),
                makeResponse("streets.json"), [] {});
  dbfs->forward(Resource::spriteJSON(prefix + "sprite", 1.0),
                makeResponse("sprite.json", true), [] {});
  dbfs->forward(Resource::spriteImage(prefix + "sprite", 1.0),
                makeResponse("sprite.png", true), [] {});
  dbfs->forward(Resource::tile(prefix + "{z}-{x}-{y}.vector.pbf", 1.0, 0, 0, 0,
                               Tileset::Scheme::XYZ),
...
TEST(Map, SourceMinimumUpdateIntervalOverride) {
  ...
  // Vector source
  auto vectorSourceA =
      std::make_unique<VectorSource>("source-a", Tileset{{"a/{z}/{x}/{y}"}});
  vectorSourceA->setMinimumTileUpdateInterval(Seconds(1));
  test.map.getStyle().addSource(std::move(vectorSourceA));

  auto vectorSourceB =
      std::make_unique<VectorSource>("source-b", Tileset{{"b/{z}/{x}/{y}"}});
...
auto makeResponse(const std::string &file, bool incrementCounter = false) {
  return [file, incrementCounter](const Resource &) {
    ...
    Response result;
    result.data = std::make_shared<std::string>(
        util::read_file("test/fixtures/resources/" + file));
    return result;
  };
}
...
bool isInsideTile(const mapbox::geometry::box<float> &box, float padding,
                  Size viewportSize) {
  if (box.min.x - padding < 0)
    return false;
  if (box.min.y - padding < 0)
    return false;
  if (box.max.x - padding > viewportSize.width)
    return false;
  if (box.max.y - padding > viewportSize.height)
    return false;
  return true;
}
...
TEST(Map, DISABLED_PlacedSymbolData) {
  ...
  test.map.jumpTo(
      CameraOptions().withZoom(16).withCenter(LatLng{50.072317, 14.444827}));
  ...
  const auto &placedSymbols =
      test.frontend.getRenderer()->getPlacedSymbolsData();
  ...
  for (const auto &placedSymbol : placedSymbols) {
    ...
    if (placedSymbol.textPlaced && placedSymbol.textCollisionBox) {
      if (isInsideTile(*placedSymbol.textCollisionBox,
                       placedSymbol.viewportPadding, viewportSize)) {
...
TEST(Map, VolatileSource) {
  ...
  test.fileSource->tileResponse = [&](const Resource &resource) {
    auto expectedPolicy = isVolatile ? Resource::StoragePolicy::Volatile
