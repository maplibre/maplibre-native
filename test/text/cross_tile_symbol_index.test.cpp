...
TEST(CrossTileSymbolLayerIndex, addBucket) {
    ...
    // Assigned new IDs
    ASSERT_EQ(mainBucket.symbolInstances.at(0).getCrossTileID(__SOURCE_LOCATION__), 1u);
    ASSERT_EQ(mainBucket.symbolInstances.at(1).getCrossTileID(__SOURCE_LOCATION__), 2u);
    ...
    // matched parent tile
    ASSERT_EQ(childBucket.symbolInstances.at(0).getCrossTileID(__SOURCE_LOCATION__), 1u);
    // does not match because of different key
    ASSERT_EQ(childBucket.symbolInstances.at(1).getCrossTileID(__SOURCE_LOCATION__), 3u);
    // does not match because of different location
    ASSERT_EQ(childBucket.symbolInstances.at(2).getCrossTileID(__SOURCE_LOCATION__), 4u);
    // matches with a slightly different location
    ASSERT_EQ(childBucket.symbolInstances.at(3).getCrossTileID(__SOURCE_LOCATION__), 2u);
    ...
    // matched child tile
    ASSERT_EQ(parentBucket.symbolInstances.at(0).getCrossTileID(__SOURCE_LOCATION__), 1u);
    ...
    ASSERT_EQ(grandchildBucket.symbolInstances.at(0).getCrossTileID(__SOURCE_LOCATION__), 1u);
    // Does not match the previous value for Windsor because that tile was removed
    ASSERT_EQ(grandchildBucket.symbolInstances.at(1).getCrossTileID(__SOURCE_LOCATION__), 5u);
}
...
TEST(CrossTileSymbolLayerIndex, resetIDs) {
    ...
    // assigns a new id
    index.addBucket(childID, mat4{}, childBucket);
    ASSERT_EQ(childBucket.symbolInstances.at(0).getCrossTileID(__SOURCE_LOCATION__), 2u);

    // overwrites the old id to match the already-added tile
    index.addBucket(mainID, mat4{}, mainBucket);
    ASSERT_EQ(mainBucket.symbolInstances.at(0).getCrossTileID(__SOURCE_LOCATION__), 2u);
}
...
TEST(CrossTileSymbolLayerIndex, noDuplicatesWithinZoomLevel) {
    ...
    index.addBucket(childID, mat4{}, childBucket);
    ASSERT_EQ(childBucket.symbolInstances.at(0).getCrossTileID(__SOURCE_LOCATION__),
              1u); // A' copies from A
    ASSERT_EQ(childBucket.symbolInstances.at(1).getCrossTileID(__SOURCE_LOCATION__),
              2u); // B' copies from B
    ASSERT_EQ(childBucket.symbolInstances.at(2).getCrossTileID(__SOURCE_LOCATION__),
              3u); // C' gets new ID
}
...
TEST(CrossTileSymbolLayerIndex, bucketReplacement) {
    ...
    ASSERT_EQ(firstBucket.symbolInstances.at(0).getCrossTileID(__SOURCE_LOCATION__), 1u);
    ASSERT_EQ(firstBucket.symbolInstances.at(1).getCrossTileID(__SOURCE_LOCATION__), 2u);

    // copies parent ids without duplicate ids in this tile
    index.addBucket(tileID, mat4{}, secondBucket);
    ASSERT_EQ(secondBucket.symbolInstances.at(0).getCrossTileID(__SOURCE_LOCATION__),
              1u); // A' copies from A
    ASSERT_EQ(secondBucket.symbolInstances.at(1).getCrossTileID(__SOURCE_LOCATION__),
              2u); // B' copies from B
    ASSERT_EQ(secondBucket.symbolInstances.at(2).getCrossTileID(__SOURCE_LOCATION__),
              3u); // C' gets new ID
}
...
TEST(CrossTileSymbolLayerIndex, offscreenSymbols) {
    ...
    EXPECT_EQ(symbolBucket.symbolInstances.at(0).getCrossTileID(__SOURCE_LOCATION__),
              SymbolInstance::invalidCrossTileID);
    EXPECT_EQ(symbolBucket.symbolInstances.at(1).getCrossTileID(__SOURCE_LOCATION__),
              SymbolInstance::invalidCrossTileID);

    populatePosMatrix(posMatrix, tileId, 39.0, -76.0, 7.0);
    index.addBucket(tileId, posMatrix, symbolBucket);

    EXPECT_EQ(symbolBucket.symbolInstances.at(0).getCrossTileID(__SOURCE_LOCATION__), 1u);
    EXPECT_EQ(symbolBucket.symbolInstances.at(1).getCrossTileID(__SOURCE_LOCATION__), 2u);
}
