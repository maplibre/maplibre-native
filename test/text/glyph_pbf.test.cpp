...
TEST(GlyphPBF, Parsing) {
    ...
    auto sdfs = parseGlyphPBF(GlyphRange{0, 255, GlyphIDType::FontPBF}, util::read_file("test/fixtures/resources/fake_glyphs-0-255.pbf"));
    EXPECT_TRUE(sdfs.size() == 1);

    auto& sdf = sdfs[0];
    EXPECT_EQ(69u, sdf.id.complex.code);
