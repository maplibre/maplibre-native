...
TEST(getIconQuads, normal) {
    ...
    ImagePosition image = {
        mapbox::Bin(-1, 15, 11, 0, 0, 0, 0), style::Image::Impl("test", PremultipliedImage({1, 1}), 1.0f)};
...
TEST(getIconQuads, style) {
    ...
    const ImagePosition image = {
        mapbox::Bin(-1, 20, 20, 0, 0, 0, 0), style::Image::Impl("test", PremultipliedImage({1, 1}), 1.0f)};
