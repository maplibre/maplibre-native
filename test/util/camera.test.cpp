...
TEST(FreeCameraOptions, SetLocation) {
    ...
    options.setLocation(
        {{util::LATITUDE_MAX, 0.0}, util::EARTH_RADIUS_M * M_PI * std::cos(util::DEG2RAD_D(util::LATITUDE_MAX))});
    ASSERT_TRUE(options.position);
...
TEST(FreeCameraOptions, SetPitchBearing) {
    ...
    const double cos60 = std::cos(util::DEG2RAD_D(60.0));
    const double sin60 = std::sin(util::DEG2RAD_D(60.0));
