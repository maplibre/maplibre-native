...
TEST(Style, Properties) {
  ...
  style.loadJSON(
      R"STYLE({"transition": { "duration": 500, "delay": 50 }})STYLE");
...
TEST(Style, DuplicateSource) {
  ...
  style.loadJSON(
      util::read_file("test/fixtures/resources/style-unused-sources.json"));

  style.addSource(
      std::make_unique<VectorSource>("sourceId", "mptiler://tiles/contours"));
...
TEST(Style, SourceImplsOrder) {
  ...
  style.addSource(
      std::make_unique<VectorSource>("c", "mptiler://tiles/contours"));
  style.addSource(
      std::make_unique<VectorSource>("b", "mptiler://tiles/contours"));
  style.addSource(
      std::make_unique<VectorSource>("a", "mptiler://tiles/contours"));
...
TEST(Style, AddRemoveImage) {
  ...
  auto fileSource = std::make_shared<StubFileSource>();
  Style::Impl style{fileSource, 1.0};
  style.addImage(std::make_unique<style::Image>(
      "one", PremultipliedImage({16, 16}), 2.0f));
  style.addImage(std::make_unique<style::Image>(
      "two", PremultipliedImage({16, 16}), 2.0f));
  style.addImage(std::make_unique<style::Image>(
      "three", PremultipliedImage({16, 16}), 2.0f));
