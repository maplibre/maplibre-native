...
TEST(StyleImage, InvalidContent) {
    ...
    try {
        style::Image("test", PremultipliedImage({16, 16}), 1, {}, {}, style::ImageContent{.left=0, .top=0, .right=24, .bottom=28});
        FAIL() << "Expected exception";
    } catch (util::StyleImageException& ex) {
    ...
    try {
        style::Image("test", PremultipliedImage({16, 16}), 1, {}, {}, style::ImageContent{.left=14, .top=14, .right=12, .bottom=10});
        FAIL() << "Expected exception";
    } catch (util::StyleImageException& ex) {
        EXPECT_STREQ("content area is invalid", ex.what());
    }
    ...
    try {
        style::Image("test", PremultipliedImage({16, 16}), 1, {}, {}, style::ImageContent{.left=-2, .top=-8, .right=12, .bottom=10});
        FAIL() << "Expected exception";
    } catch (util::StyleImageException& ex) {
        EXPECT_STREQ("content area is invalid", ex.what());
    }
