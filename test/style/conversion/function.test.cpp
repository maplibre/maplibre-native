...
TEST(StyleConversion, CompositeFunctionExpression) {
    ...
    auto fn2 =
        parseFunction(R"(["coalesce", ["interpolate", ["linear"], ["zoom"], 0, ["number", ["get", "x"]], 10, 10], 0])");
    ASSERT_TRUE(fn2);
    ...
    auto fn5 =
        parseFunction(R"(["coalesce", ["interpolate", ["linear"], ["number", ["get", "x"]], 0, ["zoom"], 10, 10], 0])");
    ASSERT_FALSE(fn5);
    ASSERT_EQ(
...
TEST(StyleConversion, TokenStrings) {
    ...
    ASSERT_EQ(
        *convertTokenStringToExpression("{token} {token}"),
    ...
    ASSERT_EQ(
        *convertTokenStringToExpression("{token {token}"), *concat(vec(literal("{token "), get(literal("token"))))
    );
}
