...
public:
  ...
  TileParameters tileParameters(MapMode mapMode = MapMode::Continuous) {
    return {1.0,          MapDebugOptions(), transformState,
            fileSource,   mapMode,           annotationManager.makeWeakPtr(),
            imageManager, glyphManager,      0};
  };
...
TEST(Source, LoadingFail) {
  ...
  test.styleObserver.sourceError = [&](Source &source,
...
TEST(Source, RasterTileEmpty) {
  ...
  Immutable<LayerProperties> layerProperties =
      makeMutable<RasterLayerProperties>(
  ...
  test.renderSourceObserver.tileChanged = [&](RenderSource &source_,
...
TEST(Source, RasterDEMTileEmpty) {
  ...
  Immutable<LayerProperties> layerProperties =
      makeMutable<HillshadeLayerProperties>(
...
TEST(Source, RasterTileFail) {
  ...
  test.renderSourceObserver.tileError = [&](RenderSource &source_,
...
TEST(Source, RasterTileCancel) {
  ...
  test.renderSourceObserver.tileChanged = [&](RenderSource &,
                                              const OverscaledTileID &) {
    FAIL() << "Should never be called";
  };
...
TEST(Source, RasterTileAttribution) {
  ...
  std::string mapboxOSM = ("<a href='https://www.mapbox.com/about/maps/' "
  ...
  test.fileSource->sourceResponse = [&](const Resource &resource) {
    ...
    response.data = std::make_unique<std::string>(
        R"TILEJSON({ "tilejson": "2.1.0", "attribution": ")TILEJSON" +
...
TEST(Source, RasterDEMTileAttribution) {
  ...
  std::string mapbox = ("<a href='https://www.mapbox.com/about/maps/' "
                        "target='_blank'>&copy; Mapbox</a> ");
  ...
  test.fileSource->sourceResponse = [&](const Resource &resource) {
    ...
    Response response;
    response.data = std::make_unique<std::string>(
        R"TILEJSON({ "tilejson": "2.1.0", "attribution": ")TILEJSON" + mapbox +
...
TEST(Source, ImageSourceImageUpdate) {
  ...
  test.fileSource->response = [&](const Resource &resource) {
    ...
    Response response;
    response.data = std::make_unique<std::string>(
        util::read_file("test/fixtures/image/no_profile.png"));
    return response;
  };
...
public:
  ...
  bool
  layerPropertiesUpdated(const Immutable<style::LayerProperties> &) override {
    return true;
  }
...
public:
  ...
  void
  updateInternal(const Tileset &tileset,
                 const std::vector<Immutable<style::LayerProperties>> &layers,
                 const bool needsRendering, const bool needsRelayout,
                 const TileParameters &parameters) override {
    tilePyramid.update(layers, needsRendering, needsRelayout, parameters,
                       *baseImpl, util::tileSize_I, tileset.zoomRange,
                       tileset.bounds, [&](const OverscaledTileID &tileID) {
                         return std::make_unique<FakeTile>(*this, tileID);
                       });
  }
...
TEST(Source, InvisibleSourcesTileNecessity) {
  ...
  Immutable<LayerProperties> layerProperties = makeMutable<LineLayerProperties>(
      staticImmutableCast<LineLayer::Impl>(layer.baseImpl));
  std::vector<Immutable<LayerProperties>> layers{layerProperties};
  EXPECT_CALL(renderTilesetSource, tileSetNecessity(TileNecessity::Required))
      .Times(1);
  renderSource->update(initialized.baseImpl, layers, true, true,
                       test.tileParameters());

  // Necessity for invisible tiles must be set to `optional`.
  EXPECT_CALL(renderTilesetSource, tileSetNecessity(TileNecessity::Optional))
      .Times(1);
  renderSource->update(initialized.baseImpl, layers, false, false,
                       test.tileParameters());

  // Necessity is again `required` once tiles get back visible.
  EXPECT_CALL(renderTilesetSource, tileSetNecessity(TileNecessity::Required))
      .Times(1);
  renderSource->update(initialized.baseImpl, layers, true, false,
                       test.tileParameters());
}
...
TEST(Source, SourceMinimumUpdateInterval) {
  ...
  Duration minimumTileUpdateInterval =
      initialized.getMinimumTileUpdateInterval();
  auto baseImpl = initialized.baseImpl;
  EXPECT_EQ(Duration::zero(), minimumTileUpdateInterval);
  EXPECT_CALL(renderTilesetSource,
              tileSetMinimumUpdateInterval(minimumTileUpdateInterval))
      .Times(1);
  ...
  initialized.setMinimumTileUpdateInterval(
      Seconds(1)); // Set the same interval again.
  ...
  // No network activity for invisible tiles, and no reason to set the update
  // interval.
  EXPECT_CALL(renderTilesetSource,
              tileSetMinimumUpdateInterval(minimumTileUpdateInterval))
      .Times(0);
...
TEST(Source, RenderTileSetSourceUpdate) {
  ...
  // Check that `updateInternal()` is called even if the updated source has not
  // yet loaded description.
  ...
  renderSource->update(uninitialized.baseImpl, layers, true, true,
                       test.tileParameters());
}
...
TEST(Source, GeoJSONSourceTilesAfterDataReset) {
  ...
  RenderGeoJSONSource renderSource{
      staticImmutableCast<GeoJSONSource::Impl>(source.baseImpl)};

  CircleLayer layer("id", "source");
  Immutable<LayerProperties> layerProperties =
      makeMutable<CircleLayerProperties>(
          staticImmutableCast<CircleLayer::Impl>(layer.baseImpl));
  std::vector<Immutable<LayerProperties>> layers{layerProperties};

  static_cast<RenderSource &>(renderSource)
      .update(source.baseImpl, layers, true, true, test.tileParameters());
  ...
  source.setGeoJSONData(nullptr);
  static_cast<RenderSource &>(renderSource)
      .update(source.baseImpl, layers, true, true,
              test.tileParameters(MapMode::Static));
...
TEST(Source, SetMaxParentOverscaleFactor) {
  ...
  EXPECT_EQ(
      1u,
      log.count({EventSeverity::Warning, Event::Style, -1,
                 "Parent tile overscale factor will cap prefetch delta to 3"}));
