...
std::ostream& operator<<(std::ostream& os, const RenderTileAction& action) {
    const int64_t x = (1ul << action.tileID.canonical.z) * action.tileID.wrap + action.tileID.canonical.x;
    return os << "RenderTileAction{ { " << int(action.tileID.canonical.z) << ", " << x << ", "
              << action.tileID.canonical.y << " }, *tile_" << int(action.tileData.tileID.overscaledZ) << "_"
              << int(action.tileData.tileID.canonical.z) << "_" << action.tileData.tileID.canonical.x << "_"
...
TEST(UpdateRenderables, SingleTile) {
    ...
    // Insert another tile, and another bucket that has a different name and
...
TEST(UpdateRenderables, PreferChildTiles) {
    ...
    // Now add more children to cover the ideal tile fully, until it is covered
    // fully, and verify that the parent doesn't get rendered.
    log.clear();
