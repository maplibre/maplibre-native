...
TEST(Sprite, SpriteImageCreationInvalid) {
    ...
    ASSERT_EQ(
        nullptr, createStyleImage("test", image_1x, 0, 0, image_1x.size.width + 1, 16, 1, false)
    ...
    ASSERT_EQ(
        nullptr, createStyleImage("test", image_1x, 0, 0, 16, image_1x.size.height + 1, 1, false)
    ...
    ASSERT_EQ(
        nullptr, createStyleImage("test", image_1x, 0, 0, 16, 16, 1, false, {}, {}, style::ImageContent{14, 14, 12, 10})
    );
    EXPECT_EQ(
        1u,
        log.count({
...
TEST(Sprite, SpriteParsing) {
    ...
    std::transform(images.begin(), images.end(), std::inserter(names, names.begin()), [](const auto& image) {
        return image->id;
    });
    ...
    {
        auto& sprite = *std::find_if(images.begin(), images.end(), [](const auto& image) {
            return image->id == "generic-metro";
        });
