...
TEST(SymbolProgram, SymbolSizeBinder) {
    ...
    binder = SymbolSizeBinder::create(
        1.0f, style::PropertyExpression<float>(interpolate(linear(), zoom(), 0., literal(8.), 10., literal(18.))), 0.0f
    );
    ...
    binder = SymbolSizeBinder::create(
        0.0f, style::PropertyExpression<float>(interpolate(linear(), zoom(), 1., literal(8.), 11., literal(18.))), 0.0f
    );
    ...
    binder = SymbolSizeBinder::create(
        12.0f, style::PropertyExpression<float>(interpolate(linear(), zoom(), 1., literal(8.), 11., literal(18.))), 0.0f
    );
    ...
    binder = SymbolSizeBinder::create(
        0.0f,
        style::PropertyExpression<float>(interpolate(linear(), number(get("x")), 1., literal(8.), 11., literal(18.))),
        0.0f
    );
    uniformValues = binder->uniformValues(12.5f);
    EXPECT_EQ(uniformValues.get<uniforms::is_size_zoom_constant>(), true);
    ...
    binder = SymbolSizeBinder::create(
        5.0f,
        style::PropertyExpression<float>(interpolate(
            linear(),
            zoom(),
            1.,
            interpolate(linear(), number(get("x")), 0., literal(8.), 100., literal(18.)),
            11.,
            interpolate(linear(), number(get("x")), 0., literal(12.), 100., literal(24.9))
        )),
        0.0f
    );
