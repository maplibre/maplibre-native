...
public:
  enum class ResponseType { Asynchronous = 0, Synchronous };

  StubFileSource(const ResourceOptions &, const ClientOptions &,
                 ResponseType = ResponseType::Asynchronous);
  ...
  using ResponseFunction =
      std::function<std::optional<Response>(const Resource &)>;

  // You can set the response callback on a global level by assigning this
  // callback:
