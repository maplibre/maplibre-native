...
TEST(OfflineDatabase, TEST_REQUIRES_WRITE(Pack)) {
    ...
    for (unsigned i = 0; i < 50; ++i) {
        const Resource tile =
            Resource::tile("maptiler://tiles/tiles/tile_" + std::to_string(i), 1, 0, 0, 0, Tileset::Scheme::XYZ);
...
TEST(OfflineDatabase, MapboxTileLimitExceeded) {
    ...
    auto insertRegionTile = [&](int64_t regionID, uint64_t i) {
        const Resource tile =
            Resource::tile("maptiler://tiles/tiles/region_tile_" + std::to_string(i), 1, 0, 0, 0, Tileset::Scheme::XYZ);
...
TEST(OfflineDatabase, Invalidate) {
    ...
    const Resource ambientTile =
        Resource::tile("maptiler://tiles/tiles/tile_ambient", 1, 0, 0, 0, Tileset::Scheme::XYZ);
    db.put(ambientTile, response);
    ...
    const Resource region1Tile =
        Resource::tile("maptiler://tiles/tiles/tile_offline_region1", 1.0, 0, 0, 0, Tileset::Scheme::XYZ);
    ...
    const Resource region2Tile =
        Resource::tile("maptiler://tiles/tiles/tile_offline_region2", 1.0, 0, 0, 0, Tileset::Scheme::XYZ);
