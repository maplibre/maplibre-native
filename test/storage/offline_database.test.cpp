...
static constexpr const char *filename_sideload =
    "test/fixtures/offline_database/offline_sideload.db";
#ifndef __QT__ // Qt doesn't expose the ability to register virtual file system
               // handlers.
static constexpr const char *filename_test_fs =
    "file:test/fixtures/offline_database/offline.db?vfs=test_fs";
#endif
...
static FixtureLog::Message error(ResultCode code, const char *message) {
  return {EventSeverity::Error, Event::Database, static_cast<int64_t>(code),
          message};
}

static MBGL_UNUSED FixtureLog::Message warning(ResultCode code,
                                               const char *message) {
  return {EventSeverity::Warning, Event::Database, static_cast<int64_t>(code),
          message};
}

static int databasePageCount(const std::string &path) {
  mapbox::sqlite::Database db =
      mapbox::sqlite::Database::open(path, mapbox::sqlite::ReadOnly);
...
static std::vector<std::string> databaseTableColumns(const std::string &path,
...
const Resource tile =
    Resource::tile("maptiler://test", 1, 0, 0, 0, Tileset::Scheme::XYZ);
...
TEST(OfflineDatabase, TEST_REQUIRES_WRITE(CreateFail)) {
  ...
  for (const auto &res : {fixture::resource, fixture::tile}) {
    EXPECT_EQ(std::make_pair(false, uint64_t(0)),
              db.put(res, fixture::response));
    EXPECT_EQ(1u, log.count(warning(
                      ResultCode::CantOpen,
                      "Can't write resource: unable to open database file")));
    EXPECT_EQ(0u, log.uncheckedCount());
  }
  ...
  for (const auto &res : {fixture::resource, fixture::tile}) {
    ...
    EXPECT_EQ(1u, log.count(warning(
                      ResultCode::CantOpen,
                      "Can't update timestamp: unable to open database file")));
    EXPECT_EQ(1u, log.count(warning(
                      ResultCode::CantOpen,
                      "Can't read resource: unable to open database file")));
    EXPECT_EQ(0u, log.uncheckedCount());
  }

  // Now, we're "freeing up" some space on the disk, and try to insert and query
  // again. This time, we should be opening the datbase, creating the schema,
  // and writing the data so that we can retrieve it again.
  fs.allowFileCreate(true);
  for (const auto &res : {fixture::resource, fixture::tile}) {
    EXPECT_EQ(std::make_pair(true, uint64_t(5)),
              db.put(res, fixture::response));
  ...
  // Next, set the file system to read only mode and try to read the data again.
  // While we can't write anymore, we should still be able to read, and the
  // query that tries to update the last accessed timestamp may fail without
  // crashing.
  fs.allowFileCreate(false);
  fs.setWriteLimit(0);
  for (const auto &res : {fixture::resource, fixture::tile}) {
  ...
  // We're allowing SQLite to create a journal file, but restrict the number of
  // bytes it can write so that it can start writing the journal file, but
  ...
  for (const auto &res : {fixture::resource, fixture::tile}) {
    ...
    EXPECT_EQ(1u, log.count(warning(
                      ResultCode::Full,
                      "Can't update timestamp: database or disk is full")));
    EXPECT_EQ(0u, log.uncheckedCount());
    ASSERT_TRUE(result && result->data);
    EXPECT_EQ("first", *result->data);
  }
  ...
  for (const auto &res : {fixture::resource, fixture::tile}) {
    ...
    auto result = db.get(res);
    EXPECT_EQ(
        1u, log.count(warning(ResultCode::Auth,
                              "Can't update timestamp: authorization denied")));
    EXPECT_EQ(1u,
              log.count(warning(ResultCode::Auth,
                                "Can't read resource: authorization denied")));
    EXPECT_EQ(0u, log.uncheckedCount());
    EXPECT_FALSE(result);
    ...
    EXPECT_EQ(1u,
              log.count(warning(ResultCode::Auth,
                                "Can't write resource: authorization denied")));
    EXPECT_EQ(0u, log.uncheckedCount());
  }
...
TEST(OfflineDatabase, TEST_REQUIRES_WRITE(SchemaVersion)) {
  ...
  {
    mapbox::sqlite::Database db = mapbox::sqlite::Database::open(
        filename, mapbox::sqlite::ReadWriteCreate);
...
TEST(OfflineDatabase, TEST_REQUIRES_WRITE(Invalid)) {
  ...
  EXPECT_EQ(
      1u,
      log.count(
          error(ResultCode::NotADB,
                "Can't open database: file is encrypted or is not a database"),
          true) +
          log.count(error(ResultCode::NotADB,
                          "Can't open database: file is not a database"),
                    true));
  EXPECT_EQ(1u, log.count({EventSeverity::Warning, Event::Database, -1,
                           "Removing existing incompatible offline database"}));
...
TEST(OfflineDatabase, PutDoesNotStoreServerErrors) {
  ...
  Response response;
  response.error =
      std::make_unique<Response::Error>(Response::Error::Reason::Server);
...
// https://github.com/mapbox/mapbox-gl-native/issues/14966
TEST(OfflineDatabase, TEST_REQUIRES_WRITE(DISABLED_MaximumAmbientCacheSize)) {
  FixtureLog log;
  deleteDatabaseFiles();

  auto databaseSize = [] { return util::read_file(filename).size(); };
...

  {
    ...
    for (unsigned i = 0; i < 250; ++i) {
      const Resource ambientTile = Resource::tile(
          "maptiler://tiles/tiles/ambient_tile_" + std::to_string(i), 1, 0, 0,
          0, Tileset::Scheme::XYZ);
      db.put(ambientTile, response);

      const Resource ambientStyle =
          Resource::style("maptiler://maps/ambient_style_" + std::to_string(i));
      db.put(ambientStyle, response);

      const Resource regionTile = Resource::tile(
          "maptiler://tiles/tiles/region_tile_" + std::to_string(i), 1, 0, 0, 0,
          Tileset::Scheme::XYZ);
      db.putRegionResource(region->getID(), regionTile, response);

      const Resource regionStyle =
          Resource::style("maptiler://maps/region_style_" + std::to_string(i));
...

  {
    ...
    for (unsigned i = 0; i < 250; ++i) {
      ...
      const Resource ambientStyle =
          Resource::style("maptiler://mas/ambient_style_" + std::to_string(i));
      db.put(ambientStyle, response);
    }
  }
...
namespace {
std::list<std::tuple<Resource, Response>>
generateResources(const std::string &tilePrefix,
  ...
  for (unsigned i = 0; i < 50; ++i) {
    resources.emplace_back(Resource::tile(tilePrefix + std::to_string(i), 1, 0,
                                          0, 0, Tileset::Scheme::XYZ),
                           response);
    resources.emplace_back(Resource::style(stylePrefix + std::to_string(i)),
                           response);
  }
  return resources;
}
...
TEST(OfflineDatabase, TEST_REQUIRES_WRITE(DeleteRegion)) {
...

  {
    ...
    db.putRegionResources(region1->getID(),
                          generateResources("maptiler://tiles/tiles/tile_1",
                                            "maptiler://maps/style_1"),
                          status);
    db.putRegionResources(region2->getID(),
                          generateResources("maptiler://tiles/tiles/tile_2",
                                            "maptiler://maps/style_2"),
                          status);
...
TEST(OfflineDatabase, TEST_REQUIRES_WRITE(Pack)) {
  ...
  for (unsigned i = 0; i < 50; ++i) {
    const Resource tile =
        Resource::tile("maptiler://tiles/tiles/tile_" + std::to_string(i), 1, 0,
                       0, 0, Tileset::Scheme::XYZ);
    db.put(tile, response);

    const Resource style =
        Resource::style("maptiler://maps/style_" + std::to_string(i));
...
TEST(OfflineDatabase, MapboxTileLimitExceeded) {
  ...
  auto insertAmbientTile = [&](unsigned i) {
    const Resource ambientTile = Resource::tile(
        "maptiler://tiles/tiles/ambient_tile_" + std::to_string(i), 1, 0, 0, 0,
        Tileset::Scheme::XYZ);
    db.put(ambientTile, response);
  };

  auto insertRegionTile = [&](int64_t regionID, uint64_t i) {
    const Resource tile = Resource::tile("maptiler://tiles/tiles/region_tile_" +
                                             std::to_string(i),
                                         1, 0, 0, 0, Tileset::Scheme::XYZ);
    db.putRegionResource(regionID, tile, response);
  };

  OfflineTilePyramidRegionDefinition definition1{
      "maptiler://maps/style1",
      LatLngBounds::hull({1, 2}, {3, 4}),
      5,
      6,
      2.0,
  ...
  // Full.
  ASSERT_THROW(insertRegionTile(region1->getID(), 200),
               MapboxTileLimitExceededException);
  ASSERT_THROW(insertRegionTile(region2->getID(), 201),
               MapboxTileLimitExceededException);
  ...
  const Resource notMapboxTile =
      Resource::tile("foobar://region_tile", 1, 0, 0, 0, Tileset::Scheme::XYZ);
  ...
  // Full again.
  ASSERT_THROW(insertRegionTile(region1->getID(), 202),
               MapboxTileLimitExceededException);
...
TEST(OfflineDatabase, Invalidate) {
  ...
  const Resource ambientStyle =
      Resource::style("maptiler://maps/style_ambient");
  db.put(ambientStyle, response);
  ...
  auto region1 = db.createRegion(definition, metadata);
  const Resource region1Tile =
      Resource::tile("maptiler://tiles/tiles/tile_offline_region1", 1.0, 0, 0,
                     0, Tileset::Scheme::XYZ);
  db.putRegionResource(region1->getID(), region1Tile, response);

  const Resource region1Style =
      Resource::style("maptiler://maps/style_offline_region1");
  db.putRegionResource(region1->getID(), region1Style, response);

  auto region2 = db.createRegion(definition, metadata);
  const Resource region2Tile =
      Resource::tile("maptiler://tiles/tiles/tile_offline_region2", 1.0, 0, 0,
                     0, Tileset::Scheme::XYZ);
  db.putRegionResource(region2->getID(), region2Tile, response);

  const Resource region2Style =
      Resource::style("maptiler://maps/style_offline_region2");
...
TEST(OfflineDatabase, TEST_REQUIRES_WRITE(ClearAmbientCache)) {
...

  {
    ...
    for (unsigned i = 0; i < 50; ++i) {
      const Resource tile =
          Resource::tile("maptiler://tiles/tiles/tile_" + std::to_string(i), 1,
                         0, 0, 0, Tileset::Scheme::XYZ);
      db.put(tile, response);
...
TEST(OfflineDatabase, CreateRegionInfiniteMaxZoom) {
  ...
  OfflineDatabase db(":memory:", fixture::tileServerOptions);
  OfflineTilePyramidRegionDefinition definition{
      "", LatLngBounds::world(), 0, INFINITY, 1.0, false};
...
TEST(OfflineDatabase, PutReturnsSize) {
  ...
  EXPECT_EQ(17u, db.put(Resource::style("http://example.com/compressible"),
                        compressible)
                     .second);

  Response incompressible;
  incompressible.data = randomString(1024);
  EXPECT_EQ(1024u, db.put(Resource::style("http://example.com/incompressible"),
                          incompressible)
                       .second);
  ...
  EXPECT_EQ(0u,
            db.put(Resource::style("http://example.com/noContent"), noContent)
                .second);
...
TEST(OfflineDatabase, PutEvictsLeastRecentlyUsedResources) {
  ...
  for (uint32_t i = 1; i <= 101; ++i) {
    Resource resource =
        Resource::style("http://example.com/"s + util::toString(i));
    db.put(resource, response);
...
TEST(OfflineDatabase, OfflineRegionDoesNotAffectAmbientCacheSize) {
  ...
  db.setMaximumAmbientCacheSize(dataSize * numberOfCachedResources +
                                pageOverhead);
  ...
  OfflineTilePyramidRegionDefinition definition("http://example.com/style.json",
                                                LatLngBounds::world(), 0.0, 0.0,
                                                1.0, false);
  ...
  EXPECT_TRUE(
      bool(db.get(Resource::style("http://example.com/ambient1.json"s))));
  EXPECT_TRUE(
      bool(db.get(Resource::style("http://example.com/ambient2.json"s))));
  EXPECT_EQ(0u, log.uncheckedCount());
}
...
TEST(OfflineDatabase, PutRegionResourceDoesNotEvict) {
  ...
  OfflineTilePyramidRegionDefinition definition{
      "", LatLngBounds::world(), 0, INFINITY, 1.0, true};
  auto region = db.createRegion(definition, OfflineRegionMetadata());
  ASSERT_TRUE(region);
  ...
  for (uint32_t i = 1; i <= 100; i++) {
    db.putRegionResource(
        region->getID(),
        Resource::style("http://example.com/"s + util::toString(i)), response);
  }
...
TEST(OfflineDatabase, PutFailsWhenEvictionInsuffices) {
  ...
  EXPECT_EQ(1u, log.count({EventSeverity::Info, Event::Database, -1,
...
TEST(OfflineDatabase, GetRegionCompletedStatus) {
  ...
  uint64_t styleSize = db.putRegionResource(
      region->getID(), Resource::style("http://example.com/"), response);
  ...
  uint64_t tileSize = db.putRegionResource(
      region->getID(),
      Resource::tile("http://example.com/", 1.0, 0, 0, 0, Tileset::Scheme::XYZ),
      response);
...
TEST(OfflineDatabase, HasRegionResource) {
  ...
  EXPECT_FALSE(
      bool(db.hasRegionResource(Resource::style("http://example.com/1"))));
  EXPECT_FALSE(
      bool(db.hasRegionResource(Resource::style("http://example.com/20"))));
  ...
  EXPECT_TRUE(
      bool(db.hasRegionResource(Resource::style("http://example.com/20"))));
  EXPECT_EQ(1024,
            *(db.hasRegionResource(Resource::style("http://example.com/20"))));
...
TEST(OfflineDatabase, OfflineMapboxTileCount) {
  ...
  Resource nonMapboxTile =
      Resource::tile("http://example.com/", 1.0, 0, 0, 0, Tileset::Scheme::XYZ);
  Resource mapboxTile1 = Resource::tile("maptiler://tiles/tiles/1", 1.0, 0, 0,
                                        0, Tileset::Scheme::XYZ);
  Resource mapboxTile2 = Resource::tile("maptiler://tiles/tiles/2", 1.0, 0, 0,
                                        1, Tileset::Scheme::XYZ);
  ...
  db.putRegionResource(region1->getID(), Resource::style("http://example.com/"),
                       response);
  EXPECT_EQ(0u, db.getOfflineMapboxTileCount());
  ...
  // Count stays the same after deleting a region when the tile is still used by
  // another region.
  db.deleteRegion(std::move(*region2));
  EXPECT_EQ(1u, db.getOfflineMapboxTileCount());
  ...
  // Count decreases after deleting a region when the tiles are not used by
  // other regions.
  db.deleteRegion(std::move(*region1));
  EXPECT_EQ(0u, db.getOfflineMapboxTileCount());
...
TEST(OfflineDatabase, BatchInsertion) {
  ...
  for (uint32_t i = 1; i <= 100; i++) {
    EXPECT_TRUE(bool(
        db.get(Resource::style("http://example.com/"s + util::toString(i)))));
  }
...
TEST(OfflineDatabase, BatchInsertionMapboxTileCountExceeded) {
  ...
  resources.emplace_back(
      Resource::tile("maptiler://tiles/1", 1.0, 0, 0, 0, Tileset::Scheme::XYZ),
      response);
  resources.emplace_back(
      Resource::tile("maptiler://tiles/2", 1.0, 0, 0, 0, Tileset::Scheme::XYZ),
      response);
  ...
  const auto completedStatus =
      db.getRegionCompletedStatus(region->getID()).value();
...
TEST(OfflineDatabase, MigrateFromV2Schema) {
  // v2.db is a v2 database containing a single offline region with a small
  // number of resources.
  FixtureLog log;
  deleteDatabaseFiles();
...
TEST(OfflineDatabase, MigrateFromV5Schema) {
  ...
  EXPECT_EQ(
      (std::vector<std::string>{"id", "url_template", "pixel_ratio", "z", "x",
                                "y", "expires", "modified", "etag", "data",
                                "compressed", "accessed", "must_revalidate"}),
      databaseTableColumns(filename, "tiles"));
  EXPECT_EQ((std::vector<std::string>{"id", "url", "kind", "expires",
                                      "modified", "etag", "data", "compressed",
...
TEST(OfflineDatabase, CorruptDatabaseOnOpen) {
  FixtureLog log;
  util::deleteFile(filename);
  util::copyFile(filename,
                 "test/fixtures/offline_database/corrupt-immediate.db");
  ...
  EXPECT_EQ(
      ...
      log.count(error(ResultCode::Corrupt,
                      "Can't open database: database disk image is malformed"),
                true));
  EXPECT_EQ(1u, log.count({EventSeverity::Warning, Event::Database, -1,
                           "Removing existing incompatible offline database"}));
  EXPECT_EQ(0u, log.uncheckedCount());
...
TEST(OfflineDatabase, CorruptDatabaseOnQuery) {
  ...
  // This database is corrupt in a way that won't manifest itself until we start
  // querying it, so just opening it will not cause an error.
  OfflineDatabase db(filename, fixture::tileServerOptions);
  ...
  // The first request fails because the database is corrupt and has to be
  // recreated.
  EXPECT_EQ(std::nullopt, db.get(fixture::tile));
  EXPECT_EQ(
      1u,
      log.count(error(ResultCode::Corrupt,
                      "Can't read resource: database disk image is malformed"),
                true));
  EXPECT_EQ(1u, log.count({EventSeverity::Warning, Event::Database, -1,
                           "Removing existing incompatible offline database"}));
  EXPECT_EQ(0u, log.uncheckedCount());
...
TEST(OfflineDatabase, TEST_REQUIRES_WRITE(DisallowedIO)) {
  ...
  OfflineTilePyramidRegionDefinition definition(
      "maptiler://maps/style",
      LatLngBounds::hull({37.66, -122.57}, {37.83, -122.32}), 0, 8, 2, false);
  auto region = db.createRegion(definition, {});
  ASSERT_TRUE(region);

  // Now forbid any type of IO on the database and test that none of the calls
  // crashes.
  fs.allowIO(false);
  ...
  EXPECT_EQ(std::make_pair(false, uint64_t(0)),
            db.put(fixture::resource, fixture::response));
  EXPECT_EQ(1u,
            log.count(warning(ResultCode::Auth,
                              "Can't write resource: authorization denied")));
  EXPECT_EQ(0u, log.uncheckedCount());
  ...
  EXPECT_EQ(1u, log.count(warning(ResultCode::Auth,
                                  "Can't list regions: authorization denied")));
  EXPECT_EQ(0u, log.uncheckedCount());

  EXPECT_FALSE(db.createRegion(definition, {}));
  EXPECT_EQ(1u,
            log.count(warning(ResultCode::Auth,
                              "Can't create region: authorization denied")));
  EXPECT_EQ(0u, log.uncheckedCount());

  EXPECT_FALSE(db.updateMetadata(region->getID(), {}));
  EXPECT_EQ(1u, log.count(warning(
                    ResultCode::Auth,
                    "Can't update region metadata: authorization denied")));
  EXPECT_EQ(0u, log.uncheckedCount());
  ...
  EXPECT_EQ(1u, log.count(warning(
                    ResultCode::Auth,
                    "Can't read region resource: authorization denied")));
  EXPECT_EQ(0u, log.uncheckedCount());

  EXPECT_EQ(std::nullopt, db.hasRegionResource(fixture::resource));
  EXPECT_EQ(1u, log.count(warning(
                    ResultCode::Auth,
                    "Can't query region resource: authorization denied")));
  EXPECT_EQ(0u, log.uncheckedCount());

  EXPECT_EQ(0u, db.putRegionResource(region->getID(), fixture::resource,
                                     fixture::response));
  EXPECT_EQ(1u, log.count(warning(
                    ResultCode::Auth,
                    "Can't write region resource: authorization denied")));
  EXPECT_EQ(0u, log.uncheckedCount());

  OfflineRegionStatus status;
  db.putRegionResources(region->getID(),
                        {std::make_tuple(fixture::resource, fixture::response)},
                        status);
  EXPECT_EQ(1u, log.count(warning(
                    ResultCode::Auth,
                    "Can't write region resources: authorization denied")));
  EXPECT_EQ(0u, log.uncheckedCount());

  EXPECT_FALSE(db.getRegionDefinition(region->getID()));
  EXPECT_EQ(1u, log.count(warning(ResultCode::Auth,
                                  "Can't load region: authorization denied")));
  EXPECT_EQ(0u, log.uncheckedCount());

  EXPECT_FALSE(db.getRegionCompletedStatus(region->getID()));
  EXPECT_EQ(
      1u, log.count(warning(ResultCode::Auth,
                            "Can't get region status: authorization denied")));
  EXPECT_EQ(0u, log.uncheckedCount());

  EXPECT_TRUE(db.deleteRegion(std::move(*region)));
  EXPECT_EQ(1u,
            log.count(warning(ResultCode::Auth,
                              "Can't delete region: authorization denied")));
  EXPECT_EQ(0u, log.uncheckedCount());

  EXPECT_EQ(std::numeric_limits<uint64_t>::max(),
            db.getOfflineMapboxTileCount());
  EXPECT_EQ(1u,
            log.count(warning(
                ResultCode::Auth,
                "Can't get offline Mapbox tile count: authorization denied")));
  EXPECT_EQ(0u, log.uncheckedCount());
...
TEST(OfflineDatabase, MergeDatabaseWithSingleRegion_New) {
  ...
  util::copyFile(filename_sideload,
                 "test/fixtures/offline_database/sideload_sat.db");
...
TEST(OfflineDatabase,
     TEST_REQUIRES_WRITE(MergeDatabaseWithSingleRegion_Update)) {
  deleteDatabaseFiles();
  util::deleteFile(filename_sideload);
  util::copyFile(filename, "test/fixtures/offline_database/satellite_test.db");
  util::copyFile(filename_sideload,
  ...
  {
    std::string tileURL =
        "maptiler://tiles/tiles/satellite/{z}/{x}/{y}{ratio}.jpg";
...
TEST(OfflineDatabase, MergeDatabaseWithSingleRegion_NoUpdate) {
  ...
  // Swap sideload/main database from update test and ensure that an older tile
  // is not copied over
  util::copyFile(filename_sideload,
                 "test/fixtures/offline_database/satellite_test.db");
  ...
  std::string tileURL =
      "maptiler://tiles/tiles/satellite/{z}/{x}/{y}{ratio}.jpg";
  auto originalTile = db.getRegionResource(
      Resource::tile(tileURL, 1, 0, 0, 1, Tileset::Scheme::XYZ));
  auto originalStamp = originalTile->first.modified;
...
TEST(OfflineDatabase, MergeDatabaseWithSingleRegion_AmbientTiles) {
  util::deleteFile(filename_sideload);
  util::copyFile(filename_sideload,
                 "test/fixtures/offline_database/sideload_ambient.db");

  OfflineDatabase db(":memory:", fixture::tileServerOptions);
  auto result = db.mergeDatabase(filename_sideload);

  EXPECT_TRUE(bool(db.hasRegionResource(
      Resource::tile("maptiler://tiles/tiles/satellite/{z}/{x}/{y}{ratio}.jpg",
                     1, 0, 0, 1, Tileset::Scheme::XYZ))));

  // Ambient resources should not be copied
  EXPECT_FALSE(
      bool(db.hasRegionResource(Resource::style("maptiler://maps/streets"))));
  EXPECT_FALSE(bool(db.hasRegionResource(
      Resource::tile("maptiler://tiles/tiles/satellite/{z}/{x}/{y}{ratio}.jpg",
                     1, 0, 1, 2, Tileset::Scheme::XYZ))));
  EXPECT_FALSE(bool(db.hasRegionResource(
      Resource::tile("maptiler://tiles/tiles/satellite/{z}/{x}/{y}{ratio}.jpg",
                     1, 1, 1, 2, Tileset::Scheme::XYZ))));
}

TEST(OfflineDatabase, MergeDatabaseWithMultipleRegions_New) {
  util::deleteFile(filename_sideload);
  util::copyFile(filename_sideload,
                 "test/fixtures/offline_database/sideload_sat_multiple.db");
...
TEST(OfflineDatabase, MergeDatabaseWithMultipleRegionsWithOverlap) {
  ...
  mapbox::sqlite::Database db =
      mapbox::sqlite::Database::open(filename, mapbox::sqlite::ReadOnly);
  {
    // clang-format off
...
TEST(OfflineDatabase, MergeDatabaseWithSingleRegionTooManyNewTiles) {
  ...
  auto result = db.mergeDatabase(filename_sideload);
  EXPECT_FALSE(result);
  EXPECT_EQ(1u, log.count({EventSeverity::Error, Event::Database, -1,
                           "Mapbox tile limit exceeded"}));
  EXPECT_EQ(0u, log.uncheckedCount());
}
...
TEST(OfflineDatabase, MergeDatabaseWithSingleRegionTooManyExistingTiles) {
  ...
  auto result = db.mergeDatabase(filename_sideload);
  EXPECT_THROW(std::rethrow_exception(result.error()),
               MapboxTileLimitExceededException);
...
TEST(OfflineDatabase, MergeDatabaseWithInvalidPath) {
  ...
  EXPECT_EQ(1u, log.count({EventSeverity::Error, Event::Database, -1,
                           "Merge database has incorrect user_version"}));
  EXPECT_EQ(0u, log.uncheckedCount());
}
#endif
...
TEST(OfflineDatabase, MergeDatabaseWithInvalidDb) {
  ...
  EXPECT_EQ(1u, log.count(error(
                    ResultCode::Corrupt,
                    "Can't attach database "
                    "(test/fixtures/offline_database/offline_sideload.db) for "
                    "merge: database disk image is malformed")));
  EXPECT_EQ(0u, log.uncheckedCount());
}
...
TEST(OfflineDatabase, TEST_REQUIRES_WRITE(UpdateDatabaseReadOnlyMode)) {
  ...
  db.clearAmbientCache();
  EXPECT_EQ(1u, log.count({EventSeverity::Error, Event::Database, -1,
                           "Can't clear ambient cache: Cannot modify database "
                           "in read-only mode"}));
