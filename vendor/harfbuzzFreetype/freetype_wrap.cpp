...
FreeTypeFaceWrap::FreeTypeFaceWrap(const std::string &fontFileName, const FreeTypeLibrary &lib) {
...
FreeTypeFaceWrap::FreeTypeFaceWrap(const char *fontData, size_t fontDataSize, const FreeTypeLibrary &lib) {
...
FreeTypeFaceWrap::~FreeTypeFaceWrap() {
    if (valid) FT_Done_Face(face);
}

int FreeTypeFaceWrap::force_ucs2_charmap(FT_Face ftf) {
...
void FreeTypeFaceWrap::rasterizeGlyph(char16_t glyph, GlyphCallBack callback) {
//    Glyph fixedMetrics;

    FT_Int32 flags = FT_LOAD_DEFAULT;

    FT_Load_Glyph(face,
                  (FT_UInt)glyph, // the glyph_index in the font file
                  flags);
